...
BOOST_AUTO_TEST_CASE(transformation) {
  ...
  space.Init(*mesh, fe_ansatz, is_cg, fe_params, hiflow::doffem::HIFLOW_CLASSIC);
  ...
  for (EntityIterator it = mesh->begin(gdim); it != mesh->end(gdim); ++it) {
    ...
    for (int vtx = 0; vtx < it->num_vertices(); ++vtx) {
      ...
      if (gdim == 2) {
        tmp[0] = space.fe_manager().get_cell_transformation(it->index())->x(tmp0);
        tmp[1] = space.fe_manager().get_cell_transformation(it->index())->y(tmp0);
        cv_bk[vtx * gdim] = tmp[0];
        cv_bk[vtx * gdim + 1] = tmp[1];
      } else {
        tmp[0] = space.fe_manager().get_cell_transformation(it->index())->x(tmp0);
        tmp[1] = space.fe_manager().get_cell_transformation(it->index())->y(tmp0);
        tmp[2] = space.fe_manager().get_cell_transformation(it->index())->z(tmp0);
        cv_bk[vtx * gdim] = tmp[0];
        cv_bk[vtx * gdim + 1] = tmp[1];
