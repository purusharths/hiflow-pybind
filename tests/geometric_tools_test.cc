...
public:
  ...
  {
    Vec<DIM, double> grad;
    grad[0] = -2.0 * x[0] / (_a * _a);
    grad[1] = -2.0 * x[1] / (_b * _b);
    if (DIM == 3)
      grad[2] = -2.0 * x[2] / (_c * _c);
    return grad;
  }
...
void test_vertices_inside_one_hyperplane()
{
  ...
  BOOST_TEST((vertices_inside_one_hyperplane<double, 3>(coords_plane_samples, 3)));
  BOOST_TEST(!(vertices_inside_one_hyperplane<double, 3>(coords_noplane_samples, 3)));
}
...
void map_ref_coord_to_other_cell()
{
  ...
  doffem::ConstRefCellPtr<double, 2> cell(new doffem::RefCellTriStd<double, 2>());
  doffem::CellTrafoPtr<double, 2> trafo_ref(
      new doffem::LinearTriangleTransformation<double, 2>(cell));
  trafo_ref->reinit(coords_ref);
  doffem::CellTrafoPtr<double, 2> trafo_other(
      new doffem::LinearTriangleTransformation<double, 2>(cell));
...
void test_cells_intersect()
{
  ...
  std::vector<double> coords1d_out_inters = { -3.9, -8.4 };
  std::vector<double> coords1d_in_notinters = { 1.4, 0.6 };
  std::vector<double> coords1d_out_notinters = { 3.4, 6.8 };
  BOOST_TEST((mesh::cells_intersect<double, 1>(coords1d_in_inters, coords1d_out_inters, 0)));
  BOOST_TEST(!(mesh::cells_intersect<double, 1>(coords1d_in_notinters, coords1d_out_notinters, 0)));
  ...
  BOOST_TEST((mesh::cells_intersect<double, 2>(coords2d_in_inters, coords2d_out_inters, 10)));
  BOOST_TEST(!(mesh::cells_intersect<double, 2>(coords2d_in_notinters, coords2d_out_notinters, 10)));

  std::vector<double> coords2d_pyramid = {
  ...
  BOOST_TEST((mesh::cells_intersect<double, 3>(coords2d_pyramid, coords2d_tetra_inters, 10)));
  BOOST_TEST(!(mesh::cells_intersect<double, 3>(coords2d_pyramid, coords2d_hexa_notinters, 10)));
}
