...
class JumpTest {
public:
  JumpTest(const std::string &param_filename, const std::string &path_mesh)
      : path_mesh(path_mesh), comm_(MPI_COMM_WORLD), rank_(-1),
        num_partitions_(-1), is_done_(false), refinement_level_(0),
    ...
    if (num_partitions_ > 1) {
      std::cerr << "Test can only be run sequentially!" << std::endl;
      // exit ( -1 );
    }
#endif
  }
...
private:
  ...
  HpFemAssembler< double, DIMENSION > global_asm_;
  DGGlobalAssembler< double, DIMENSION > jump_term_asm_;
...
// Program entry point
BOOST_AUTO_TEST_CASE(parallel_adaptive_jump_term) {
  ...
  try {
    ...
    if (DIMENSION == 1) {
      std::cout << "Combination of MeshP4est and DIMENSION==1 does not work"
                << std::endl;
    }
#endif
    app.run();
...
void JumpTest::build_initial_mesh() {
  ...
  if (rank_ == MASTER_RANK) {
    ...
    master_mesh_ = read_mesh_from_file(mesh_filename, DIMENSION, DIMENSION, 0,
                                       mesh::IMPL_P4EST, period, false);
#else
    master_mesh_ = read_mesh_from_file(mesh_filename, DIMENSION, DIMENSION, 0,
                                       mesh::IMPL_DBVIEW, period, false);
#endif
    assert(master_mesh_ != 0);
...
#else
  mesh_ = partition_and_distribute(master_mesh_, MASTER_RANK, comm_,
                                   uniform_ref_steps, mesh::IMPL_DBVIEW);
...
void JumpTest::prepare_system() {
  ...
  //Lagrange element
  const std::vector < FEType > fe_ansatz(1, FE_TYPE_LAGRANGE);
  
  //DG Ansatz function
  const std::vector < bool > is_cg(1, false);
  
  
  // Initialize the VectorSpace object.
  space_.clear();
  space_.Init(*mesh_ptr, fe_ansatz, is_cg, degrees);
  ...
  LOG_DEBUG(2, "[" << rank_ << "] sparsity.diagonal_cols = "
                   << string_from_range(sparsity.diagonal_cols.begin(),
                                        sparsity.diagonal_cols.end()));
  LOG_DEBUG(2, "[" << rank_ << "] sparsity.diagonal_rows = "
                   << string_from_range(sparsity.diagonal_rows.begin(),
                                        sparsity.diagonal_rows.end()));
  LOG_DEBUG(2, "[" << rank_ << "] sparsity.off_diagonal_cols = "
                   << string_from_range(sparsity.off_diagonal_cols.begin(),
                                        sparsity.off_diagonal_cols.end()));
  LOG_DEBUG(2, "[" << rank_ << "] sparsity.off_diagonal_rows = "
                   << string_from_range(sparsity.off_diagonal_rows.begin(),
                                        sparsity.off_diagonal_rows.end()));
  LOG_DEBUG(2, "[" << rank_ << "] local dofs = " << sol_->size_local()
                   << ", ghost size = " << sol_->size_local_ghost());
}
...
void JumpTest::assemble_jump_terms() {
  ...
  // ***************************************************************
  // set dofs
  for (mesh::EntityIterator it = mesh_ptr->begin(DIMENSION),
                            end_it = mesh_ptr->end(DIMENSION);
       it != end_it; ++it) {
    std::vector< int > global_dof_ids;
    //TODO: 2nd arguemtn correct?
    space_.get_dof_indices(0, it->index(), &global_dof_ids);
    ...
    for (int i = 0; i < num_dofs; i++) {
      if (rank_ == space_.dof().owner_of_dof(global_dof_ids.at(i))) {
        ...
        LOG_DEBUG(2, "dof " << global_dof_ids.at(i) << " has coord " << x
                            << ", " << y);
      }
    }
  }
  ...
  for (mesh::EntityIterator it = mesh_ptr->begin(DIMENSION),
    ...
    std::vector< Coord > coords;
    space_.dof().aaa_get_coord_on_cell(0, it->index(), coords);
    for (int i = 0; i < num_dofs; i++) {
      double x = coords[i][0];
      double y = coords[i][1];
      double val = 0.;
  ...
  for (InterfaceList::const_iterator it = if_list.begin(),
    ...
    mesh_ptr->get_attribute_value("_remote_index_", mesh_ptr->tdim(),
  ...
  jump_term_asm_.assemble_interface_scalar_cells(space_, rho_jump_int,
                                                 rho_jump_);
  ...
  double total_rho_jump =
      std::accumulate(rho_jump_.begin(), rho_jump_.end(), 0.);
  double global_rho_jump = 0.;
  MPI_Allreduce(&total_rho_jump, &global_rho_jump, 1, MPI_DOUBLE, MPI_SUM,
                comm_);
  int local_size_E = if_list.size() - Number_Boundary;
  int global_size_E = 0;
  MPI_Allreduce(&local_size_E, &global_size_E, 1, MPI_DOUBLE, MPI_SUM, comm_);

  LOG_INFO("rho_E", "partition "
                        << rank_ << ": size = " << local_size_E
                        << ", local value = " << std::sqrt(total_rho_jump));
  if (rank_ == MASTER_RANK) {
    LOG_INFO("rho_E", "global size = " << global_size_E << ", global value = "
...
void JumpTest::visualize() {
  ...
  //TODO CellVisualization instead of ParallelCellVisualisation?
  CellVisualization< double, DIMENSION > visu(space_, num_intervals);

  // Generate filename.
  std::stringstream name;
  name << "solution" << refinement_level_;

  std::vector< double > remote_index(mesh_ptr->num_entities(mesh_ptr->tdim()),
                                     0.);
  std::vector< double > sub_domain(mesh_ptr->num_entities(mesh_ptr->tdim()),
                                   0.);
  std::vector< double > material_number(
      mesh_ptr->num_entities(mesh_ptr->tdim()), 0.);

  std::vector< double > local_index(mesh_ptr->num_entities(mesh_ptr->tdim()),
                                    -1.);

  for (mesh::EntityIterator it = mesh_ptr->begin(mesh_ptr->tdim());
       it != mesh_ptr->end(mesh_ptr->tdim()); ++it) {
    int temp1, temp2, temp3;
    mesh_ptr->get_attribute_value("_remote_index_", mesh_ptr->tdim(),
                                  it->index(), &temp1);
    mesh_ptr->get_attribute_value("_sub_domain_", mesh_ptr->tdim(), it->index(),
                                  &temp2);

    mesh_ptr->get_attribute_value("_local_index_", mesh_ptr->tdim(),
                                  it->index(), &temp3);
    ...
    material_number.at(it->index()) =
        mesh_ptr->get_material_number(mesh_ptr->tdim(), it->index());
  }

  std::vector<std::string> names (1, "u");
  visu.visualize(FeEvalCellWise< double, DIMENSION >(space_, *(sol_)), names);
