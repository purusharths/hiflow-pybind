...
struct TrafoProperties {
  ...
  Mat<DIM, DIM, DataType> J0;
  Mat<DIM, DIM, DataType> H[DIM];
};
...
template<int DIM>
bool mat_equal_eps(Mat<DIM, DIM, double> m1, Mat<DIM, DIM, double> m2, double eps)
{
  for (int i = 0; i < DIM; ++i) {
    for (int j = 0; j < DIM; ++j) {
...
template<int DIM>
void init_linear_line_properties(struct TrafoProperties<double, DIM>& props)
{
  if (DIM == 1) {
...
template<int DIM>
void init_linear_triangle_properties(struct TrafoProperties<double, DIM>& props)
{
  if (DIM == 2) {
    props.coords = {
      4.6, -1.8,
...
template<int DIM>
void init_aligned_quad_transformation(struct TrafoProperties<double, DIM>& props)
{
  props.coords = {
    4.8, 3.8,
    -6.3, 3.1,
...
template<int DIM>
void init_bilinear_quad_transformation(struct TrafoProperties<double, DIM>& props)
{
  if (DIM == 2) {
    props.coords = {
      0.1, 3.3,
...
template<int DIM>
void init_linear_tetrahedron_transformation(struct TrafoProperties<double, DIM>& props)
{
  props.coords = {
   -3.6, -0.5, -3.2,
   -1.5, 7.0, 5.9,
...
template<int DIM>
void init_linear_pyramid_transformation(struct TrafoProperties<double, DIM>& props)
{
  props.coords = {
    0.6, 1.9, 5.3,
    -9.9, -9.6, -0.7,
...
template<int DIM>
void init_aligned_hexahedron_transformation(struct TrafoProperties<double, DIM>& props)
{
  props.coords = {
    -1.0, -1.9, -1.0,
    1.0, -0.9, -1.0,
...
template<int DIM>
void init_trilinear_hexahedron_transformation(struct TrafoProperties<double, DIM>& props)
{
  props.coords = {
    3.0, 3.0, 0.0,
    -2.88, 2.4, 0.5,
...
template<int DIM>
void test_transformation(CellTrafoPtr<double, DIM> trafo,
    struct TrafoProperties<double, DIM> props)
{
  ...
  trafo->J(x, J0);
  BOOST_TEST(mat_equal_eps<DIM>(J0, props.J0, TOL));
...
{
  doffem::ConstRefCellPtr<double, DIM> cell(new doffem::RefCellLineStd<double, DIM>());
  doffem::CellTrafoPtr<double, DIM> trafo(
...
{
  doffem::ConstRefCellPtr<double, DIM> cell(new doffem::RefCellTriStd<double, DIM>());
  doffem::CellTrafoPtr<double, DIM> trafo(
...
{
  doffem::ConstRefCellPtr<double, DIM> cell(new doffem::RefCellQuadStd<double, DIM>());
  doffem::CellTrafoPtr<double, DIM> trafo(
      new doffem::AlignedQuadTransformation<double, DIM>(cell));
...
{
  doffem::ConstRefCellPtr<double, DIM> cell(new doffem::RefCellTetStd<double, DIM>());
  doffem::CellTrafoPtr<double, DIM> trafo(
...
{
  doffem::ConstRefCellPtr<double, DIM> cell(new doffem::RefCellPyrStd<double, DIM>());
  doffem::CellTrafoPtr<double, DIM> trafo(
...
{
  doffem::ConstRefCellPtr<double, DIM> cell(new doffem::RefCellHexStd<double, DIM>());
  doffem::CellTrafoPtr<double, DIM> trafo(
      new doffem::AlignedHexahedronTransformation<double, DIM>(cell));
...
BOOST_AUTO_TEST_CASE(cell_transformation)
{
  ...
  test_linear_line_transformation<2>();
  test_linear_line_transformation<3>();
