...
///                 (ca, pa, cb, pb)
/// 6) Use continuous numbering, and check whether all DoF IDs that
...
int main(int argc, char *argv[]) {
  ...
  for (int test_number = 0; test_number < static_cast< int >(filenames.size());
    ...
    std::vector<std::vector<std::vector< int > > >data(1, std::vector<std::vector<int>>(1));
    data[0][0].push_back(degree);
    

    std::vector<FEType> fe_ansatz_c (1, FE_TYPE_LAGRANGE);
    std::vector<FEType> fe_ansatz_dc (1, FE_TYPE_LAGRANGE);
    
    std::vector<bool> continuous (1, true);
    std::vector<bool> discontinuous(1,false);

    // continuous fe manager
    FEManager< double, DIM > *fe_manager_c = new FEManager< double, DIM >();
    fe_manager_c->set_mesh(*mesh);
    fe_manager_c->init(fe_ansatz_c, continuous, data);

    // discontinuous fe manager
    FEManager< double, DIM > *fe_manager_dc = new FEManager< double, DIM >();
    fe_manager_dc->set_mesh(*mesh);
    fe_manager_dc->init(fe_ansatz_dc, discontinuous, data);
                       
    /////////////////////////////////////
    // degrees of freedom

    // continuous dof partition
    DofPartition<double, DIM > *dof_c = new DofPartition< double, DIM>;
    dof_c->set_mesh(mesh.get());
    dof_c->set_fe_manager(fe_manager_c);
    

    NumberingStrategy<double, DIM>* number_strategy_c = new NumberingLagrange< double, DIM >;
    number_strategy_c->initialize(*dof_c);
    dof_c->set_numbering_strategy(number_strategy_c);
    assert(dof_c->check_mesh() == true);
    std::cout << dof_c << std::endl;
    dof_c->number(HIFLOW_CLASSIC);      //still throwing error: Attribute error: attribute _sub_domain_ missing
 
    

    
    //dof_c->update_number_of_dofs(); don't need. It's in number(...)

    // discontinuous dof
    DofPartition< double, DIM > *dof_dc = new DofPartition< double, DIM >;
    ...
    dof_dc->set_numbering_strategy(number_strategy_dc);
    dof_dc->number(HIFLOW_CLASSIC);      //still throwing error: Attribute error: attribute _sub_domain_ missing
