...
#define BOOST_TEST_MODULE coupled_matrix_dense
...
/// To test an arbitrary number of cores the global dimension is chosen as a
/// multiple of the local dimension.

BOOST_AUTO_TEST_CASE(coupled_matrix_dense, *utf::tolerance(1.0e-12)) {
  ...
  offdiag_offsets[0] = 0;
  for (int i = 0; i < num_procs; ++i)
    offdiag_offsets[i + 1] = offdiag_offsets[i] + (rank == i ? 0 : local_dim);

  // for (int i=0; i < num_procs + 1; ++i)
  //  std::cout << "[" << rank << "] offdiag_offsets" << "[" << i << "] = " <<
  //  offdiag_offsets[i] << std::endl;

  // Generate offdiag cols
  int offdiag_cols_dim = (num_procs - 1) * local_dim;
  std::vector< int > offdiag_cols(offdiag_cols_dim);
  for (int i = 0; i < rank * local_dim; ++i)
    offdiag_cols[i] = i;
  for (int i = (rank + 1) * local_dim; i < num_procs * local_dim; ++i)
    offdiag_cols[i - local_dim] = i;

  // for (int i=0; i < offdiag_cols_dim; ++i)
  //  std::cout << "[" << rank << "] offdiag_cols" << "[" << i << "] = " <<
  //  offdiag_cols[i] << std::endl;
