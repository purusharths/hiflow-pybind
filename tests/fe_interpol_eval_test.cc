...
{ 
  ...
  CONSOLE_OUTPUT(rank, "  Full Mapping Interpolation with BasisEvalLocal");
  Timer timer;
  timer.start();
  ...
  fe_mapping.init(&in_space, &out_space, in_sol, out_sol, in_fe_ind, out_fe_ind);
                       ...
                       CoupledVector<DataType>& sol)
{
  ...
  for (mesh::EntityIterator it = mesh_ptr->begin(DIM), e_it = mesh_ptr->end(DIM); it != e_it; ++it) 
  {
    ...
    std::vector<DofID> all_dofs(num_dofs);
                   ...
                   int init_level)
{
  ...
  std::string filename = "FeInterpolEval_" + cell_name + "_lvl" + std::to_string(init_level + level) 
                        + "_" + fe_name  
...
#ifdef VISUALIZE
    std::string filename_c = "FeInterpolEval_" + cell_name + "_lvl" 
                            + std::to_string(init_level + coarse_lvl) + "_" + fe_name 
                            + std::to_string(degrees[0]) + "_inter.vtu";
