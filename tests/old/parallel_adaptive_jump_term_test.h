...
public:
  ...
  void operator()(const Element< double, DIMENSION > &left_elem,
    ...
    const bool is_boundary =
        (right_if_side == DGGlobalAssembler< double, DIMENSION >::INTERFACE_BOUNDARY);
    ...
    LOG_DEBUG(2, "[" << rank << "] trial cell = " << left_elem.get_cell_index()
                     << " test cell = " << right_elem.get_cell_index()
