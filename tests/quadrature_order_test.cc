...
#define BOOST_TEST_MODULE quadrature_order
...
const double INTEGRATION_TOL = 1.e-10;
...
double beta(int i, int j) {
  return double(gamma_table.at(i) * gamma_table.at(j)) /
         double(gamma_table.at(i + j));
}
...
struct F3d {
  ...
  double operator()(const Quadrature< double > &quad, int q) const {
    return std::pow(quad.x(q), p_) * std::pow(quad.y(q), p_) *
           std::pow(quad.z(q), p_);
  }
...
BOOST_AUTO_TEST_CASE(quadrature_order, *utf::tolerance(INTEGRATION_TOL)) {
