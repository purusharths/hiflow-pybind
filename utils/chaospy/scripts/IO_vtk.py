...
def read_vtk_file ( filename, array_names ):
  """
  ...
  for an in array_names:
    ...
    if ( an == "Points" ):
      ...
      array = grid.GetPointData( ).GetArray( an )
    ...
    if ( data_tmp.ndim > 1 ):
      data.append( np.concatenate( data_tmp ) )
    else:
...
def read_vtk_nodes ( work_folder, vtk_filename, array_names, num_nodes ):
  """
  ...
  for n in range( num_nodes ):
    ...
    if console_info:
      print ( "Read vtk file:\n", node_filename )
    data, offsets_tmp = read_vtk_file ( node_filename, array_names )
