...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3) {
    if (rank == 0) {
      std::cerr << "Usage : mpirun -n <num_partitions> ./check_partitioning "
                   "<filename> <option>\n";
  ...
  // output
  if (partitioning_ok)
    std::cout << "Partitioning on process " << rank
              << " is defintely correct. (No isolated cells).\n";
  else
    std::cout << "Partitioning on process " << rank
...
bool check_partitioning(MeshPtr mesh, int option, int num_partitions) {
  ...
  if (option == 1) {
    if (rank == 0)
      std::cerr << "METIS partitioner NOT available. Need to recompile Hiflow "
  ...
  MeshPtr local_mesh =
      partition_and_distribute(mesh, 0, MPI_COMM_WORLD, p, uniform_ref_steps);
  assert(local_mesh != 0);
  ...
  for (EntityIterator it = local_mesh->begin(tdim); it != local_mesh->end(tdim);
       ++it) {
  ...
  for (int i = 0; i < num_proc; ++i) {
    ...
    if (i == rank) {
      std::cerr << "=== PROCESS " << i << " ===\n";
      std::cerr << "Cells with indeces: " << stream.str()
                << "could be isolated.\n\n";
    }
    MPI_Barrier(MPI_COMM_WORLD);
  }
