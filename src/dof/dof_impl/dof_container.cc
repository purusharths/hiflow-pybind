...
template < class DataType, int DIM >
std::vector< DofID > const & DofContainer< DataType, DIM >::get_dof_on_subentity(int tdim, int index) const 
{
  assert(tdim >= 0 && tdim <= ref_cell_->tdim());
...
{
  if (this->type_ == DOF_CONTAINER_NOT_SET || dof_slave.type_ == DOF_CONTAINER_NOT_SET) 
  {
    assert(0);
  }
