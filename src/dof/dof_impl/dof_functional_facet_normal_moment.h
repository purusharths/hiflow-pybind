...
public:
  ...
  {
    // compare type
    if (dof_slave.type() != DOF_FACET_MOMENT)
    {
      return false;
    }
                ...
                const CellTransformation<DataType, DIM> &c_trafo_b) 
{
  ...
  ConstRefCellPtr<DataType, DIM> ref_cell_a = c_trafo_a->get_ref_cell();
  ConstRefCellPtr<DataType, DIM> ref_cell_b = c_trafo_b->get_ref_cell();
