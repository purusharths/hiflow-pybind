...
/// OK
template < class DataType, int DIM >
int DofPartition< DataType, DIM >::owner_of_dof(DofID global_id) const 
{
  int result = 0;
  int bound = 0;
...
////////////////////////////////////////////////////////////////////////
template < class DataType, int DIM >
void DofPartition< DataType, DIM >::local2global(DofID local_id, DofID *global_id) const 
{
...
template < class DataType, int DIM >
void DofPartition< DataType, DIM >::global2local(DofID global_id, DofID *local_id) const 
{
...
template < class DataType, int DIM >
void DofPartition< DataType, DIM >::global2cells (DofID gl, std::vector<int>& cells) const
{
...
template < class DataType, int DIM >
void DofPartition< DataType, DIM >::fe2global (size_t fe_ind, std::vector<DofID>& ids) const
{
...
/// OK
template < class DataType, int DIM >
void DofPartition< DataType, DIM >::get_dofs_on_cell_local( size_t fe_ind, int cell_index, std::vector< DofID > &ids) const 
{
...
/// OK
template < class DataType, int DIM >
void DofPartition< DataType, DIM >::get_dofs_on_cell_local(int cell_index, std::vector< DofID > &ids) const 
{
...
/// OK
template < class DataType, int DIM >
void DofPartition< DataType, DIM >::get_dofs_on_cell( size_t fe_ind, int cell_index, std::vector< DofID > &ids) const 
{
...
template < class DataType, int DIM >
void DofPartition< DataType, DIM >::get_dofs_on_cell(int cell_index, std::vector< DofID > &ids) const 
{
/* new
  const size_t nb_dof = this->nb_dofs_on_cell(cell_index);
  ids.clear();
                                                           ...
                                                           std::vector< DofID > &ids) const 
{
  ...
  const std::vector< DofID > sub_dof = ref_fe.get_dof_on_subentity(tdim, sindex);
...
////////////////////////////////////////////////////////////////////////
template < class DataType, int DIM >
void DofPartition< DataType, DIM >::number(DOF_ORDERING order)
{
  assert (mesh_flag_);
  ...
  this->number_strategy_->number_locally(order);
  ...
  {  
    ...
    {
      std::vector< DofID > gl_dofs_on_cell_fe;
...
{ 
  ...
  for (mesh::EntityIterator it = this->mesh_->begin(DIM), e_it = this->mesh_->end(DIM); it != e_it; ++it) 
  {
    ...
    if (remote_ind >= 0)
    {
      ...
      // get dofs on cell
      std::vector<DofID> local_dof_ids_on_cell;
...
template < class DataType, int DIM >
void DofPartition< DataType, DIM >::update_number_of_dofs( const std::string &description) 
{
