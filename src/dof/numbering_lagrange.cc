...
void log_debug_fe_pattern_info(const mesh::Entity &entity,
                               const RefElement< DataType, DIM > &fe_type, int gdim) {
...
// TODO_COORD
  ...
  const std::vector< RefElement< DataType, DIM > const * > &fe_type_slaves = pattern.fe_type_slaves();
...
template < class DataType, int DIM >
void NumberingLagrange< DataType, DIM >::number_locally(DOF_ORDERING order) 
{
...
void NumberingLagrange< DataType, DIM >::initial_numbering() 
{
  ...
  {
    // loop over cells
    this->numer_cell_2_global_offsets_->at(fe_ind).reserve(this->mesh_->num_entities(this->fe_manager_->tdim()));
         ...
         it != e_it; ++it) 
    {
      ...
      (*this->numer_cell_2_global_offsets_)[fe_ind].push_back(numer_size);
       ...
       interface != e_interface; ++interface) 
  {
    ...
    for (size_t fe_ind = 0; fe_ind != this->nb_fe_; ++fe_ind) 
    {
      ...
      if (!this->fe_manager_->is_dg(fe_ind)) 
      {
        ...
        RefElement< DataType, DIM > const *master_type = this->fe_manager_->get_fe(interface->master_index(), fe_ind);

        // FE ansatz of slave cells
        std::vector< RefElement< DataType, DIM > const * > slave_types;
    ...
    DofInterpolationPattern<DataType> &interpolation) 
{
  ...
  const RefElement< DataType, DIM > &master_ansatz = pattern.fe_type_master();
  ...
  compute_weights(master_cell, *virtual_ansatz, virtual_facet_nr, 
                  master_cell, master_ansatz, master_facet_nr,
                  this->mesh_->get_period(), weights_m2v);

#ifndef NDEBUG
  for (int l = 0; l < weights_m2v.size(); ++l) 
  {
    LOG_DEBUG(1, string_from_range(weights_m2v[l].begin(), weights_m2v[l].end()));
  ...
  compute_weights(master_cell, master_ansatz, master_facet_nr,
                  master_cell, *virtual_ansatz, virtual_facet_nr,
...
//  const int needed_num_unconstrained_dofs = virtual_ansatz->nb_shared_dof_on_subentity(tdim - 1, pattern.master_facet_number());
  ...
  for (size_t s = 0, e_s = pattern.num_slaves(); s != e_s; ++s) 
  {
    ...
    const RefElement< DataType, DIM > &slave_ansatz = *pattern.fe_type_slaves()[s];
    int slave_facet_nr = interface.slave_facet_number(s);
    ...
    {
      LOG_DEBUG(2, string_from_range(weights_s2v[l].begin(), weights_s2v[l].end()));
    ...
    //std::cout << "Slave " << s << " s2m = s2v * v2m " << std::endl;
    ...
    InterpolationWeights &weights) const 
{
  ...
  std::shared_ptr< const CellTransformation< DataType, DIM > > transA = this->fe_manager_->get_cell_transformation(cellA.index());
  std::shared_ptr< const CellTransformation< DataType, DIM > > transB = this->fe_manager_->get_cell_transformation(cellB.index());
  ...
  MappingRef2Phys<DataType, DIM, RefElement< DataType, DIM > > * evalA 
    = new MappingRef2Phys<DataType, DIM, RefElement< DataType, DIM > > ( &ansatzA, ansatzA.fe_trafo(), transA);
  ...
  MappingPhys2Ref <DataType,DIM,MappingRef2Phys<DataType,DIM,RefElement< DataType, DIM > > > * evalA_on_B
    = new MappingPhys2Ref <DataType, DIM, MappingRef2Phys<DataType, DIM, RefElement< DataType, DIM > > > (evalA, &cellB, ansatzB.fe_trafo(), transB);
