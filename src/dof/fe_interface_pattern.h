...
public:
  FEInterfacePattern(mesh::InterfacePattern, 
                     RefElement< DataType, DIM > const *,
                     std::vector< RefElement< DataType, DIM > const * >);
  ~FEInterfacePattern();
  ...
  RefElement< DataType, DIM > const &fe_type_master() const { return *fe_type_master_; }

  std::vector< RefElement< DataType, DIM > const * > &fe_type_slaves() {
    return fe_type_slaves_;
  }

  std::vector< RefElement< DataType, DIM > const * > const &fe_type_slaves() const {
    return fe_type_slaves_;
  }
...
private:
  ...
  RefElement< DataType, DIM > const *fe_type_master_;

  /// \brief finite element ansatz of each slave cell
  std::vector< RefElement< DataType, DIM > const * > fe_type_slaves_;
