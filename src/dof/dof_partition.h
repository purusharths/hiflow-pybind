...
public:
  ...
  void get_dofs_on_cell(int cell_index, std::vector< DofID > &ids) const;
  ...
  void get_dofs_on_cell_local(int cell_index, std::vector< DofID > &ids) const;
  ...
  void global2cells (DofID gl, std::vector<int>& cells) const;
  void fe2global (size_t fe_ind, std::vector<DofID>& ids) const;
...
private:
  ...
  inline std::vector< DofID > * numer_cell_2_global() 
  {
    return &numer_cell_2_global_;
  }

  inline std::vector< DofID > * numer_cell_2_local() 
  {
  ...
  std::map< DofID, std::set<int> > global2cells_;
  std::vector< std::set<DofID> > fe2global_;
