...
template < class T, class Cmp = typename std::less< T > > class SortedArray {
public:
  ...
  explicit SortedArray(size_type size, const T &value = T(),
                       const Cmp &cmp = Cmp());
  ...
  /// \param[in] cmp     comparison functor object
  explicit SortedArray(const array_type &vec, const Cmp &cmp = Cmp());
  ...
  ~SortedArray() { this->data_.clear(); }

  SortedArray &operator=(const SortedArray< T > &arr) {
    if (this == &arr) {
      return *this;
    }
    this->cmp_ = arr.cmp_;
  ...
  /// \return an iterator pointing to the next element in the array, or
  /// array::end() if no such element exists.

  iterator erase(iterator it) {
    // TODO(Staffan, Thomas): Create range version of this.
    assert(it < this->data_.end());
    return this->data_.erase(it); // std::vector::erase will advance iterator
  }
  ...
  /// \param[out] pos    position where value was found (optional output
  /// argument) \return true if @p value was found in the array, false otherwise
  ...
  /// \return true if @p value already existed in the array; false if @p value
  /// was inserted in this function
  ...
  size_type size() const { return this->data_.size(); }
  ...
  const array_type &data() const { return this->data_; }

  /// \return Reference to data array

  array_type &data() { return this->data_; }
  ...
  void resize(size_type size, T val = T()) {
    ...
    this->data_.resize(size, val);

    // re-sort if necessary
    if (size > old_size && this->cmp_(val, last_elem)) {
  ...
  reference front() { return this->data_.front(); }

  /// \return immutable reference to first element of the array

  const_reference front() const { return this->data_.front(); }

  /// \return mutable reference to last element of the array

  reference back() { return this->data_.back(); }

  /// \return immutable reference to last element of the array

  const_reference back() const { return this->data_.back(); }
  ...
  void swap(SortedArray< T, Cmp > &array) { this->data_.swap(array.data_); }
...
SortedArray< T, Cmp >::SortedArray(const Cmp &cmp) : cmp_(cmp) {}

template < typename T, typename Cmp >
SortedArray< T, Cmp >::SortedArray(
    typename SortedArray< T, Cmp >::size_type size, const T &value,
    const Cmp &cmp)
    : data_(size, value), cmp_(cmp) {}

template < typename T, typename Cmp >
template < class InputIterator >
SortedArray< T, Cmp >::SortedArray(InputIterator first, InputIterator last,
                                   const Cmp &cmp)
    : data_(first, last), cmp_(cmp) {
  std::stable_sort(this->data_.begin(), this->data_.end(), this->cmp_);
}

template < typename T, typename Cmp >
SortedArray< T, Cmp >::SortedArray(
    const typename SortedArray< T, Cmp >::array_type &vec, const Cmp &cmp)
    : data_(vec), cmp_(cmp) {
  std::stable_sort(this->data_.begin(), this->data_.end(), this->cmp_);
}
...
void SortedArray< T, Cmp >::insert(const T &value) {
  ...
  if (this->data_.size() == this->data_.capacity()) {
    this->data_.reserve(static_cast< int >(this->data_.capacity() * 2.0) + 1);
  }
  ...
  if (this->data_.empty()) {
    ...
    int lower = 0;
    int upper = this->data_.size();

    do {
      const int pos = lower + static_cast< int >((upper - lower) / 2);
      if (this->cmp_(this->data_[pos], value)) {
        lower = pos + 1;
      } else {
        upper = pos;
      }
    ...
    assert(lower >= 0);
    assert(lower <= this->data_.size());
    assert(lower == 0 || this->cmp_(this->data_[lower - 1], value));
    assert(lower == this->data_.size() ||
           this->cmp_(value, this->data_[lower]));

    this->data_.insert(this->data_.begin() + lower, value);
  }
}
...
bool SortedArray< T, Cmp >::find(const T &value, int *pos) const {
  ...
  do {
    ...
    if (this->cmp_(this->data_[cur], value)) {
      lower = cur + 1;
    } else {
      upper = cur;
    }
  ...
  // if lower != this->data_.size(), check the data_ array to see if
  // the value matches
  const bool found =
      lower < this->data_.size() && (this->data_[lower] == value);
...
bool SortedArray< T, Cmp >::find_insert(const T &value) {
  ...
  // if lower != this->data_.size(), check the data_ array to see if
  // the value matches
  const bool found = lower < this->data_.size() && (data_[lower] == value);
