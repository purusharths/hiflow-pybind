...
public:
  ...
  int sum(const std::vector< std::vector< Vec<DIM,T> > >& local_vals, 
                std::vector< std::vector< Vec<DIM,T> > >& result) const;
...
template <typename T, int DIM>
int ParCom::sum(const std::vector< std::vector< Vec<DIM,T> > >& local_vals, 
                std::vector< std::vector< Vec<DIM,T> > >& result) const
{
  assert (local_vals.size() > 0);
  std::vector<size_t> sizes;
  ...
  expand_to_2d_vec_array (tmp_result, sizes, result);
