...
template < size_t N, class DataType > class Vec {
public:
...
#pragma clang loop vectorize(enable)
  ...
  explicit Vec(const std::vector< value_type > values, size_t begin) {
    assert(begin >= 0);
...
#pragma clang loop vectorize(enable)
  ...
  template < size_t M >
  Vec< N, DataType > &operator=(const Vec< M, DataType > &v) {
...
#pragma clang loop vectorize(enable)
  ...
  inline bool operator< (const Vec< N, DataType >rhs) const {
#pragma clang loop vectorize(enable)
    for (size_t i = 0; i < N; ++i) 
    {
      if (this->v_[i] < rhs[i])
...
#pragma clang loop vectorize(enable)
  ...
  inline bool operator==(Vec< N, DataType > v) const {
    for (size_t i = 0; i < N; ++i) {
  ...
  inline bool eq(Vec< N, DataType > v, DataType eps) const {
    for (size_t i = 0; i < N; ++i) {
  ...
  inline bool neq(Vec< N, DataType > v, DataType eps) const {
...
inline Vec< N, DataType >
operator*(const Vec< N, DataType > &v,
...
template < size_t N, class DataType >
inline Vec< N, DataType >
operator*(const typename Vec< N, DataType >::value_type s,
          const Vec< N, DataType > &v) {
  return v * s;
}

template < size_t N, class DataType >
inline Vec< N, DataType > operator+(const Vec< N, DataType > &v1,
...
template < size_t N, class DataType >
inline typename Vec< N, DataType >::value_type dot(const Vec< N, DataType > &v1, 
                                                   const Vec< N, DataType > &v2) 
{
...
inline Vec< 3, DataType > cross(const Vec< 3, DataType > &v1,
...
template < class DataType >
inline Vec< 2, DataType > cross(const Vec< 2, DataType > &v1,
                                const Vec< 2, DataType > &v2) {
  assert (false);
  Vec< 2, DataType > v3;
  return v3;
}
...
/// \brief this is just a dummy function
template < class DataType >
inline Vec< 1, DataType > cross(const Vec< 1, DataType > &v1,
                                const Vec< 1, DataType > &v2) {
  assert (false);
  Vec< 1, DataType > v3;
  return v3;
}
...
template < size_t N, class DataType >
inline typename Vec< N, DataType >::value_type
sum(const Vec< N, DataType > &v) {
  typename Vec< N, DataType >::value_type res =
      typename Vec< N, DataType >::value_type();
  for (size_t i = 0; i < N; ++i) {
    res += v[i];
  }
  return res;
...
template < class DataType >
inline Vec< 3, DataType > normal(const Vec< 3, DataType > &v1,
                                 const Vec< 3, DataType > &v2) {
...
template < class DataType >
inline bool is_parallel(const Vec<2, DataType> &v1, const Vec<2, DataType> &v2) {
...
template < class DataType >
inline bool is_parallel(const Vec<3, DataType> &v1, const Vec<3, DataType> &v2) {
  const DataType EPS = 1.0e3 * std::numeric_limits<DataType>::epsilon();
  Vec<3, DataType> w = cross(v1, v2);
...
#pragma clang loop vectorize(enable)
  ...
  inline bool operator==(const Mat< M, N, DataType > &m) {
  ...
  inline void VectorMult(const Vec< N, DataType > &in,
                         Vec< M, DataType > &out) const {
...
/*
template < size_t M, class DataType > class Mat< M, 0, DataType > {};

template < size_t N, class DataType > class Mat< 0, N, DataType > {};
*/
// Matrix-Matrix multiplication: out = mat1 * mat2

template < size_t M, size_t N, size_t P, class DataType >
inline void MatrixMatrixMult(Mat< M, P, DataType > &out,
...
inline Mat< M, N, DataType >
operator*(const Mat< M, N, DataType > &m,
...
template < size_t M, size_t N, class DataType >
inline Mat< M, N, DataType >
operator*(const typename Mat< M, N, DataType >::value_type s,
...
template < size_t M, size_t N, class DataType >
inline Mat< M, N, DataType > operator+(const Mat< M, N, DataType > &m1,
...
template < size_t M, size_t N, size_t P, class DataType >
inline Mat< M, P, DataType > operator*(const Mat< M, N, DataType > &A,
...
template < size_t M, size_t N, class DataType >
inline Vec< M, DataType > operator*(const Mat< M, N, DataType > &m,
                                    const Vec< N, DataType > &v) {
  Vec< M, DataType > mv;
  for (size_t i = 0; i < M; ++i) {
    for (size_t j = 0; j < N; ++j) {
...
template < size_t M, size_t N, class DataType >
inline Vec< N, DataType > operator*(const Vec< M, DataType > &v,
                                    const Mat< M, N, DataType > &m) {
  Vec< N, DataType > mv;
  for (size_t i = 0; i < M; ++i) {
#pragma clang loop vectorize(enable)
...
template < size_t M, size_t N, class DataType >
inline typename Mat< M, N, DataType >::value_type dot(const Mat< M, N, DataType > &m1, 
                                                      const Mat< M, N, DataType > &m2) 
{
  typename Mat< M, N, DataType >::value_type res = typename Mat< M, N, DataType >::value_type();
  for (size_t i = 0; i < M; ++i) 
  {
    for (size_t j = 0; j < N; ++j) 
    {
...
template < size_t M, size_t N, class DataType >
inline typename Mat< M, N, DataType >::value_type inner(const Vec< M, DataType > &v1, 
                                                        const Mat< M, N, DataType > &m,
                                                        const Vec< N, DataType > &v2) 
{
  typename Mat< M, N, DataType >::value_type res = typename Mat< M, N, DataType >::value_type();
...
template < size_t M, size_t N, class DataType >
inline typename Mat< M, N, DataType >::value_type innerT(const Vec< N, DataType > &v1, 
                                                         const Mat< M, N, DataType > &m,
                                                         const Vec< M, DataType > &v2) 
{
  typename Mat< M, N, DataType >::value_type res = typename Mat< M, N, DataType >::value_type();
  for (size_t i = 0; i < M; ++i) 
  {
...
template < class DataType >
inline void invTransp(const Mat< 1, 1, DataType > &m,
...
template < class DataType >
inline void invTransp(const Mat< 2, 2, DataType > &m,
...
template < class DataType >
inline void invTransp(const Mat< 3, 3, DataType > &m,
...
template < size_t M, size_t N, class DataType >
inline void trans(const Mat< M, N, DataType > &m,
...
template < size_t N, class DataType >
inline DataType frob(const Mat< N, N, DataType > &m1,
                     const Mat< N, N, DataType > &m2) {
