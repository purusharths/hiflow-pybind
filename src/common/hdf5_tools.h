...
/// \brief Classes and functions which help reading and writing data from/to
/// HDF5 files. \author Teresa Beck, Simon Gawlok
...
inline size_t get_stringlength(const std::string &input) {
  return input.size();
}
...
inline size_t get_max_stringlength(const std::vector< std::string > &vi) {
  size_t maxsize = 0;
...
public:
  ...
  H5File(const std::string &filename, const std::string &mode,
         const MPI_Comm &comm)
    : rank_(-1)
    {
      H5Propertylist fapl(H5P_FILE_ACCESS);
      H5Pset_fapl_mpio(fapl.id(), comm, MPI_INFO_NULL);

      FILE *file = fopen(filename.c_str(), "r");
      if (file == NULL) {
        if (mode == "w") {
          id_ = H5Fcreate(filename.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT,
                          fapl.id());
        } else {
          LOG_ERROR ("HDF5 ERROR:  H5File " << filename << " does not exist!");
        }
      } else {
        id_ = H5Fopen(filename.c_str(), H5F_ACC_RDWR, fapl.id());
        fclose(file);
      }
...
typedef SharedPtr< H5File >::Type H5FilePtr;
...
public:
  /// \brief Constructor
  ///
  /// \param loc_id Identifier of place where to create the group (can be a file
  /// identifier or a group identifier) \param groupname Name of group \param
  /// mode Read or write
  ///
  ...
  H5Group(const H5FilePtr &file_ptr, const std::string &groupname,
          const std::string &mode) {
    H5Propertylist lapl(H5P_LINK_ACCESS);
    H5Propertylist lcpl(H5P_LINK_CREATE);
    H5Propertylist gcpl(H5P_GROUP_CREATE);
    ...
    id_ = H5Gopen2(temp_id, "/", H5P_DEFAULT);
#else
    LOG_ERROR ("HDF5 version is too low!");
    exit(-1);
#endif
    temp_id = id_;
    ...
    if (newpath != "/") {
      do {
        ...
        link = H5Lexists(temp_id, checkpath.c_str(), lapl.id());
        if (link <= 0) {
          if (mode == "w") {
#if H5_VERS_MAJOR == 1 && H5_VERS_MINOR >= 8
            id_ = H5Gcreate2(temp_id, checkpath.c_str(), lcpl.id(), gcpl.id(), H5P_DEFAULT);
#else
            LOG_ERROR ("HDF5 version is too low!");
            exit(-1);
#endif
          } else {
            LOG_ERROR ("HDF5 ERROR:  H5Group does not exist!");
          }
        } else {
#if H5_VERS_MAJOR == 1 && H5_VERS_MINOR >= 8
          id_ = H5Gopen2(temp_id, checkpath.c_str(), H5P_DEFAULT);
#else
          LOG_ERROR ("HDF5 version is too low!");
          exit(-1);
#endif
...
typedef SharedPtr< H5Group >::Type H5GroupPtr;
...
template < class T >
inline H5Datatype get_hdf5_data_type(const T & /*unused*/) {
  assert(false);
  throw "Unknown type!";
  return H5Datatype(H5T_NATIVE_INT);
}

template <> inline H5Datatype get_hdf5_data_type(const int & /*unused*/) {
  return H5Datatype(H5T_NATIVE_INT);
}

template <> inline H5Datatype get_hdf5_data_type(const long int & /*unused*/) {
  return H5Datatype(H5T_NATIVE_LONG);
}

template <> inline H5Datatype get_hdf5_data_type(const double & /*unused*/) {
  return H5Datatype(H5T_NATIVE_DOUBLE);
}

template <> inline H5Datatype get_hdf5_data_type(const float & /*unused*/) {
  return H5Datatype(H5T_NATIVE_FLOAT);
}

template <> inline H5Datatype get_hdf5_data_type(const char & /*unused*/) {
...
inline H5Datatype get_hdf5_data_type(const std::vector< std::string > &data) {
  size_t max = get_max_stringlength(data);
  H5Datatype strtype = H5Tcopy(H5T_C_S1);
  if (max <= 0) {
    H5Tset_size(strtype.id(), H5T_VARIABLE);
  } else {
    H5Tset_size(strtype.id(), max);
  }
...
template <> inline H5Datatype get_hdf5_data_type(const std::string &data) {
  size_t max = get_stringlength(data);
  H5Datatype datatype = H5Tcopy(H5T_C_S1);
  if (max <= 0) {
    H5Tset_size(datatype.id(), H5T_VARIABLE);
  } else {
    H5Tset_size(datatype.id(), max);
  }
  return datatype;
}

template <> inline H5Datatype get_hdf5_data_type(const StringContainer &data) {
  hid_t strtype = H5Tcopy(H5T_C_S1);
  H5Tset_size(strtype, MaxStrLength);

  hid_t datatype = H5Tcreate(H5T_COMPOUND, sizeof(StringContainer));
  H5Tinsert(datatype, "string", HOFFSET(StringContainer, string), strtype);
  return datatype;
}
...
H5Datatype
get_hdf5_data_type(const std::vector< std::pair< KeyT, ValueT > > &data) {
...
public:
  ...
  H5Dataset(H5GroupPtr group_ptr, hsize_t dim_space,
            const std::string &datasetname, const std::string &mode,
            DataType *data) {
    group_ptr_ = group_ptr;
    h5dtype = get_hdf5_data_type(*data);
    int rank = -1;
    rank = group_ptr_->rank();
    assert (rank >= 0);
    
    // Open Dataset
    // Check if Dataset already exists.
    H5Propertylist lcpl(H5P_LINK_CREATE);
    H5Propertylist dapl(H5P_DATASET_ACCESS);
    H5Propertylist lapl(H5P_LINK_ACCESS);
    htri_t dataset = H5Lexists(group_ptr->id(), datasetname.c_str(), lapl.id());
    ...
    H5Propertylist dcpl(H5P_DATASET_CREATE);
    H5Pset_chunk(dcpl.id(), 1, &dim_space);
    ...
    if (mode == "r") {
      if (dataset == true) {
#if H5_VERS_MAJOR == 1 && H5_VERS_MINOR >= 8
        id_ = H5Dopen2(group_ptr->id(), datasetname.c_str(), dapl.id());
#else
        LOG_ERROR ("HDF5 version is too low!");
        exit(-1);
#endif
      } else
        LOG_ERROR ("HDF5 ERROR:  H5Dataset " + datasetname + " does not exist!");
    } else {
      if (dataset == true) {
#if H5_VERS_MAJOR == 1 && H5_VERS_MINOR >= 8
        id_ = H5Dopen2(group_ptr->id(), datasetname.c_str(), dapl.id());
        ...
        // If Datatypes don't match, old Dataset will be unlinked from the Group
        // and new Dataset with new Datatype is linked to Group instead. Memory
        // of old Dataset can not be released.
        if (H5Tequal(h5dtype.id(), filetype.id()) <= 0) {
          if (rank == 0)
          {
            LOG_WARNING ("HDF5 WARNING: H5Dataset already exists with a different Datatype!");
            LOG_WARNING ("The existing H5Dataset will be overwritten.");
          }
          H5Ldelete(group_ptr->id(), datasetname.c_str(), lapl.id());
          H5Dclose(id_);
#if H5_VERS_MAJOR == 1 && H5_VERS_MINOR >= 8
          id_ = H5Dcreate2(group_ptr->id(), datasetname.c_str(), h5dtype.id(),
                           space.id(), lcpl.id(), dcpl.id(), dapl.id());
#else
          LOG_ERROR ("HDF5 version is too low!");
          exit(-1);
#endif
          ...
          // If Dataspaces in File is smaller than new Dataspace, Dataset is
          // extended.
          if (filespace.number_of_elements() < space.number_of_elements()) {
            if (rank == 0)
            {
              LOG_WARNING ("HDF5 WARNING: H5Dataset already exists with a "
                      "different Dataspace!");
              LOG_WARNING ("The existing H5Dataset will be overwritten.");
            }
            H5Dset_extent(id_, &dim_space);
          } // If Dataspaces in File is bigger than new Dataspace, old Dataset
          // will be unlinked from the Group and new Dataset with new
          // Dataspace is linked to Group instead. Memory of old Dataset can
          // not be released.
          else if (filespace.number_of_elements() >
  ...
  H5Dataset(H5GroupPtr group_ptr, const std::string &datasetname) {
    ...
    if (dataset == true) {
      ...
      LOG_ERROR ("HDF5 ERROR:  H5Dataset does not exist!");
  }
  ...
  ///
  /// \param dim_memspace Size of dimensions of dataspace describing the
  /// dimensionality of dataset \param offset Offset of processors' ownership of
  /// data to be written to the file \param data Buffer with data to be written
  /// to the file
  ...
  void write(hsize_t dim_memspace, hsize_t offset, DataType *data) {
    ...
    H5Sselect_hyperslab(space.id(), H5S_SELECT_SET, &offset, NULL,
                        &dim_memspace, NULL);

    // Create property list for collective dataset write.
    H5Propertylist dxpl(H5P_DATASET_XFER);
    H5Pset_dxpl_mpio(dxpl.id(), H5FD_MPIO_INDEPENDENT);

    // Write Data
    H5Dwrite(id_, h5dtype.id(), memspace.id(), space.id(), dxpl.id(), data);
  }
  ...
  /// \param dim_memspace Size of dimensions of dataspace describing the
  /// dimensionality of dataset \param offset Offset of processors' ownership of
  /// data to be written to the file \param data Buffer to receive data read
  /// from file
  ...
  void read(hsize_t dim_memspace, hsize_t offset, DataType *buffer) {
    ...
    H5Sselect_hyperslab(space.id(), H5S_SELECT_SET, &offset, NULL,
                        &dim_memspace, NULL);
    // Read Data from File.
    H5Dread(id_, memtype.id(), memspace.id(), space.id(), H5P_DEFAULT, buffer);
  }
...
typedef SharedPtr< H5Dataset >::Type H5DatasetPtr;
...
template < class DataType > class H5Attribute {
public:
  /// \brief Constructor
  ...
  ///
  /// \param dataset_ptr Pointer to HDF5 dataset where to append the attribute
  /// or where the attribute is appended to \param size Size of dimensions of
  /// attribute \param att_name Name of attribute \param mode Read or write
  ///
  /// If the attribute already exists, it will be overwritten.
  /// Otherwise it will be created.

  H5Attribute(H5DatasetPtr dataset_ptr, hsize_t &size,
              const std::string &att_name, const std::string &mode) {
    if (mode == "r") {
  ...
  H5Attribute(H5GroupPtr group_ptr, hsize_t &size, const std::string &att_name,
              const std::string &mode) {
    if (mode == "r") {
...
private:
  ...
  ///
  /// \param pointer_id Pointer to HDF5 dataset or group where to append the
  /// attribute \param size Size of dimensions of attribute \param att_name Name
  /// of attribute

  void create(hid_t pointer_id, hsize_t size, const std::string &att_name) {
    // Create Attribute property list
//    H5Propertylist acpl(H5P_ATTRIBUTE_CREATE);
//    H5Propertylist aapl(H5P_ATTRIBUTE_ACCESS);
    ...
    // Create Attribute
#if H5_VERS_MAJOR == 1 && H5_VERS_MINOR >= 8
    id_ = H5Acreate2(pointer_id, att_name.c_str(), h5dtype.id(), attspace.id(),
  ...
  ///
  /// \param pointer_id Pointer to HDF5 dataset or group the attribute is
  /// appended to \param att_name Name of attribute

  void open(hid_t pointer_id, const std::string &att_name) {
    // Create Attribute property list
    H5Propertylist atpl(H5P_ATTRIBUTE_CREATE);
    ...
    if (attr == false) {
      LOG_ERROR ("HDF5 ERROR:  H5Attribute does not exist!");
    }

    // Open Attribute
    id_ = H5Aopen(pointer_id, att_name.c_str(), atpl.id());
  }
};
...
template < typename T >
void write_array(H5GroupPtr group_ptr, const std::string &datasetname,
                 const std::vector< T > &array) {
  H5DatasetPtr dataset_ptr(
    new H5Dataset(group_ptr, array.size(), datasetname, "w", &array[0]));
...
void write_array(H5GroupPtr group_ptr, const std::string &datasetname,
  ...
  H5DatasetPtr dataset_ptr(
    new H5Dataset(group_ptr, size, datasetname, "w", array));
...
template < typename T >
inline void write_array_parallel(H5GroupPtr group_ptr, std::string datasetname,
                                 const T *data, int data_size,
                                 const MPI_Comm &comm) {
  int rank, num_part;
  ...
  if (global_data_size > 0) {
    H5DatasetPtr data_dataset_ptr(
      new H5Dataset(group_ptr, global_data_size, datasetname, "w", data));
    data_dataset_ptr->write(local_data_size, data_offset, data);
  } else {
    // std::cerr << "Could not write dataset " << datasetname << " because it
    // has no elements!" <<std::endl;
  }
  std::stringstream data_offset_name;
  data_offset_name << datasetname << "_offset_";
  H5DatasetPtr data_offset_ptr(new H5Dataset(
                                 group_ptr, num_part, data_offset_name.str(), "w", &data_next_offset));
...
template < typename KeyT, typename ValueT >
inline void write_map_parallel(H5GroupPtr group_ptr, std::string datasetname,
                               const std::map< KeyT, ValueT > &map_data,
                               const MPI_Comm &comm) {
  int rank, num_part;
  MPI_Comm_rank(comm, &rank);
  MPI_Comm_size(comm, &num_part);
...
template < typename T >
void read_array_parallel(H5GroupPtr group_ptr, std::string datasetname,
                         T *&data, int &data_size, const MPI_Comm &comm) {
  int rank, num_part;
  MPI_Comm_rank(comm, &rank);
  MPI_Comm_size(comm, &num_part);
  ...
  H5DatasetPtr data_offset_ptr(new H5Dataset(
                                 group_ptr, num_part, data_offset_name.str(), "r", &data_next_offset));
  ...
  if (global_data_size > 0) {
    ...
    H5DatasetPtr data_dataset_ptr(
      new H5Dataset(group_ptr, global_data_size, datasetname, "r", data));
...
template < typename T >
void read_array_parallel(H5GroupPtr group_ptr, std::string datasetname,
                         std::vector< T > &data, const MPI_Comm &comm) {
  int rank, num_part;
  MPI_Comm_rank(comm, &rank);
  MPI_Comm_size(comm, &num_part);
  ...
  if (global_data_size > 0) {
    H5DatasetPtr data_dataset_ptr(
      new H5Dataset(group_ptr, global_data_size, datasetname, "r", &data[0]));
    data_dataset_ptr->read(local_data_size, data_offset, &data[0]);
  }
}

/*void read_array_parallel(H5GroupPtr group_ptr, std::string datasetname,
std::vector<std::string>& data, MPI_Comm& comm)
{
...
template < typename T >
void write_value(H5GroupPtr group_ptr, const std::string &datasetname,
                 const T &value) {
...
inline void write_array(H5GroupPtr group_ptr, const std::string &datasetname,
                        const std::vector< std::string > &v) {

  H5DatasetPtr dataset_ptr(
    new H5Dataset(group_ptr, v.size(), datasetname, "w", &v));
...
inline void write_value(H5GroupPtr group_ptr, const std::string &datasetname,
...
template < typename T >
void read_array(H5GroupPtr group_ptr, const std::string &datasetname,
                std::vector< T > &array) {
  ...
  H5DatasetPtr dataset_ptr(
    new H5Dataset(group_ptr, size, datasetname, "r", &array[0]));
...
template < typename T >
void read_value(H5GroupPtr group_ptr, const std::string &datasetname,
                T *value) {
...
inline void read_array(H5GroupPtr group_ptr, const std::string &datasetname,
                       std::vector< std::string > &v) {
  ...
  H5DatasetPtr dataset_ptr(
    new H5Dataset(group_ptr, size, datasetname, "r", &v[0]));
  ...
  H5Dread(dataset_ptr->id(), datatype.id(), memspace.id(), filespace.id(),
...
///
/// Reading arrays of strings is based on the read_array function for vectors of
/// strings. A temporary vector is used for reading data from file. Data in
/// vector is written into array afterwards.
...
inline void read_array_parallel(H5GroupPtr group_ptr, std::string datasetname,
...
///
/// Reading single values is based on the read_array function for arrays of
/// strings.

inline void read_value(H5GroupPtr group_ptr, const std::string &datasetname,
                       std::string &value) {
  read_array(group_ptr, datasetname, &value);
}
...
template < typename KeyT, typename ValueT >
void write_map(H5GroupPtr group_ptr, const std::string &datasetname,
  ...
  H5DatasetPtr dataset_ptr(
    new H5Dataset(group_ptr, size, datasetname, "w", &temp));
...
void write_map_content(
  ...
  int max = 0;
  for (int i = 0; i < size; i++) {
    if (max < get_stringlength((temp[i]).second))
      max = ((temp[i]).second).size();
  }
...
void write_map_content(
  ...
  int max = 0;
  for (int i = 0; i < size; i++) {
    if (max < get_stringlength((temp[i]).first))
      max = ((temp[i]).first).size();
  }
...
/// The function read_map_content reads the map from file into a vector of
/// pairs.

template < typename KeyT, typename ValueT >
void read_map(H5GroupPtr group_ptr, const std::string &datasetname,
  ...
  // Create DatasetPtr
  H5DatasetPtr dataset_ptr(
    new H5Dataset(group_ptr, size, datasetname, "r", &buffer));
...
template < typename KeyT, typename ValueT >
void read_map_parallel(H5GroupPtr group_ptr, const std::string &datasetname,
                       std::map< KeyT, ValueT > &map_data,
                       const MPI_Comm &comm) {
  int rank, num_part;
  MPI_Comm_rank(comm, &rank);
  MPI_Comm_size(comm, &num_part);
...
/// \brief Read content of map from dataset in file into vector of pairs with
/// string key.
///
/// \param dataset_ptr Pointer to HDF5 dataset
/// \param temp Buffer for vector of pairs to be read from file.
...
inline void
read_map_content(H5DatasetPtr dataset_ptr, hsize_t size,
