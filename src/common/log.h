...
/// \return std::string with space-delimited representation of elements in the
/// range.
...
std::string string_from_range(const Iterator &begin, const Iterator &end) {
  ...
  std::copy(
      ...
      std::ostream_iterator<
          typename std::iterator_traits< Iterator >::value_type >(stream, " "));
  return stream.str();
}
...
template < typename Iterator >
std::string precise_string_from_range(const Iterator &begin,
                                      const Iterator &end) {
  std::stringstream stream;
  stream.precision(16);
  std::copy(
...
std::string string_from_pointer_range(const ValueType *begin,
                                      const ValueType *end) {
  std::stringstream stream;
...
///
/// LOG_DEBUG(1, "Hello, " << your_name << ", the number of the day is " <<
/// number);
///
...
/// Log a message with a given level "lvl". The message is only logged if lvl is
/// not greater than the integer variable DEBUG_LEVEL, which must be defined
/// whereever the macro is used. Debug messages are logged to the "debug" log.
  ...
  {                                                                            \
    if (lvl <= DEBUG_LEVEL) {                                                  \
      hiflow::LogKeeper::get_log("debug").create_entry(                        \
          ...
          << std::setw(20) << std::left << "DEBUG" << " : " << stream;                  \
      hiflow::LogKeeper::get_log("debug").flush();                             \
    }                                                                          \
  }
...
#define LOG_ERROR(stream)                                                      \
  {                                                                            \
    hiflow::LogKeeper::get_log("error").create_entry(                          \
        __FILE__, __FUNCTION__, __LINE__, hiflow::LOG_ERROR_TAG)               \
        << std::setw(20) << std::left << "ERROR" << " : " << stream;                    \
    hiflow::LogKeeper::get_log("error").flush();                               \
  }

/// Log a warning message to the log "warning".
#define LOG_WARNING(stream)                                                    \
  {                                                                            \
    hiflow::LogKeeper::get_log("warning").create_entry(                        \
        __FILE__, __FUNCTION__, __LINE__, hiflow::LOG_WARNING_TAG)             \
        << std::setw(20) << std::left << "WARNING" << " : " << stream;                    \
    hiflow::LogKeeper::get_log("warning").flush();                             \
  }
  ...
  {                                                                            \
    if (INFO) {                                                                \
      hiflow::LogKeeper::get_log("info").create_entry(                         \
          ...
          << std::setw(20) << std::left << name << " : " << stream;            \
      hiflow::LogKeeper::get_log("info").flush();                              \
    }                                                                          \
  }
#define PLOG_INFO(rank, name, stream)                                   \
  {                                                                     \
    hiflow::LogKeeper::get_log("info").create_entry(                    \
          __FILE__, __FUNCTION__, __LINE__, hiflow::LOG_INFO_TAG)       \
          << std::setw(20) << std::left << name                         \
          << " [" << std::setw(4) << rank <<"] : " << stream;            \
      hiflow::LogKeeper::get_log("info").flush();                       \
  }
#else
#define LOG_INFO(name, stream)                                                 \
