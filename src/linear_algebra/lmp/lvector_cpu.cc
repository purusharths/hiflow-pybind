...
template < typename ValueType > CPU_lVector< ValueType >::CPU_lVector() {
...
template < typename ValueType > void CPU_lVector< ValueType >::Clear() {
...
template < typename ValueType > void CPU_lVector< ValueType >::Zeros() {
...
template < typename ValueType >
void CPU_lVector< ValueType >::add_values(const int *indices, int length,
...
template < typename ValueType >
void CPU_lVector< ValueType >::SetValues(const int *index, const int size,
                                         const ValueType *values) {
  assert(0 <= size);
...
template < typename ValueType >
void CPU_lVector< ValueType >::GetValues(const int *index, const int size,
...
void CPU_lVector< ValueType >::SetBlockValues(const int start_i,
  ...
  if ((this->get_size() > 0) && (start_i < end_i) && (values != NULL)) {
...
void CPU_lVector< ValueType >::GetBlockValues(const int start_i,
  ...
  if ((this->get_size() > 0) && (start_i < end_i)) {
    for (int i = 0, i_e = end_i - start_i; i != i_e; ++i) {
...
void CPU_lVector< ValueType >::GetIndexedValues(ValueType *values) const {
  ...
  if ((this->get_indexset_size() > 0) && (this->get_size() > 0)) {
...
template < typename ValueType >
lVector< ValueType > &CPU_lVector< ValueType >::
operator=(const lVector< ValueType > &vec2) {
  if (this == &vec2)
    return *this;
...
template < typename ValueType >
lVector< ValueType > *
CPU_lVector< ValueType >::extract_subvector(const int start_i,
  ...
  sub_vector = init_vector< ValueType >(end_i - start_i, sub_vec_name,
                                        this->get_platform(),
  ...
  if (const CPU_lVector< ValueType > *casted_vec =
    ...
    this->GetBlockValues(start_i, end_i, casted_vec->buffer);

    // actually this cannot happen
  } else {
    LOG_ERROR(
        "CPU_lVector<ValueType>::extract_subvector(); unsupported vector type");
    this->print();
    sub_vector->print();
    quit_program();
  }
...
template < typename ValueType >
void CPU_lVector< ValueType >::partial_replace_subvector(
    const int start_i, const int start_sub_vec, const int size,
    const lVector< ValueType > &sub_vec) {

  if (const CPU_lVector< ValueType > *casted_vec =
          dynamic_cast< const CPU_lVector< ValueType > * >(&sub_vec)) {

    this->partial_replace_subvector(start_i, start_sub_vec, size, *casted_vec);

  } else {
    LOG_ERROR("CPU_lVector<ValueType>::partial_replace_subvector() unsupported "
              "vector type");
    this->print();
    sub_vec.print();
    quit_program();
...
template < typename ValueType >
void CPU_lVector< ValueType >::partial_add_subvector(
    const int start_i, const int start_sub_vec, const int size,
    const ValueType weight, const lVector< ValueType > &sub_vec) {
  if (const CPU_lVector< ValueType > *casted_vec =
          dynamic_cast< const CPU_lVector< ValueType > * >(&sub_vec)) {

    this->partial_add_subvector(start_i, start_sub_vec, size, weight,
                                *casted_vec);

  } else {
    LOG_ERROR("CPU_lVector<ValueType>::partial_add_subvector() unsupported "
              "vector type");
    this->print();
    sub_vec.print();
    quit_program();
...
void CPU_lVector< ValueType >::partial_add_subvector(
    const int start_i, const int start_sub_vec, const int size,
    const ValueType weight, const CPU_lVector< ValueType > &sub_vec) {
  assert(start_i >= 0);
  assert(start_sub_vec >= 0);
  assert(size > 0);
  assert(start_sub_vec + size <= sub_vec.get_size());
...
template < typename ValueType >
void CPU_lVector< ValueType >::ElementWiseMult(
  ...
  if (const CPU_lVector< ValueType > *casted_vec =
    ...
    LOG_ERROR(
        "CPU_lVector<ValueType>::ElementWiseMult() unsupported vector type");
    this->print();
    vec.print();
    quit_program();
  }
...
template < typename ValueType >
void CPU_lVector< ValueType >::ElementWiseMult(
    const CPU_lVector< ValueType > &vec) const {
...
void CPU_lVector< ValueType >::ReadFile(const char *filename) {
  ...
  if (myfile.is_open()) {
    ...
    LOG_ERROR(
        "CPU_lVector<ValueType>::ReadFile() Unable to open file: " << filename);
    this->print();
    quit_program();
  }
}
...
void CPU_lVector< ValueType >::WriteFile(const char *filename) {
  ...
  if (myfile.is_open()) {
    ...
    LOG_ERROR("CPU_lVector<ValueType>::WriteFile() Unable to open file: "
              << filename);
    this->print();
    quit_program();
  }
