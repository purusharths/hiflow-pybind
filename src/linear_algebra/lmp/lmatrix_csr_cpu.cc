...
template < typename ValueType > void CPU_CSR_lMatrix< ValueType >::Clear() {
...
template < typename ValueType > void CPU_CSR_lMatrix< ValueType >::Zeros(void) {
...
void CPU_CSR_lMatrix< ValueType >::Init(const int init_nnz,
  ...
  LOG_DEBUG(2, "init_nnz = " << init_nnz << " init_num_row = " << init_num_row
                             << " init_num_col = " << init_num_col);
...
template < typename ValueType > void CPU_CSR_lMatrix< ValueType >::Compress() {
  std::vector< int > rows;
  std::vector< int > cols;
  ...
  if (this->get_nnz() > 0) {
    ...
    for (int i = 0, e_i = this->get_num_row(); i != e_i; ++i) {
      for (int j = this->matrix.row[i], e_j = this->matrix.row[i + 1]; j != e_j;
           ++j) {
...
void CPU_CSR_lMatrix< ValueType >::ScaleOffdiag(const ValueType alpha) {
  ...
  if (this->get_nnz() > 0)
    for (size_t i = 0, i_e = this->get_num_row(); i != i_e; ++i)
      for (size_t j = this->matrix.row[i], j_e = this->matrix.row[i + 1];
           j != j_e; ++j)
...
template < typename ValueType >
lMatrix< ValueType > &CPU_CSR_lMatrix< ValueType >::
operator=(const lMatrix< ValueType > &mat2) {
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::CastFrom(
    const hiflow::la::lMatrix< double > &other) {
  ...
  if (const CSR_lMatrix< double > *other_csr =
    ...
    LOG_ERROR("CPU_CSR_lMatrix<ValueType>::CastFrom<double> called with "
              "non-CSR matrix argument.");
    this->print();
    other.print();
    quit_program();
  }
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::CastFrom(
    const hiflow::la::lMatrix< float > &other) {
  assert(this->get_num_row() == other.get_num_row());
  assert(this->get_num_col() == other.get_num_col());
  assert(this->get_nnz() == other.get_nnz());
  ...
  if (const CSR_lMatrix< double > *other_csr =
    ...
    LOG_ERROR("CPU_CSR_lMatrix<ValueType>::CastFrom<float> called with non-CSR "
              "matrix argument.");
    this->print();
    other.print();
    quit_program();
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::CastTo(
    hiflow::la::lMatrix< double > &other) const {
  assert(this->get_num_row() == other.get_num_row());
  assert(this->get_num_col() == other.get_num_col());
  assert(this->get_nnz() == other.get_nnz());
  ...
  if (CSR_lMatrix< double > *other_csr =
    ...
    LOG_ERROR("CPU_CSR_lMatrix<ValueType>::CastTo<double> called with non-CSR "
              "matrix argument.");
    this->print();
    other.print();
    quit_program();
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::CastTo(
    hiflow::la::lMatrix< float > &other) const {
  assert(this->get_num_row() == other.get_num_row());
  assert(this->get_num_col() == other.get_num_col());
  assert(this->get_nnz() == other.get_nnz());
  ...
  if (CSR_lMatrix< double > *other_csr =
    ...
    LOG_ERROR("CPU_CSR_lMatrix<ValueType>::CastTo<float> called with non-CSR "
              "matrix argument.");
    this->print();
    other.print();
    quit_program();
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::CopyStructureFrom(
    const lMatrix< ValueType > &mat2) {

  if (this != &mat2) {

    this->Init(mat2.get_nnz(), mat2.get_num_row(), mat2.get_num_col(),
               mat2.get_name());
    ...
    if (const CPU_CSR_lMatrix< ValueType > *casted_mat =
      ...
      if (this->get_nnz() > 0) {
        ...
        memcpyhost(this->matrix.row, casted_mat->matrix.row,
                   this->get_num_row() + 1);
      }
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::CopyStructureTo(
    lMatrix< ValueType > &mat2) const {
  mat2.CopyStructureFrom(*this);
}

template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::ConvertFrom(
    const hiflow::la::lMatrix< ValueType > &mat2) {

  // CPU CSR = COO CPU
  if (const CPU_COO_lMatrix< ValueType > *casted_mat =
          dynamic_cast< const CPU_COO_lMatrix< ValueType > * >(&mat2)) {

    this->Init(casted_mat->get_nnz(), casted_mat->get_num_row(),
               casted_mat->get_num_col(), casted_mat->get_name());
    ...
    if (this->get_nnz() > 0)
      this->TransformFromCOO(casted_mat->matrix.row, casted_mat->matrix.col,
                             casted_mat->matrix.val, this->get_num_row(),
                             this->get_num_col(), this->get_nnz());
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::VectorMultNoDiag(
    const hiflow::la::lVector< ValueType > &in,
    hiflow::la::lVector< ValueType > *out) const {
  assert(in.get_size() == this->get_num_col());
  assert(out->get_size() == this->get_num_row());

  const CPU_lVector< ValueType > *cpu_in =
      dynamic_cast< const CPU_lVector< ValueType > * >(&in);
  CPU_lVector< ValueType > *cpu_out =
      dynamic_cast< CPU_lVector< ValueType > * >(out);

  if (cpu_in && cpu_out) {
    this->VectorMultNoDiag(*cpu_in, cpu_out);
  } else {
    LOG_ERROR("CPU_CSR_lMatrix<ValueType>::VectorMultNoDiag called with "
              "non-CPU vector argument.");
    this->print();
    in.print();
    out->print();
  }
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::VectorMultNoDiag(
    const CPU_lVector< ValueType > &in, CPU_lVector< ValueType > *out) const {
  assert(in.get_size() == this->get_num_col());
  assert(out->get_size() == this->get_num_row());
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::Psgauss_seidel(
  ...
  if ((casted_invec == NULL) && (casted_outvec == NULL)) {
    LOG_ERROR("CPUsimple_csr_matrix<ValueType>::Psgauss_seidel unsupported in "
              "or out vector");
    this->print();
    invec.print();
    outvec->print();
  ...
  for (int i = 0; i < this->get_num_row(); ++i) {
    ...
    for (int j = this->matrix.row[i]; j < this->matrix.row[i + 1]; ++j) {
      if (i > this->matrix.col[j])
        casted_outvec->buffer[i] -=
            casted_outvec->buffer[this->matrix.col[j]] * this->matrix.val[j];
      if (i == this->matrix.col[j]) {
        last_j = j;
        break;
      }
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::BlocksPsgauss_seidel(
    const lVector< ValueType > &invec, lVector< ValueType > *outvec,
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::BlockPsgauss_seidel(
    const lVector< ValueType > &invec, lVector< ValueType > *outvec,
  ...
  if ((casted_invec == NULL) && (casted_outvec == NULL)) {
    LOG_ERROR("CPUsimple_csr_matrix<ValueType>::BlockPsgauss_seidel "
              "unsupported in or out vector");
    this->print();
    invec.print();
    outvec->print();
    quit_program();
  ...
  for (int i = start_i; i < end_i; ++i) {
    ...
    for (int j = this->matrix.row[i]; j < this->matrix.row[i + 1]; ++j) {
      ...
      if ((this->matrix.col[j] >= start_i) && (this->matrix.col[j] < end_i)) {
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::Pssor(const ValueType omega,
  ...
  if ((casted_invec == NULL) && (casted_outvec == NULL)) {
    LOG_ERROR(
        "CPUsimple_csr_matrix<ValueType>::Pssor unsupported in or out vector");
    this->print();
    invec.print();
    outvec->print();
    quit_program();
  ...
  for (int i = 0; i < this->get_num_row(); ++i) {
    ...
    for (int j = this->matrix.row[i]; j < this->matrix.row[i + 1]; ++j) {
      if (i > this->matrix.col[j])
        casted_outvec->buffer[i] -= omega *
                                    casted_outvec->buffer[this->matrix.col[j]] *
                                    this->matrix.val[j];
      if (i == this->matrix.col[j]) {
        last_j = j;
        break;
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::Psor(const ValueType omega,
                                        const lVector< ValueType > &invec,
                                        lVector< ValueType > *outvec) const {
  ...
  if ((casted_invec == NULL) && (casted_outvec == NULL)) {
    LOG_ERROR(
        "CPUsimple_csr_matrix<ValueType>::Psor unsupported in or out vector");
    this->print();
    invec.print();
    outvec->print();
    quit_program();
  ...
  for (int i = 0; i < this->get_num_row(); ++i) {
    ...
    assert(this->matrix.val[last_j] != 0.0);
    casted_outvec->buffer[i] =
        casted_outvec->buffer[i] * omega / this->matrix.val[last_j];
  }
}

template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::Pgauss_seidel(
    const lVector< ValueType > &invec, lVector< ValueType > *outvec) const {
  ...
  if ((casted_invec == NULL) && (casted_outvec == NULL)) {
    LOG_ERROR("CPUsimple_csr_matrix<ValueType>::Pgauss_seidel unsupported in "
              "or out vector");
    this->print();
    invec.print();
    outvec->print();
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::Pjacobi(const lVector< ValueType > &invec,
                                           lVector< ValueType > *outvec) const {
  const CPU_lVector< ValueType > *casted_invec =
      dynamic_cast< const CPU_lVector< ValueType > * >(&invec);
  CPU_lVector< ValueType > *casted_outvec =
  ...
  if ((casted_invec == NULL) && (casted_outvec == NULL)) {
    LOG_ERROR("CPUsimple_csr_matrix<ValueType>::Pjacobi unsupported matrix or "
              "in/out vector");
    this->print();
    invec.print();
    outvec->print();
  ...
  for (int i = 0; i < this->get_num_row(); ++i)
    for (int j = this->matrix.row[i]; j < this->matrix.row[i + 1]; ++j)
      if (i == this->matrix.col[j]) {
        ...
        casted_outvec->buffer[i] =
            casted_invec->buffer[this->matrix.col[j]] / this->matrix.val[j];
      }
}

template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::TransformFromCOO(
    const int *rows, const int *cols, const ValueType *data, const int num_rows,
  ...
  COO_lMatrixTypeConst<ValueType> coo_data;
  coo_data.row = rows;
  coo_data.col = cols;
  coo_data.val = data;
  
  coo_2_csr <ValueType, int> (coo_data, num_rows, num_cols, num_nonzeros, false, this->matrix);
}
...
void CPU_CSR_lMatrix< ValueType >::ZeroRows(const int *index_set,
  ...
  for (int i = 0; i != size; ++i) {
    ...
    for (int j = this->matrix.row[current_index],
             j_e = this->matrix.row[current_index + 1];
         j != j_e; ++j) {
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::init_structure(const int *rows,
                                                  const int *cols) {
  ...
  this->TransformFromCOO(rows, cols, NULL, this->get_num_row(),
                         this->get_num_col(), this->get_nnz());
}

template < typename ValueType >
bool CPU_CSR_lMatrix< ValueType >::found_entry(const int row, const int col, ValueType val ) const 
{
  assert(this->get_nnz() > 0);
  assert(this->get_num_row() > 0);
  assert(this->get_num_col() > 0);
  ...
  LOG_ERROR("entry ( " << row << " , " << col << " ) is not present in matrix csr structure");
  return false;
}

template < typename ValueType >
bool CPU_CSR_lMatrix< ValueType >::found_entries(const int *rows, int num_rows,
                                                 ...
                                                 const ValueType *values) const 
{
  ...
  {
    const int cur_row = rows[i]; 
    const int row_offset = i * num_cols;
    
    for (int j = 0; j != num_cols; ++j) 
    {
      const int cur_col = cols[j];
      
      if (!this->found_entry(cur_row, cur_col, values[row_offset + j]))
      {
        return false;
      }
    }
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::add_value(const int row, const int col,
                                             ValueType val) {
  ...
  if (!this->found_entry(row, col, val))
  {
    quit_program(); 
  }
#endif
  ...
  for (size_t j = this->matrix.row[row], e_j = this->matrix.row[row + 1];
       j != e_j; ++j) {
    if (col == this->matrix.col[j]) {
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::add_values(const int *rows, int num_rows,
                                              const int *cols, int num_cols,
...
#ifndef NDEBUG
#ifdef CHECK_STRUCTURE
  if (!this->found_entries(rows, num_rows, cols, num_cols, values))
  {
    quit_program(); 
  }
#endif
  ...
  for (int i = 0; i != num_rows; ++i) {
    int k = 0;
    const int row_offset = i * num_cols;
    for (size_t j = this->matrix.row[rows[i]],
                j_e = this->matrix.row[rows[i] + 1];
         k < num_cols && j != j_e; ++j) 
    {
      ...
      {
        //std::cout << j << " " << k << " " << num_cols << std::endl;
        //std::cout << cols[k] << std::endl;
        //std::cout << this->matrix.col[j] << std::endl;
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::set_values(const int *rows, int num_rows,
                                              const int *cols, int num_cols,
                                              const ValueType *values) {
  assert(this->get_nnz() > 0);
  assert(this->get_num_row() > 0);
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::get_value(const int row, const int col,
                                             ValueType *val) const {
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::get_add_values(const int *rows, int num_rows,
                                                  const int *cols, int num_cols,
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::VectorMultAdd_submatrix(
    const int *rows, int num_rows, const int *cols, int num_cols,
    const int *cols_input, const ValueType *in_values,
  ...
  for (int i = 0; i < num_rows; ++i) {
    int k = 0;
    for (size_t j = this->matrix.row[rows[i]];
         k < num_cols && j < this->matrix.row[rows[i] + 1]; ++j) {
      if (cols[k] == this->matrix.col[j]) {
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::VectorMultAdd_submatrix_vanka(
    const int *rows, int num_rows, const CPU_lVector< ValueType > &invec,
    ValueType *out_values) const {
  assert(this->get_nnz() > 0);
  assert(this->get_num_row() > 0);
  assert(this->get_num_col() > 0);
...
void CPU_CSR_lMatrix< ValueType >::VectorMultAdd_submatrix_vanka(
  ...
  if (casted_invec == NULL) {
    LOG_ERROR("CPU_CSR_lMatrix<ValueType>::VectorMultAdd_submatrix_vanka "
              "unsupported in vector");
    this->print();
    invec.print();
    quit_program();
  }

  this->VectorMultAdd_submatrix_vanka(rows, num_rows, *casted_invec,
                                      out_values);
}

template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::VectorMultAdd(
    const lVector< ValueType > &invec, lVector< ValueType > *outvec) const {
  const CPU_lVector< ValueType > *casted_invec =
      dynamic_cast< const CPU_lVector< ValueType > * >(&invec);
  CPU_lVector< ValueType > *casted_outvec =
  ...
  if ((casted_invec == NULL) && (casted_outvec == NULL)) {
    LOG_ERROR("CPU_CSR_lMatrix<ValueType>::VectorMultAdd unsupported in or out "
              "vector");
    this->print();
    invec.print();
    outvec->print();
...
void CPU_CSR_lMatrix< ValueType >::VectorMultAdd(
  ...
  assert(invec.get_size() == this->get_num_col() || outvec->get_size() == 0 ||
         this->get_num_col() == 0);
  assert((outvec->get_size() == this->get_num_row()) ||
         (invec.get_size() == 0) || this->get_num_row() == 0);

  if (this->get_nnz() > 0) {
    for (size_t i = 0, e_i = this->get_num_row(); i != e_i; ++i) {
      ValueType res = 0.;
      for (size_t j = this->matrix.row[i], e_j = this->matrix.row[i + 1];
           j != e_j; ++j) {
        res += this->matrix.val[j] * invec.buffer[this->matrix.col[j]];
      }
      outvec->buffer[i] += res;
...
template < typename ValueType >
hiflow::la::lMatrix< ValueType > *CPU_CSR_lMatrix< ValueType >::MatrixMult(
    const hiflow::la::lMatrix< ValueType > &inmat) const {

  const CPU_CSR_lMatrix< ValueType > *casted_inmat =
      dynamic_cast< const CPU_CSR_lMatrix< ValueType > * >(&inmat);
...
template < typename ValueType >
hiflow::la::lMatrix< ValueType > *CPU_CSR_lMatrix< ValueType >::MatrixMult(
    const CPU_CSR_lMatrix< ValueType > &inmat) const {
  ...
  LOG_DEBUG(2, "MatrixMatrix Multiplication row = "
                   << this->get_num_row() << " nnz = " << this->get_nnz());
  ...
  if (inmat.get_num_row() > 0) {
    for (int row = 0; row < this->get_num_row(); ++row) {
      for (int col = 0; col < inmat.get_num_col(); ++col) {
        ...
        for (int i = this->matrix.row[row]; i < this->matrix.row[row + 1];
             ++i) {
          c = this->matrix.col[i];
  ...
  outmat = init_matrix< ValueType >(
      mat_nnz, this->get_num_row(), inmat.get_num_col(), mat_name,
      this->get_platform(), this->get_implementation(),
...
template < typename ValueType >
hiflow::la::lMatrix< ValueType > *
CPU_CSR_lMatrix< ValueType >::MatrixMultSupStructure(
    const hiflow::la::lMatrix< ValueType > &inmat) const {
  ...
  if (casted_inmat == NULL) {
    LOG_ERROR("CPU_CSR_lMatrix<ValueType>::MatrixMultSupStructure unsupported "
              "inmatrix");
    this->print();
    inmat.print();
    quit_program();
...
CPU_CSR_lMatrix< ValueType >::MatrixMultSupStructure(
  ...
  CPUsimple_CSR_lMatrix< ValueType > *outmat =
  ...
  LOG_DEBUG(2, "MatrixMultSupStructure number of nodes: "
                   << nrow << " number of nonzeros: " << this->get_nnz());
  ...
  for (int i = 0; i < nrow; ++i) {
    ...
    for (int nz_ii = this->matrix.row[i]; nz_ii < this->matrix.row[i + 1];
         ++nz_ii) {
      k = this->matrix.col[nz_ii];
      // loop on nonzeros of B in row k
      for (int nz_k = inmat.matrix.row[k]; nz_k < inmat.matrix.row[k + 1];
           ++nz_k) {
...
template < typename ValueType >
hiflow::la::lMatrix< ValueType > *
CPU_CSR_lMatrix< ValueType >::MatrixSupSPower(const int p) const {
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::MatrixAdd(
    const hiflow::la::lMatrix< ValueType > &inmat) {
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::MatrixAdd(
    const CPU_CSR_lMatrix< ValueType > &inmat) {
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::GershgorinSpectrum(
    ValueType *lambda_min, ValueType *lambda_max) const {
  ...
  *lambda_max = 0.0;
  ...
  for (int i = 0; i < this->get_num_row(); ++i) {
    ...
    *lambda_min =
        ((*lambda_min) < (diag - offdiag)) ? (*lambda_min) : (diag - offdiag);
    *lambda_max =
        ((*lambda_max) > (diag + offdiag)) ? (*lambda_max) : (diag + offdiag);
  }
}
...
void CPU_CSR_lMatrix< ValueType >::ReadFile(const char *filename) {
  ...
  this->Init(matrix_coo.get_nnz(), matrix_coo.get_num_row(),
             matrix_coo.get_num_col(), matrix_coo.get_name());
...
void CPU_CSR_lMatrix< ValueType >::WriteFile(const char *filename) const {
  ...
  matrix_coo.Init(this->get_nnz(), this->get_num_row(), this->get_num_col(),
                  this->get_name());
...
template < typename ValueType >
hiflow::la::lMatrix< ValueType > *
CPU_CSR_lMatrix< ValueType >::extract_submatrix(const int start_row,
                                                const int start_col,
                                                const int end_row,
  ...
  sub_matrix->Init(sub_mat_nnz, end_row - start_row, end_col - start_col,
                   sub_mat_name);
  ...
  for (int i = start_row; i < end_row; ++i) {
    ...
    for (int j = this->matrix.row[i]; j < this->matrix.row[i + 1]; ++j) {
      ...
      if ((this->matrix.col[j] >= start_col) &&
        ...
        sub_matrix->add_value(rows[sub_mat_ind], cols[sub_mat_ind],
                              this->matrix.val[j]);
...
void CPU_CSR_lMatrix< ValueType >::Reorder(const int *index) {
  ...
  CPU_COO_lMatrix< ValueType > *mat_coo =
      new CPUsimple_COO_lMatrix< ValueType >;
...
template < typename ValueType > void CPU_CSR_lMatrix< ValueType >::ilu0(void) {
  int nrow = this->get_num_row();

  LOG_DEBUG(1, "lMatrix ILU(0) number of nodes: " << this->get_num_row()
                                                  << " number of nonzeros: "
                                                  << this->get_nnz());
  ...
  for (int row_i = 1; row_i < nrow; ++row_i) {
    if (row_i % ((nrow - 1 - (nrow - 1) % 20) / 20) == 0) {
      LOG_DEBUG(3, "Building ILU0 "
                       << float(100 * float((row_i + 1) / float(nrow - 1)))
                       << "%");
    }

    for (int nz_ik = this->matrix.row[row_i]; this->matrix.col[nz_ik] < row_i;
         ++nz_ik) {
      ...
      if (this->matrix.col[diag_k] != this->matrix.col[nz_ik]) {
        LOG_DEBUG(0, "ILU0 facotization - the matrix has vanishing diagonal "
                     "elements!!!");
      }

      this->matrix.val[nz_ik] =
          this->matrix.val[nz_ik] / (1.0 * this->matrix.val[diag_k]);

      for (int nz_ij = nz_ik + 1; nz_ij < this->matrix.row[row_i + 1];
           ++nz_ij) {
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::ilup(const lMatrix< ValueType > &mat,
                                        const int p) {
  ...
  // set levels
  for (int i = 0; i < nrow; ++i) {
    for (int nzj = outmat->matrix.row[i]; nzj < outmat->matrix.row[i + 1];
         ++nzj) {
  ...
  for (int i = 1; i < nrow; ++i) {
    ...
    if (i % ((nrow - 1 - (nrow - 1) % 10) / 10) == 0) {
      LOG_DEBUG(3, "ILU(" << p << ") "
                          << float(100 * float((i + 1) / float(nrow - 1)))
                          << "%");
    }

    for (int nz_ik = outmat->matrix.row[i]; outmat->matrix.col[nz_ik] < i;
         ++nz_ik) {
      ...
      if (lev->matrix.val[nz_ik] <= pp) {
        ...
        // norm elements
        outmat->matrix.val[nz_ik] =
            outmat->matrix.val[nz_ik] / (1.0 * outmat->matrix.val[nz_kk]);

        // LOG_INFO("ilup", "lower nnz=" << nz_ik+1 << " upper=" <<
        // outmat->matrix.row[i+1] );
        for (int nz_ij = nz_ik + 1; nz_ij < outmat->matrix.row[i + 1];
             ++nz_ij) {
          ...
          outmat->matrix.val[nz_ij] =
              outmat->matrix.val[nz_ij] - val_kj * outmat->matrix.val[nz_ik];
          if (lev->matrix.val[nz_ij] > lev_tmp)
            lev->matrix.val[nz_ij] = lev_tmp;
        }
      }
    }
    for (int nzh = outmat->matrix.row[i]; nzh < outmat->matrix.row[i + 1];
         ++nzh)
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::ilu_solve(
    const hiflow::la::lVector< ValueType > &invec,
  ...
  if ((casted_invec == NULL) && (casted_outvec == NULL)) {
    LOG_ERROR(
        "CPU_CSR_lMatrix<ValueType>::ilu_solve unsupported in or out vector");
    this->print();
    invec.print();
    outvec->print();
    quit_program();
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::Multicoloring(int &ncolors,
                                                 int **color_sizes,
    ...
    for (int k = m; k < m1; ++k)
      for (int i = m; i < m1; ++i) {
        ...
        if (dep != j) {
          ...
          // check whether any linked node has same colour as node j if so
          // change colour
          if (nc[dep] == nc[j]) {
            ++nc[j];
          }
  ...
  if (tmp != this->get_num_row()) {
    LOG_ERROR("multi-coloring - check for consistency between number of nodes "
              "- FAILED !!!");
    this->print();
    LOG_ERROR("multi-coloring - not exiting !!!");
  }
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::Levelscheduling(int &nlevels,
                                                   int **level_sizes,
                                                   int **permut_index) const {
  // src code
  // Niels Wegh
  ...
  for (int L = 0; L < nrow; ++L) {
    ...
    for (int k = 0; k < nrow; ++k) {
      ...
      if (depth[k] == L) {
        ...
        // this loops scans the L part if there is any node that has to be
        // calculated before this node
        for (int j = this->matrix.row[k]; j < this->matrix.row[k + 1]; ++j) {
        ...
        for (int j = this->matrix.row[k]; j < this->matrix.row[k + 1];
             ++j) { // makes the depth for the nodes that are behind this node.
          if (this->matrix.col[j] > k) {
            if (besuch[this->matrix.col[j]] == true) {
              LOG_ERROR("CPU_CSR_lMatrix<ValueType>::Levelscheduling - "
    ...
    LOG_DEBUG(5, "CPU_CSR_lMatrix<ValueType>::Levelscheduling - Anzahl der "
                 "Knoten im Level("
                     << L << ") :" << Level[L]);
  }

  LOG_DEBUG(2,
            "CPU_CSR_lMatrix<ValueType>::Levelscheduling  - Anzahl der Level: "
                << nlev << " und done: " << done);
  ...
  for (int i = 0; i < nlevels; ++i) {
    ...
    LOG_DEBUG(3, "CPU_CSR_lMatrix<ValueType>::Levelscheduling - L="
                     << Level[i] - sum);
    sum = Level[i];
  }
}
...
template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::extract_diagelements(
    const int start_i, const int end_i, lVector< ValueType > *vec) const {
  ...
  vec->SetBlockValues(0, end_i - start_i, tmp_buff);

  delete[] tmp_buff;
}

template < typename ValueType >
void CPU_CSR_lMatrix< ValueType >::extract_invdiagelements(
    const int start_i, const int end_i, lVector< ValueType > *vec) const {
...
void CPU_CSR_lMatrix< ValueType >::compress_me() {
  if (this->get_nnz() > 0) {
    ...
    int new_nnz = 0;
    for (int i = 0; i < nrow; ++i) {
      for (int nzj = this->matrix.row[i]; nzj < this->matrix.row[i + 1];
           ++nzj) {
        if (this->matrix.val[nzj] != 0.0)
          ++new_nnz;
      }
    }
    ...
    if (new_nnz < this->get_nnz()) {
      ...
      // init the memory for the matrix
      this->Clear(); //
      this->Init(new_nnz, mat->get_num_row(), mat->get_num_col(),
                 mat->get_name());
      this->name_.append(data_info);
      ...
      for (int i = 0; i < nrow; ++i) {
        nz_row = -1;
        for (int nzj = mat->matrix.row[i]; nzj < mat->matrix.row[i + 1];
             ++nzj) {
