...
CPUsimple_CSR_lMatrix< ValueType >::CPUsimple_CSR_lMatrix(
    int init_nnz, int init_num_row, int init_num_col, std::string init_name) {
...
template < typename ValueType >
void CPUsimple_CSR_lMatrix< ValueType >::VectorMult(
  ...
  if ((casted_invec == NULL) && (casted_outvec == NULL)) {
    LOG_ERROR("CPUsimple_CSR_lMatrix<ValueType>::VectorMult unsupported in or "
              "out vector");
    this->print();
    invec.print();
    outvec->print();
...
template < typename ValueType >
void CPUsimple_CSR_lMatrix< ValueType >::CastFrom(
    const CPU_CSR_lMatrix< double > &other) {
...
template < typename ValueType >
void CPUsimple_CSR_lMatrix< ValueType >::CastFrom(
    const CPU_CSR_lMatrix< float > &other) {
  assert(this->get_num_row() == other.get_num_row());
  assert(this->get_num_col() == other.get_num_col());
  assert(this->get_nnz() == other.get_nnz());
...
template < typename ValueType >
void CPUsimple_CSR_lMatrix< ValueType >::CastTo(
    CPU_CSR_lMatrix< double > &other) const {
  assert(this->get_num_row() == other.get_num_row());
  assert(this->get_num_col() == other.get_num_col());
  assert(this->get_nnz() == other.get_nnz());
...
template < typename ValueType >
void CPUsimple_CSR_lMatrix< ValueType >::CastTo(
    CPU_CSR_lMatrix< float > &other) const {
  assert(this->get_num_row() == other.get_num_row());
  assert(this->get_num_col() == other.get_num_col());
  assert(this->get_nnz() == other.get_nnz());
