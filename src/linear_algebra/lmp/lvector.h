...
/// @author Dimitar Lukarski
///
/// lVector (local Vector) - lmpLAtoolbox (Local Multi-Platform Linear Algebra
/// Tool Box) Provides the base class to all local vectors for different
/// platforms (like CPU,GPU,etc) with different implementations
/// (sequential,parallel,mkl,cblas,...)

template < typename ValueType > class lVector {
public:
  lVector();
  ...
  virtual void add_values(const int *indices, int length,
  ...
  ///                            allocated on the CPU)
  virtual void SetValues(const int *index, const int size,
                         const ValueType *values) = 0;
  ...
  ///                           allocated on the CPU)
  virtual void GetValues(const int *index, const int size,
  ...
  /// @param values - values set (the memory should be
  ///                             allocated on the CPU)
  virtual void SetBlockValues(const int start_i, const int end_i,
                              const ValueType *values) = 0;
  ...
  /// @return values - return values set (the memory should be
  ///                           allocated on the CPU)
  virtual void GetBlockValues(const int start_i, const int end_i,
                              ValueType *values) const = 0;
  ...
  virtual void set_indexset(const int *indexset, const int size) = 0;
  ...
  virtual void
  partial_replace_subvector(const int start_i, const int start_sub_vec,
                            const int size,
  ...
  /// @param sub_vec - the sub vector
  virtual void partial_add_subvector(const int start_i, const int start_sub_vec,
                                     const int size, const ValueType weight,
                                     const lVector< ValueType > &sub_vec) = 0;

  /// Initialize (i.e. allocate) the vector
  /// @param size - size of the vector
  /// @param name - name of the vector
  virtual void Init(const int size, const std::string name) = 0;
  ...
  /// Clone without content of the class - only initialization of the vector is
  /// done
  ...
  /// Plane rotation
  virtual void Rot(lVector< ValueType > *vec, const ValueType &sc,
  ...
  virtual void Rotm(lVector< ValueType > *vec, const ValueType &sparam) = 0;

  /// Returns parameters for a modified Givens rotation
  virtual void Rotmg(ValueType *sd1, ValueType *sd2, ValueType *x1,
                     const ValueType &x2, ValueType *sparam) const = 0;

  /// Returns entries of lvector

  virtual ValueType *GetBuffer() const { return 0; }
