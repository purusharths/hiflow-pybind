...
template < typename ValueType > lMatrix< ValueType >::lMatrix() {
  this->nnz_ = 0;
  this->num_col_ = 0;
  this->num_row_ = 0;
...
lMatrix< ValueType > *lMatrix< ValueType >::CloneWithoutContent() const {
  ...
  lMatrix< ValueType > *new_matrix = init_matrix< ValueType >(
      this->get_nnz(), this->get_num_row(), this->get_num_col(), cloned_name,
...
template < typename ValueType > void lMatrix< ValueType >::Compress() {}
...
void lMatrix< ValueType >::print(std::ostream &out) const {
  ...
  LOG_INFO("lMatrix",
           "name='" << this->get_name()
                    << "' Matrix format=" << this->get_matrix_format_name()
                    << " nnz=" << this->get_nnz()
                    << ", num_row=" << this->get_num_row()
...
template < typename ValueType > void lMatrix< ValueType >::Sync(void) const {
  // do nothing
}
...
template < typename ValueType >
void lMatrix< ValueType >::Pjacobi(const lVector< ValueType > &invec,
...
template < typename ValueType >
void lMatrix< ValueType >::Pgauss_seidel(const lVector< ValueType > &invec,
                                         lVector< ValueType > *outvec) const {
...
template < typename ValueType >
void lMatrix< ValueType >::Psor(const ValueType omega,
...
template < typename ValueType >
void lMatrix< ValueType >::Pssor(const ValueType omega,
                                 const lVector< ValueType > &invec,
                                 lVector< ValueType > *outvec) const {
...
template < typename ValueType >
void lMatrix< ValueType >::Psgauss_seidel(const lVector< ValueType > &invec,
                                          lVector< ValueType > *outvec) const {
  LOG_ERROR(
      "Preconditioner Symmetric Gauss Seidel does not support this matrix");
  this->print();
  quit_program();
}

template < typename ValueType >
void lMatrix< ValueType >::BlockPsgauss_seidel(
    const lVector< ValueType > &invec, lVector< ValueType > *outvec,
    const int start_i, const int end_i) const {
  LOG_ERROR("Preconditioner Block Symmetric Gauss Seidel does not support this "
            "matrix");
  this->print();
  quit_program();
}
...
template < typename ValueType >
void lMatrix< ValueType >::BlocksPsgauss_seidel(
    const lVector< ValueType > &invec, lVector< ValueType > *outvec,
    const int num_blocks) const {
  LOG_ERROR("Preconditioner Blocks Symmetric Gauss Seidel does not support "
            "this matrix");
  this->print();
  quit_program();
}
...
template < typename ValueType >
void lMatrix< ValueType >::ZeroRows(const int *index_set, const int size,
...
template < typename ValueType >
void lMatrix< ValueType >::ZeroCols(const int *index_set, const int size,
                                    const ValueType alpha) {
...
template < typename ValueType >
void lMatrix< ValueType >::add_value(const int row, const int col,
                                     const ValueType val) {
...
template < typename ValueType >
void lMatrix< ValueType >::add_values(const int *rows, int num_rows,
                                      const int *cols, int num_cols,
                                      const ValueType *values) {
  LOG_ERROR("lMatrix::add_values() does not support this matrix");
  this->print();
  quit_program();
}

template < typename ValueType >
void lMatrix< ValueType >::set_values(const int *rows, int num_rows,
                                      const int *cols, int num_cols,
                                      const ValueType *values) {
  LOG_ERROR("lMatrix::set_values() does not support this matrix");
  this->print();
  quit_program();
}

template < typename ValueType >
void lMatrix< ValueType >::get_value(const int row, const int col,
                                     ValueType *val) const {
...
template < typename ValueType >
void lMatrix< ValueType >::get_add_values(const int *rows, int num_rows,
                                          const int *cols, int num_cols,
...
template < typename ValueType >
void lMatrix< ValueType >::VectorMultAdd_submatrix(
...
template < typename ValueType >
void lMatrix< ValueType >::VectorMultAdd_submatrix_vanka(
    const int *rows, int num_rows,
    const hiflow::la::lVector< ValueType > &invec,
    ValueType *out_values) const {
  LOG_ERROR(
      "lMatrix::VectorMultAdd_submatrix_vanka() does not support this matrix");
  this->print();
  quit_program();
}
...
template < typename ValueType >
lMatrix< ValueType > *
lMatrix< ValueType >::extract_submatrix(const int start_row,
                                        const int start_col, const int end_row,
  ...
  cpu_sub_matrix =
      cpu_matrix->extract_submatrix(start_row, start_col, end_row, end_col);
  ...
  sub_matrix->Init(cpu_sub_matrix->get_nnz(), cpu_sub_matrix->get_num_row(),
                   cpu_sub_matrix->get_num_col(), sub_mat_name);
...
template < typename ValueType >
lMatrix< ValueType > *
lMatrix< ValueType >::MatrixMult(const lMatrix< ValueType > &inmat) const {
...
template < typename ValueType >
lMatrix< ValueType > *lMatrix< ValueType >::MatrixMultSupStructure(
...
lMatrix< ValueType >::MatrixSupSPower(const int p) const {
...
template < typename ValueType >
void lMatrix< ValueType >::GershgorinSpectrum(ValueType *lambda_min,
...
template < typename ValueType >
void lMatrix< ValueType >::extract_diagelements(
...
template < typename ValueType >
void lMatrix< ValueType >::extract_invdiagelements(
    const int start_i, const int end_i, lVector< ValueType > *vec) const {
...
template < typename ValueType >
void lMatrix< ValueType >::Levelscheduling(int &nlevels, int **level_sizes,
...
template < typename ValueType >
void lMatrix< ValueType >::Multicoloring(int &ncolors, int **color_sizes,
                                         int **permut_index) const {
...
template < typename ValueType > void lMatrix< ValueType >::ilu0(void) {
  LOG_ERROR("lMatrix::ilu0() does not support this matrix");
  this->print();
  quit_program();
}

template < typename ValueType > void lMatrix< ValueType >::ilup(const int p) {
...
template < typename ValueType > void lMatrix< ValueType >::compress_me() {
  LOG_ERROR("ERROR lMatrix::compress_me() does not support this matrix");
  this->print();
  quit_program();
}

template < typename ValueType > bool lMatrix< ValueType >::issymmetric() {
...
template < typename ValueType > void lMatrix< ValueType >::delete_diagonal() {
...
void lMatrix< ValueType >::delete_lower_triangular() {
  LOG_ERROR(
      "ERROR lMatrix::delete_lower_triangular() does not support this matrix");
  this->print();
  quit_program();
}
...
void lMatrix< ValueType >::delete_strictly_lower_triangular() {
  LOG_ERROR("ERROR lMatrix::delete_strictly_lower_triangular() does not "
            "support this matrix");
  this->print();
  quit_program();
}
...
void lMatrix< ValueType >::delete_upper_triangular() {
  LOG_ERROR(
      "ERROR lMatrix::delete_upper_triangular() does not support this matrix");
  this->print();
  quit_program();
}
...
void lMatrix< ValueType >::delete_strictly_upper_triangular() {
  LOG_ERROR("ERROR lMatrix::delete_strictly_upper_triangular() does not "
            "support this matrix");
  this->print();
  quit_program();
}

template < typename ValueType > void lMatrix< ValueType >::transpose_me() {
