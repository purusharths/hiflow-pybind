...
/// @author Dimitar Lukarski
///
/// lMatrix (local Matrix) - lmpLAtoolbox (Local Multi-Platform Linear Algebra
/// Tool Box) Provides the base class to all local matrices for different
/// platforms (like CPU,GPU,etc) with different implementations
/// (sequential,parallel,mkl,cblas,...)

template < typename ValueType > class lMatrix {
public:
  lMatrix();
  ...
  virtual void Init(const int init_nnz, const int init_num_row,
                    const int init_num_col, const std::string init_name) = 0;
  ...
  /// Set values to a index pairs rows x cols; check for the
  /// structure is not performed
  /// @param rows - the row indices
  /// @param num_rows - number of row indices
  /// @param cols - the column indices
  ...
  virtual lMatrix< ValueType > *extract_submatrix(const int start_row,
                                                  const int start_col,
                                                  const int end_row,
  ...
  /// @return vec - insert the values in this vector (the vector has the
  /// allocated)
  virtual void extract_diagelements(const int start_i, const int end_i,
                                    lVector< ValueType > *vec) const;

  /// Extract the inverse diagonal elements of the matrix
  virtual void extract_invdiagelements(const int start_i, const int end_i,
                                       lVector< ValueType > *vec) const;
  ...
  /// Clone without content of the class - only initialization of the matrix is
  /// done
  ...
  virtual lMatrix< ValueType > *
  MatrixMult(const lMatrix< ValueType > &inmat) const;
  ...
  virtual lMatrix< ValueType > *
  MatrixMultSupStructure(const lMatrix< ValueType > &inmat) const;
  ...
  /// @param invec - input vector (z)
  /// @return outvec - output vector (r)
  virtual void Psor(const ValueType omega, const lVector< ValueType > &invec,
                    lVector< ValueType > *outvec) const;
  ...
  /// @param invec - input vector (z)
  /// @return outvec - output vector (r)
  virtual void Pssor(const ValueType omega, const lVector< ValueType > &invec,
                     lVector< ValueType > *outvec) const;
