...
public:
  ...
  virtual void Init(const int size, const std::string name);
  ...
  virtual hiflow::la::lVector< ValueType > &
  operator=(const hiflow::la::lVector< ValueType > &vec2);
  ...
  virtual hiflow::la::lVector< ValueType > *
  extract_subvector(const int start_i, const int end_i) const;
  ...
  virtual void
  partial_add_subvector(const int start_i, const int start_sub_vec,
  ...
  virtual void set_indexset(const int *indexset, const int size);
  ...
  virtual void
  ElementWiseMult(const hiflow::la::lVector< ValueType > &vec) const;
  virtual void ElementWiseMult(const CPU_lVector< ValueType > &vec) const;

  virtual ValueType *GetBuffer() const { return this->buffer; }
...
public:
  CPUsimple_lVector();
  CPUsimple_lVector(const int size, const std::string name);
...
public:
  CPUopenmp_lVector();
  CPUopenmp_lVector(const int size, const std::string name);
...
public:
  CPUcblas_lVector();
  CPUcblas_lVector(const int size, const std::string name);
