...
class BlockManager;
using ConstBlockManagerPtr = std::shared_ptr< const BlockManager >;
...
public:
  ...
  void Init(const MPI_Comm &comm, 
            ...
            PLATFORM plat,
            IMPLEMENTATION impl,
            ConstBlockManagerPtr block_manager)
  {
...
public:
  CoupledVector< DataType > *params(const PropertyTree &c) {
    CoupledVector< DataType > *newCoupledVector =
      new CoupledVector< DataType >();
    if (c.contains("Platform") && c.contains("Implementation") &&
      ...
      newCoupledVector->Init_la_system(la_sys_.Platform, la_impl_, dummy);
