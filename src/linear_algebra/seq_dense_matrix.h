...
public:
  ...
  int nrows() const { return this->nrows_; }
  /// @return number of columns

  int ncols() const { return this->ncols_; }
  ...
  void MatrixMult(const SeqDenseMatrix< DataType > &inMat,
                  SeqDenseMatrix< DataType > &outMat);

  /// @return eigenvalues in ascending order

  std::vector< DataType > get_eigenvalues() const { return this->eigenvalues_; }
  ...
  void VectorMult(const CPU_lVector< DataType > &invec,
                  CPU_lVector< DataType > &outvec);

  /// Matrix-Vector multiplication
  /// @param invec vector to be multiplied with matrix
  /// @param outvec result vector
  void VectorMult(const std::vector< DataType > &invec,
                  std::vector< DataType > &outvec);
  ...
  void QRFactorize(SeqDenseMatrix< DataType > &Q,
  ...
  void HessenbergReduction(SeqDenseMatrix< DataType > &H,
                           SeqDenseMatrix< DataType > &V);

  /// Forward/Backward solve linear system Ax=b; need to run Factorize() or
  /// Solve() prior to execution
  ...
  /// @param x result vector
  /// @param b right hand side vector
  void ForwardBackward(const std::vector< DataType > &b,
                       std::vector< DataType > &x);

  /// Solve linear system Ax=b
  /// @param x result vector
  /// @param b right hand side vector
  bool Solve(const lVector< DataType > &b, lVector< DataType > &x);

  /// Solve linear system Ax=b
  /// @param x result vector
  /// @param b right hand side vector
  bool Solve(const std::vector< DataType > &b, std::vector< DataType > &x);
  ...
  /// Add smaller Matrix entrywise at position ij. A_(i+k)(j+l) := A_(i+k)(j+l)
  /// + B_kl
  ...
  /// @param B Matrix of smaller dimensions, that is to be inserted.
  void extr(SeqDenseMatrix< DataType > &B, const int i, const int j,
  ...
  inline void set_blocksize(int size) { this->block_ = size; }

  /// Get block size for LU decomposition

  inline int get_blocksize() { return this->block_; }
  ...
  enum PLATFORM get_platform(void) const { return platform_id_; }
  ...
  std::string get_platform_name(void) const { return platform_name_; }

  /// Set implementation
  /// @param impl Implementation ID
  /// @param plat Platform ID
  void set_implementation_and_platform(enum IMPLEMENTATION impl,
                                       enum PLATFORM plat);
...
protected:
  ...
  std::vector< int >
      permutation_; // permutation vector of rows in LU Decomposition
...
private:
  ...
  void MatrixMult_cblas(const SeqDenseMatrix< DataType > *inMat,
                        SeqDenseMatrix< DataType > *outMat);
  /// Compute Matrix-Matrix product with MKL routine
  void MatrixMult_mkl(const SeqDenseMatrix< DataType > *inMat,
                      SeqDenseMatrix< DataType > *outMat);
  /// Compute Matrix-Matrix product with simple routine
  void MatrixMult_simple(const SeqDenseMatrix< DataType > *inMat,
                         SeqDenseMatrix< DataType > *outMat);
  /// Compute Matrix-Matrix product with OpenMP routine
  void MatrixMult_openmp(const SeqDenseMatrix< DataType > *inMat,
                         SeqDenseMatrix< DataType > *outMat);
  ...
  void QRFactorize_simple(SeqDenseMatrix< DataType > &Q,
                          SeqDenseMatrix< DataType > &R);
  /// QR factorization with OpenMP routine
  void QRFactorize_openmp(SeqDenseMatrix< DataType > &Q,
                          SeqDenseMatrix< DataType > &R);
  /// QR factorization with MKL routine
  void QRFactorize_mkl(SeqDenseMatrix< DataType > &Q,
                       SeqDenseMatrix< DataType > &R);
  /// QR factorization with BLAS routine
  void QRFactorize_cblas(SeqDenseMatrix< DataType > &Q,
                         SeqDenseMatrix< DataType > &R);

  /// Hessenberg reduction with simple routine
  void HessenbergReduction_simple(SeqDenseMatrix< DataType > &H,
                                  SeqDenseMatrix< DataType > &V);
  /// Hessenberg reduction with OpenMP routine
  void HessenbergReduction_openmp(SeqDenseMatrix< DataType > &H,
                                  SeqDenseMatrix< DataType > &V);
  /// Hessenberg reduction with MKL routine
  void HessenbergReduction_mkl(SeqDenseMatrix< DataType > &H,
                               SeqDenseMatrix< DataType > &V);
  /// Hessenberg reduction with BLAS routine
  void HessenbergReduction_cblas(SeqDenseMatrix< DataType > &H,
                                 SeqDenseMatrix< DataType > &V);
...
inline const DataType &SeqDenseMatrix< DataType >::operator()(int i,
                                                              int j) const {
  assert(i >= 0);
  assert(i < this->nrows());
  assert(j >= 0);
