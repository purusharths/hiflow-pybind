...
template < class DataType > void CoupledVector< DataType >::InitStructure() 
{
...
template < class DataType >
void CoupledVector< DataType >::Add(const int *indices, const int size_indices,
...
void CoupledVector< DataType >::SetValues(const int *indices,
  ...
  for (int i = 0; i < size_indices; i++) {
    ...
    // LOG_DEBUG(2, "index " << indices[i] << " begin " <<
    // this->ownership_begin_ <<" end " << this->ownership_end_); assert(
    // this->ownership_begin_ <= indices[i] && indices[i] < this->ownership_end_
    // );
  }
#endif // NDEBUG
