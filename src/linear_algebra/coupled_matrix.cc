...
/*
template < class DataType >
void CoupledMatrix< DataType >::Init_la_system(PLATFORM plat,
    IMPLEMENTATION impl,
    MATRIX_FORMAT format) {
  if (this->print_level_ > 0) {
    LOG_INFO("Platform", plat);
  ...
  this->diagonal_ =
    init_matrix< DataType >(0, 0, 0, "diagonal", plat, impl, format);
  assert(this->diagonal_ != nullptr);
  // init empty offdiagonal block
  this->offdiagonal_ =
    init_matrix< DataType >(0, 0, 0, "offdiagonal", plat, impl, format);
  assert(this->offdiagonal_ != nullptr);
}
*/
...
template < class DataType > void CoupledMatrix< DataType >::Compress() {
...
void CoupledMatrix< DataType >::Add(const int global_row_id,
  ...
  if (this->col_ownership_begin_ <= global_col_id &&
    ...
    this->diagonal_->add_value(global_row_id - this->row_ownership_begin_,
                               global_col_id - this->col_ownership_begin_, val);
  }

  // offdiagonal block
  else {
    this->offdiagonal_->add_value(
      global_row_id - this->row_ownership_begin_,
      this->col_couplings_->Global2Offdiag(global_col_id), val);
  }
}
...
void CoupledMatrix< DataType >::GetValues(const int *row_indices, int num_rows,
  ...
  if (this->offdiagonal().get_num_row() > 0) {
    this->offdiagonal().get_add_values(
      ...
      col_ids_offdiag.size(), vec2ptr(col_ind_offdiag), num_cols, values);
  }
}
...
void CoupledMatrix< DataType >::Add(const int *rows, int num_rows,
  ...
  std::vector< DataType > offdiag_values(row_ids.size() *
                                         col_ids_offdiag.size());
  ...
  for (size_t i = 0, e_i = row_ids.size(); i != e_i; ++i) {
    ...
    for (size_t j = 0, e_j = col_ids_offdiag.size(); j != e_j; ++j) {
      offdiag_values[row_offset + j] =
        values[row_offset_old + col_ind_offdiag[j]];
    }
  }

  // add values to submatrices
  if (col_ids_diag.size() > 0)
  {
    this->diagonal_->add_values(vec2ptr(row_ids), row_ids.size(),
                                vec2ptr(col_ids_diag), col_ids_diag.size(),
                                vec2ptr(diag_values));
  }
  if (col_ids_offdiag.size() > 0) 
  {
    this->offdiagonal_->add_values(
      vec2ptr(row_ids), row_ids.size(), vec2ptr(col_ids_offdiag),
      col_ids_offdiag.size(), vec2ptr(offdiag_values));
  }
}
...
void CoupledMatrix< DataType >::SetValues(const int *rows, int num_rows,
  ...
  this->diagonal_->set_values(vec2ptr(row_ids), row_ids.size(),
                              vec2ptr(col_ids_diag), col_ids_diag.size(),
                              vec2ptr(diag_values));
  if (this->offdiagonal_->get_nnz() > 0) {
    this->offdiagonal_->set_values(
      vec2ptr(row_ids), row_ids.size(), vec2ptr(col_ids_offdiag),
      col_ids_offdiag.size(), vec2ptr(offdiag_values));
  }
}
