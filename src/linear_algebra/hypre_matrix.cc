...
void HypreMatrix< DataType >::CreateTransposedFrom(
  ...
  if (this != &other) {
    ...
    hypre_ParCSRMatrixTranspose(*(other.GetParCSRMatrix()), &(this->parcsr_A_), 1);

    hypre_CSRMatrix const* other_diagT = this->parcsr_A_->diag;
    hypre_CSRMatrix const* other_off_diagT = this->parcsr_A_->offd;
    ...
    // global row ids, col ids, and vals
    std::vector<int> global_row_ids, global_col_ids;
    ...
    for (int i = this->ilower_; i <= this->iupper_; ++i) {
      // Get the row
      HYPRE_ParCSRMatrixGetRow(this->parcsr_A_, i, &size, &col_ind, &values);
      // Set the row
      HYPRE_IJMatrixSetValues(A_, 1, &size, &i, col_ind, values);
      // Prepare Hypre for the next operation
      HYPRE_ParCSRMatrixRestoreRow(this->parcsr_A_, i, &size, &col_ind, &values);
    }
