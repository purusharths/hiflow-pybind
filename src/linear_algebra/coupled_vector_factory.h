...
template < class DataType > class CoupledVectorFactory {
  typedef std::map< std::string, CoupledVectorCreator< DataType > * >
      products_t;
  products_t products;
...
public:
  ...
  CoupledVectorCreator< DataType > *Get(const std::string &id) const {
    ...
    if (it != products.end())
      ...
      LOG_ERROR(
          "CoupledVectorFactory::Get: No format of this name registered.");
      return NULL;
    }
  }
};
