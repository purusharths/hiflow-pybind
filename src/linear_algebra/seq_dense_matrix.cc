...
bool SeqDenseMatrix< DataType >::contains_nan() {
  for (size_t i=0, e=this->val_.size(); i!=e; ++i)
  {
    if (std::isnan(this->val_[i]))
    {
      return true;
    }
  }
...
template < class DataType >
void SeqDenseMatrix< DataType >::set_implementation_and_platform(
    enum IMPLEMENTATION impl, enum PLATFORM plat) {
  ...
  LOG_ERROR("set_implementation_and_platform() incompatibility "
...
void SeqDenseMatrix< DataType >::Axpy(const SeqDenseMatrix< DataType > &B,
...
void SeqDenseMatrix< DataType >::ins(const SeqDenseMatrix< DataType > &B,
                                     const int row, const int col) {
...
void SeqDenseMatrix< DataType >::extr(SeqDenseMatrix< DataType > &B,
...
void SeqDenseMatrix< Datatype >::transpose_me(
    SeqDenseMatrix< Datatype > &trans_mat) {
...
void SeqDenseMatrix< DataType >::MatrixMult(
    const SeqDenseMatrix< DataType > &inMat,
  ...
  if (platform_id_ == CPU) {
    ...
    case NAIVE:
      ...
      this->MatrixMult_simple(&inMat, &outMat);
    ...
    case OPENMP:
      ...
      this->MatrixMult_openmp(&inMat, &outMat);
    ...
    case MKL:
      ...
      this->MatrixMult_mkl(&inMat, &outMat);
    ...
    case BLAS:
      ...
      this->MatrixMult_cblas(&inMat, &outMat);
...
void SeqDenseMatrix< DataType >::VectorMult(
    const CPU_lVector< DataType > &invec, CPU_lVector< DataType > &outvec) {
  ...
  if (platform_id_ == CPU) {
    ...
    case NAIVE:
      ...
      this->VectorMult_simple(invec.buffer, outvec.buffer);
    ...
    case OPENMP:
      ...
      this->VectorMult_openmp(invec.buffer, outvec.buffer);
    ...
    case MKL:
      ...
      this->VectorMult_mkl(invec.buffer, outvec.buffer);
    ...
    case BLAS:
      ...
      this->VectorMult_cblas(invec.buffer, outvec.buffer);
...
void SeqDenseMatrix< DataType >::VectorMult(
    const std::vector< DataType > &invec, std::vector< DataType > &outvec) {
...
void SeqDenseMatrix< DataType >::ForwardBackward(
    const std::vector< DataType > &b, std::vector< DataType > &x) {
...
void SeqDenseMatrix< DataType >::ForwardBackward(const lVector< DataType > &b,
                                                 lVector< DataType > &x) {
...
bool SeqDenseMatrix< DataType >::Solve(const lVector< DataType > &b,
                                       lVector< DataType > &x) {

  bool success = Factorize();
...
bool SeqDenseMatrix< DataType >::Solve(const std::vector< DataType > &b,
...
bool SeqDenseMatrix< DataType >::Factorize() {
  ...
  if (platform_id_ == CPU) {
    ...
    case NAIVE:
      ...
      return Factorize_simple();

      break;

    case OPENMP:

      return Factorize_openmp();

      break;

    case MKL:

      return Factorize_mkl();

      break;

    case BLAS:

      return Factorize_cblas();
...
void SeqDenseMatrix< DataType >::QRFactorize(SeqDenseMatrix< DataType > &Q,
                                             SeqDenseMatrix< DataType > &R) {
...
void SeqDenseMatrix< DataType >::HessenbergReduction(
    SeqDenseMatrix< DataType > &H, SeqDenseMatrix< DataType > &V) {
  if (platform_id_ == CPU) {
...
void SeqDenseMatrix< DataType >::HessenbergReduction_simple(
    SeqDenseMatrix< DataType > &H, SeqDenseMatrix< DataType > &V) {
  ...
  for (int i = ncols_ - 1; i >= 0; i--) {
    ...
    // EV_matrix_new((k+1:end,k+1:end) = EV_matrix_new(k+1:end,k+1:end) -
    // 2*x*(x'*EV_matrix_new(k+1:end,k+1:end))
    for (int j = i + 1; j < nrows_; ++j) {
...
void SeqDenseMatrix< DataType >::HessenbergReduction_openmp(
    SeqDenseMatrix< DataType > &H, SeqDenseMatrix< DataType > &V) {
#ifdef WITH_OPENMP
  // Initialize H to this
#pragma omp parallel for
...
template < class DataType >
void SeqDenseMatrix< DataType >::MatrixMult_simple(
    const SeqDenseMatrix< DataType > *inMat,
    SeqDenseMatrix< DataType > *outMat) {
  outMat->Zeros();
...
template < class DataType >
void SeqDenseMatrix< DataType >::MatrixMult_openmp(
    const SeqDenseMatrix< DataType > *inMat,
    SeqDenseMatrix< DataType > *outMat) {
#ifdef WITH_OPENMP
  outMat->Zeros();
...
void SeqDenseMatrix< DataType >::compute_eigenvalues_and_vectors_simple() {
  ...
  EV_matrix_new.set_implementation_and_platform(implementation_id_,
                                                platform_id_);
  ...
  SeqDenseMatrix< DataType > Q_local, Z_new_local, R_local, Z_local, EV_local,
      EV_new_local;
...
template < class DataType >
void SeqDenseMatrix< DataType >::VectorMult_simple(const DataType *invec,
                                                   DataType *outvec) {
  for (int i = 0; i != nrows_; ++i) {
    outvec[i] = 0;
    const int index = i * ncols_;
...
template < class DataType >
void SeqDenseMatrix< DataType >::VectorMult_openmp(const DataType *invec,
                                                   DataType *outvec) {
#ifdef WITH_OPENMP
...
template < class DataType >
bool SeqDenseMatrix< DataType >::Factorize_simple() {
  ...
  for (int i = 0; i < nrows_; i += block_) {
    // number of row or column, respectively, of the beginning of the block of
    // the delayed updates
    j = i + block_;
    ...
    for (int k = i; k < blockend; ++k) {
      ...
      if (max_value <= std::numeric_limits<DataType>::epsilon()) {
        LOG_ERROR("The diagonal element of the triangular factor of A, U("
                  << k << "," << k
                  << ") is zero up to machine precision, so that A is singular; the solution could not "
                     "be computed");
        return false;
      }
...
template < class DataType >
bool SeqDenseMatrix< DataType >::Factorize_openmp() {
#ifdef WITH_OPENMP
...
#pragma omp critical
      ...
      if (max_value == 0.0) {
        LOG_ERROR("The diagonal element of the triangular factor of A, U("
                  << k << "," << k
                  << ") is zero, so that A is singular; the solution could not "
                     "be computed");
        return false;
      }
...
template < class DataType >
void SeqDenseMatrix< DataType >::QRFactorize_simple(
    SeqDenseMatrix< DataType > &Q, SeqDenseMatrix< DataType > &R) {
...
template < class DataType >
void SeqDenseMatrix< DataType >::QRFactorize_openmp(
    SeqDenseMatrix< DataType > &Q, SeqDenseMatrix< DataType > &R) {
#ifdef WITH_OPENMP
