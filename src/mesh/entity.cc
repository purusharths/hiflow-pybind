...
Entity::Entity() : mesh_(0), tdim_(-1), index_(-1) {
  vertices_.clear();
  coordinates_.clear();
  children_.clear();
}
...
AlignNumber Entity::get_align_number() const {
  assert(is_valid());
  return mesh_->get_align_number(tdim_, index_);
}
...
bool Entity::is_valid() const {
  ...
  return tdim_ >= 0 && tdim_ <= mesh_->tdim() && index_ >= 0 &&
         index_ < mesh_->num_entities(tdim_);
}

std::ostream &operator<<(std::ostream &os, const Entity &entity) {
  os << "Entity " << entity.index() << " [id = " << entity.id()
     << ", tdim = " << entity.tdim() << ", gdim = " << entity.gdim() << "]\n";
  os << "\tVertices = { ";
  std::copy(entity.begin_vertex_ids(), entity.end_vertex_ids(),
            std::ostream_iterator< Id >(os, " "));
  os << "}\n";
  ...
  for (int v = 0; v < entity.num_vertices(); ++v) {
    ...
    std::copy(coords.begin(), coords.end(),
              std::ostream_iterator< Coordinate >(os, " "));
    os << ")";
  }
  os << "\n";
  return os;
