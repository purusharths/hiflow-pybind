...
public:
  ...
  /// @param[in] history_index index of mesh in array mesh_history of underlying
  /// database db
  /// @param[in] cell_is_local flag indicating for each cell whether it is
  /// locally owned or contained in ghost layer. can be empty array
  /// @param[in] period MasterSlave struct describing periodic boundaries
  MeshPXest(TDim tdim, GDim gdim, MeshPXestDatabasePtr db,
            ...
            std::map< int, ConstMeshPtr > const *mesh_history = nullptr);
  ...
  /// \brief create a new mesh by refining / coarsening this one. The output is
  /// always balanced. <br> Convention for refinement flags: <br> flags.size ==
  /// cells.size <br> flags[i] = 0: keep cell i <br> flags[i] = 1: refine cell i
  /// <br> flags[i] < 0: refine family F = {c_1, .. c_n} of cells iff \sum{i =
  /// 1}^{n} flags[c_i] <= - n
  /// @param[in] refinements refinement indicator for each cell in current mesh
  /// (>= 0: refine one level, <0: do nothing)
  /// @return pointer to newly created refined mesh
  MeshPtr refine(std::vector< int > &flags) const;
  ...
  std::vector< Id > get_children_cell_ids(EntityNumber cell_index) const;
  ...
  std::vector< EntityNumber > get_sibling_cell_indices(EntityNumber cell_index) const;
  ...
  MeshPXestDatabasePtr get_db() const {
    ...
    MeshPXestDatabasePtr pXest_db =
        boost::static_pointer_cast< MeshPXestDatabase >(this->db_);
    assert(pXest_db != 0);
    return pXest_db;
  }

  // STRUCT_TODO -> AdaptionP4est
  /// \brief refine sequential master mesh uniformly until a minimum number of
  /// cells is contained
  /// @param[in] num_ref_steps number of times, the mesh should be refined
  /// uniformly
  ...
  /// \brief create mesh of type MeshDbView of topological dimension tdim-1 that contains 
  /// the boundary facets
  MeshPtr extract_boundary_mesh() const;
  
  /// \brief returns information if a given cell belongs to the locally owned
  /// subdomain (i.e. no ghost cell)
  /// @param[in] cell_index index of some cell
  /// @return true if cell is locally owned
  bool cell_is_local(EntityNumber cell_index) const;

  void save(std::string filename, const MPI_Comm &comm) const;

  void load(std::string filename, const MPI_Comm &comm);

  /// \brief copy data from reference mesh. Here, the underlying databse is not
  /// copied, only its pointer
  /// @param[in] pointer to mesh which should be copied
  void copy_from(ConstMeshPtr mesh);

  /// \brief copy data from reference mesh, inlcuding the underlying database
  /// @param[in] pointer to mesh which should be copied
  void deep_copy_from(ConstMeshPtr mesh);

  /// \brief set flag for type of connectivity in balance mode. Balance ensures
  /// that two cell connected in the specified way differ in at most one level
  /// of refinement.
  /// @param[in] mode (2D: 0: Edge connectivity, 1: corner connectivity; 3D: 0:
  /// Face connectivity, 1: edge connectivity, 2: corner connectivity)
  void set_connection_mode(int mode);

  /// \brief set flag for path mode. If set to true, then additional cells are
  /// selected for refinement in order to obtain a mesh which is uniformly
  /// coarsenable
  /// @param[in] flag
  void set_patch_mode(bool flag);

  /// \brief checks if mesh is in such a state, that each family of cells is
  /// coarsenable.
  /// @return true if mesh is uniformyl coarsenable
  bool is_uniformly_coarsenable() const;

  int get_history_index() const { return history_index_; }

  // get level of refinement of cell with given index
  int get_cell_ref_level(EntityNumber cell_index) const {
    return this->pXest_db_->get_cell_ref_level(
        this->get_id(this->tdim(), cell_index));
  }

protected:
  // STRUCT_TODO -> AdaptionP4est
  /// \brief refines the mesh in order to ensure haviong at most one hanging
  /// node per facet. This is done by calling the p4est_balance_ext routine.
  /// @param[out] add_cells new cells that were created during the balanciing
  /// process and which should be added to the mesh
  /// @param[out] rm_cells cells that were refined during the balancing process
  /// and which should be removed from the mesh
  /// @param[in] connection_mode type of connectivity
  void balance(std::vector< Id > &add_cells, std::vector< Id > &rm_cells,
  ...
  void
  find_coarsenable_cells(SortedArray< EntityNumber > &coarsenable_cells) const;

  /// \brief yields a mesh which is uniformly coarsenable by refining non
  /// coarsenable cells
  /// @return pointer to newly created, refined mesh
  MeshPtr make_mesh_coarsenable();

  /// \brief for each local quadrant, store index of corresponding cell in
  /// associated QuadData. Stores -1 in quadrants whose corresponding cell is
  /// not in current mesh
  /// @param[in] update_ghost flag indicating whether quadrants in ghost layer
  ...
  MeshPtr add_ghost_cells(MPI_Comm comm, int layer_width) const;

  /// \brief Access the id of an ancestor of a cell entity
  /// @param[in] cell_id Id of the descendant
  /// @param[in] level_diff number of generations between cell and its ancestor
  /// (e.g. for parent: level_diff = 1)
  ...
  /// @param[in] level_diff number of generations between cell and its
  /// descendants (e.g. for children: level_diff = 1)
  ...
  // STRUCT_TODO -> GhostP4est
  /// \brief Put data that should be exchanged into mirror quads (local quads
  /// which are in ghost layer of other processes)
  void init_ghost_data() const;

  // STRUCT_TODO -> GhostP4est
  /// \brief exchange data in ghost layers and put it into builder, sub_domains
  /// and remote_indices
  /// @param[out] ids of all cells in local_mesh + ghost_layer
  void exchange_ghost_data(std::vector< SortedArray< Id > > &new_cells) const;

  // STRUCT_TODO -> GhostP4est
  /// \brief put cell indices into ghost data after exchanging ghost cells ->
  /// communicate indices of newly created ghost cells
  ...
  // STRUCT_TODO -> GhostP4est
  /// \brief compute subdomain index and remote index for each cell in local
  /// mesh
  /// @param[out] sub_domain
  /// @param[out] remote_indices
  void compute_subdomains_and_remote_indices(
  ...
  /// \brief set flags indicating for each cell, whether it is locally owned or
  /// not
  /// @param[in] flags
  void set_cell_flags(std::vector< bool > &flags);

  // STRUCT_TODO -> AdaptionDataBase
  /// \brief recursively refine a given cell according to a given tree. This
  /// functions adds new cell entities to the underlying database for each node
  /// in the tree, i.e. also for each non-leaf node.
  ...
  /// @param[out] tree_node_numbers the corresponding index in the refined_cell
  /// array of the tree for each created cell
  ...
  std::vector< Id >
  compute_refined_cells_recursive(const Entity &cell, RefinementTree *tree,
  ...
  // STRUCT_TODO -> GhostP4est
  /// \brief pointer to temporary data that has to be communicated when
  /// exchanging ghost data
  ...
  // STRUCT_TODO -> put into MeshDbView
  /// \brief flag indicating for each cell whether it is local or in the ghost
  /// layer
...
private:
  ...
  friend MeshPtr hiflow::partition_and_distribute_pXest(
      const mesh::MeshPtr master_mesh, const int master_rank,
      const MPI_Comm &comm, int &uniform_ref_steps);
...
public:
  ...
  /// \brief Constructor to use when building a Mesh with an existing
  /// MeshDatabase
  /// @param[in] db pointer to database
  explicit MeshPXestBuilder(
      MeshDatabasePtr db,
      std::vector< MasterSlave > period = std::vector< MasterSlave >(0),
      bool is_rectangular = false);
  ...
  explicit MeshPXestBuilder(
      int tdim, int gdim, const MeshPXestDatabasePtr db,
      const std::vector< EntityHandle > &cells,
      std::vector< MasterSlave > period = std::vector< MasterSlave >(0),
      bool is_rectangular = false);

  /// \brief Constructor to use when building a Mesh without an existing
  /// MeshDatabase
  /// @param[in] tdim topological dimension of mesh
  /// @param[in] gdim geometric dimension of mesh
  explicit MeshPXestBuilder(
  ...
  void set_p4est(p4est_t *forest) { pXest_db_->set_p4est_forest(forest); }

  /// \brief set p8est forest object
  /// \param[in] forest pointer to forest

  void set_p8est(p8est_t *forest) { pXest_db_->set_p8est_forest(forest); }
#endif
  ...
  /// @param[in] coord struct that describes coordinates of quadrant in forest
  /// which corresponds to given entity
  /// @param[in] which_mapping 0: mapping for coarse mesh, 1: mapping for
  /// refined mesh
  void add_entity_to_quad_coord(int tdim, Id entity_id, QuadCoord &coord,
                                int which_mapping);
  ...
  /// @param[in] tree_to_vertices array containing vertex ids belonging to each
  /// cell
  void set_conn_data(int num_vertices, int num_cells,
  ...
  std::vector< Id > get_cells() { return this->cells_; }
