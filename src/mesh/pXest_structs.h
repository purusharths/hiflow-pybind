...
typedef std::map< Id, QuadCoord > EntityToQuadMap;
typedef std::vector< EntityToQuadMap > EntityToQuadMapping;

/// \brief struct containing all information to find the corresponding p4est
/// quadrant for a given hiflow entitty

struct QuadCoord {
  // constructor

  QuadCoord()
      : tree(-1), level(-1), x(-1), y(-1), z(-1), localId(-1), tdim(-1),
        coarseId(-1) {}

  QuadCoord(int32_t t, int l, int32_t co_x, int32_t co_y, int32_t co_z, int Id,
            int dim, mesh::Id coarse_id)
      : tree(t), level(l), x(co_x), y(co_y), z(co_z), localId(Id), tdim(dim),
        coarseId(coarse_id) {}
  ...
  // comparison operator, checks if two quad_maps point to the same quadrant in
  // p4est forest
  ...
  void print() {
    std::cout << "tree: " << tree << " level: " << level << " x: " << x
              << " y: " << y << " z: " << z << " localId: " << localId
              << " tdim: " << tdim << " coarseId: " << coarseId << std::endl;
  }
  ...
  int32_t y;
  int32_t z;
  // Id of entity inside of corresponding quadrant. If entity is a cell: 0,
  // facet:0-6, edge: 0-11, vertex:0-7
  int localId;
...
struct ForestInitData {
  ...
  ForestInitData(int mpi_rank) : rank(mpi_rank) {}
...
/// \brief struct that contains that should be communicated with ghost cells.
/// '22' -> tdim=2, gdim=2
...
struct GhostCommData22 {
  ...
  // size = #cells x num_vert(cell) + #facets x num_vert(facet) = 12
  int connections[P4EST_MAXLEVEL * 12];

  // size = #cells x num_vert(cell)*gdim + #facetsx num_vert(facet)*gdim = 24
  double coords[P4EST_MAXLEVEL * 24];

  // size = #cells + #facets = 1 + 4 = 5
  int tdims[P4EST_MAXLEVEL * 5];
  int gdim;

  // size = 2* (#cells + #facets) = 10
  int offsets[P4EST_MAXLEVEL * 10];

  // size = #cells + #facets = 5
  int material_numbers[P4EST_MAXLEVEL * 5];

  // size = #cells + #facets = 5
  int align_numbers[P4EST_MAXLEVEL * 5];

  // size = #cells + #facets + 1 = 6
  int conn_offsets[P4EST_MAXLEVEL * 6];

  // size = #cells + #facets + 1 = 6
  int coord_offsets[P4EST_MAXLEVEL * 6];
...
struct GhostCommData33 {
  ...
  // size = #cells x num_vert(cell) + #facets x num_vert(facet) = 32
  int connections[P4EST_MAXLEVEL * 32];

  // size = #cells x num_vert(cell)*gdim + #facetsx num_vert(facet)*gdim = 24 +
  // 72 = 96
  double coords[P4EST_MAXLEVEL * 96];

  // size = #cells + #facets = 1 + 6 = 7
  int tdims[P4EST_MAXLEVEL * 7];
  int gdim;

  // size = 2* (#cells + #facets) = 14
  int offsets[P4EST_MAXLEVEL * 14];

  // size = #cells + #facets = 7
  int material_numbers[P4EST_MAXLEVEL * 7];

  // size = #cells + #facets = 7
  int align_numbers[P4EST_MAXLEVEL * 7];

  // size = #cells + #facets + 1 = 8
  int conn_offsets[P4EST_MAXLEVEL * 8];

  // size = #cells + #facets + 1 = 8
  int coord_offsets[P4EST_MAXLEVEL * 8];
...
/// \brief struct that contains data which can be accessed by p4est_init_fn when
/// initialzing the forest

struct QuadData {

  QuadData() { init(-1, -1, -1, -1, -1, -1, -1, -1, true, false, 0); }

  QuadData(treeId tree, int rank, int t_dim) {
    init(-1, -1, -1, -1, tree, tree, rank, t_dim, true, false, 0);
  }

  QuadData(const QuadData &data) { set(data); }

  void init(Id id, Id id_remote, EntityNumber index, EntityNumber index_remote,
            treeId tree, treeId tree_remote, int rank, int t_dim, bool status,
            bool ref, int coa) {
    cell_id[0] = id;
  ...
  inline void set_cell_index(EntityNumber index) { cell_index = index; }

  inline void set_remote_cell_index(EntityNumber index) {
    cell_index_remote = index;
  }

  inline void set_cell_id(Id id, int level) { cell_id[level] = id; }

  inline void set_remote_cell_id(Id id, int level) {
    cell_id_remote[level] = id;
  }

  inline void set_active(bool flag) { active = flag; }

  inline void set_refine(bool flag) { refine = flag; }

  inline void set_coarsen(int flag) { coarsen = flag; };

  inline void set_owner_rank(int rank) { owner_rank = rank; };

  inline Id get_cell_id(int level) { return this->cell_id[level]; }
  ...
  inline EntityNumber get_cell_index() { return this->cell_index; }

  inline treeId get_tree_id() { return this->tree_id; }

  inline treeId get_remote_tree_id() { return this->tree_id; }

  inline int get_owner_rank() { return this->owner_rank; }

  inline bool is_active() { return this->active; }

  inline bool do_refine() { return this->refine; }

  inline int do_coarsen() { return this->coarsen; }

  void print() {
    std::cout << " owner rank: " << owner_rank << " is active: " << active
              << std::endl
              << " tree_id: " << tree_id
              << " remote_tree_id: " << tree_id_remote << std::endl
              << " cell index: " << cell_index
              << " remote cell_index: " << cell_index_remote << std::endl
              << " tdim: " << tdim << std::endl
  ...
  // if sum_{quad in family} quad.coarsen <= -1 * #{quad in family}, then family
  // is coarsened
  int coarsen;
};
