...
public:
  ...
  virtual void get_coordinates(Id vertex_id, std::vector< Coordinate >& coords) const;
  
  /// \brief Access the coordinates of a list of vertices
  virtual std::vector< Coordinate >
  get_coordinates(const std::vector< Id > &vertex_ids) const;

  virtual void
  get_coordinates(const std::vector< Id > &vertex_ids, std::vector< Coordinate >& coords) const;
  
  /// \brief for each vertex x: x -> Ax + b
  ...
  virtual bool find_closest_vertex(const Coordinate *x, double eps,
                                   Id *closest_vertex) const;
  ...
  /// 0 < d2 <= d1 <= D
  virtual void compute_incident_entities(TDim d,
                                         const SortedArray< Id > &d_entities,
                                         Connectivity &d_d_connectivity) const;

  virtual void
  compute_incident_entities(TDim d1, TDim d2,
  ...
  virtual void vertex_entity_incidence(TDim d,
                                       const SortedArray< Id > &vertices,
                                       const SortedArray< Id > &d_entities,
  ...
  /// \brief Set the axis alignement type of a cell. <br>
  /// Cell is aligned to axis i <:=> inverse cell transformation for coordinate
  /// x_i is an 1D-affine function that is independend of other directions j !=
  /// i <br> align_number := 4 * (z_align == true) + 2 * (y_aligned == true) + 1
  /// * (x_aligned == true) \param tdim       the topological dimension of the
  /// entity \param id         id for the entity \param align      new align
  /// number for the entity
  virtual void set_align_number(TDim d, Id id, AlignNumber align);

  /// \brief Returns the material number of an entity
  virtual MaterialNumber get_material_number(TDim d, Id id) const;

  /// \brief Returns the align number of an entity
  virtual AlignNumber get_align_number(TDim d, Id id) const;

  // virtual void reset_coordinates(Id vertex_id, std::vector<Coordinate>
  // coords);
  ...
  /// \brief creates an EntityPackage containing all entities of given
  /// topological dimension and Ids
  virtual void create_entity_package(int tdim, const std::vector< Id > &ids,
                                     EntityPackage *entities) const;

  /// \brief make a copy of complete database and return pointer to copy
  virtual void copy_from(const MeshDatabasePtr db);

  /// \brief make a copy of complete database and return pointer to copy.
  /// Compared to copy_from, this function copies the mesh objects stored in
  /// mesh_history, rather than copying the corresponding MeshPtrs only.
...
protected:
  ...
  /// Alignment numbers
  std::vector< AlignNumber > align_numbers_[2];
};
...
class VertexConnectivity : public Connectivity {
  // TODO(Staffan): maybe this class can be refactored to use Connectivity
  // instead by moving functions outside
public:
  ...
  bool find_entity(const std::vector< Id > &entity_vertices, const TDim tdim,
                   const MeshDatabase *db, Id &id) const;
};
...
private:
  ...
  std::vector< Id >
      vertices_; // vector of vertex Id:s sorted by distance from origin
  const GDim gdim_;
  const std::vector< Coordinate >
      &coordinates_; // reference to coordinates_ vector in database
};
