    ...
    : GeometricSearch<DataType, DIM> (mesh) 
{
  ...
  {
    this->bbox_max_[d] = bbox.max(d);
    this->bbox_min_[d] = bbox.min(d);
  }
                                                    ...
                                                    std::vector< Coord > &ref_points) const 
{
  ...
  {
    LOG_DEBUG(1, "Point " << string_from_range(point.begin(), point.end())
...
template <class DataType, int DIM>
Vec<DIM, DataType> GridGeometricSearch<DataType, DIM>::find_closest_point(const Coord &point,
                                                                          int &facet_index,
                                                                          int material_id,
                                                                          bool &success) const 
{
...
template <class DataType, int DIM>
Vec<DIM, DataType> GridGeometricSearch<DataType, DIM>::find_closest_point_parallel( const Coord &point, int &facet_index,
...
template <class DataType, int DIM>
std::vector< Vec<DIM, DataType> > GridGeometricSearch<DataType, DIM>::intersect_boundary( const Coord &point_a,
...
void GridGeometricSearch<DataType, DIM>::compute_cell_index_map() 
{
  ...
  inverse_cell_index_map_.clear();
  cell_index_map_.resize(grid_->get_num_cells());
  inverse_cell_index_map_.resize(this->mesh_->num_entities(tdim));

  std::vector< MasterSlave > period = this->mesh_->get_period();

  // iterate mesh cells
  for (mesh::EntityIterator it = this->mesh_->begin(tdim), end_it = this->mesh_->end(tdim);
       it != end_it; ++it) {
    /*
                    // skip ghost cells
                    ...
                    {
                        int remote_index;
                        this->mesh_->get_attribute_value ( "_remote_index_", tdim,
       it->index ( ), &remote_index ); if ( remote_index != -1 ) continue;
                    }
     */
         ...
         ind_it != grid_cell_indices.end(); ++ind_it) 
    {
      ...
      inverse_cell_index_map_[it->index()].push_back(*ind_it);
    }
  }
}
                                                               ...
                                                               Coord &ref_coord) const 
{
  ...
  else 
  {
    ...
    if (!found) 
    {
      LOG_DEBUG(3, "Point " << string_from_range(point.begin(), point.end())
