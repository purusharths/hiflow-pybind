...
MeshPXestDatabase::MeshPXestDatabase(TDim tdim, GDim gdim)
    : MeshDatabase(tdim, gdim)
#ifdef WITH_P4EST
      ,
      p4est_conn_(nullptr), p8est_conn_(nullptr), p4est_forest_(nullptr),
      p8est_forest_(nullptr), p4est_ghost_(nullptr), p8est_ghost_(nullptr),
      ctype4_(P4EST_CONNECT_FACE), ctype8_(P8EST_CONNECT_FACE),
      layer_width_(-1), quadrant_arrays_sorted_(false)
#endif
{
...
void MeshPXestDatabase::build(TDim dim, const SortedArray< Id > &cells,
  ...
  tmp_d_entities.reserve(d_entities.data().size() +
                         cells.size() * num_sub_entities);
  tmp_d_entities.insert(tmp_d_entities.begin(), d_entities.data().begin(),
                        d_entities.data().end());
  ...
  /* std approach */
  // d_entities.reserve (d_entities.data().size() + cells.size() *
  // num_sub_entities);
  ...
  for (IdIterator cell_it = cells.begin(); cell_it != cells.end(); ++cell_it) {
    ...
    const std::vector< Id > cell_vertices(begin_vertices(tdim_, cell_id),
                                          end_vertices(tdim_, cell_id));
    const CellType &cell_type =
        CellType::get_instance(tdim_, cell_vertices.size());
  ...
  tmp_d_entities.erase(unique(tmp_d_entities.begin(), tmp_d_entities.end()),
                       tmp_d_entities.end());
...
void MeshPXestDatabase::set_conn_data(int num_vertices, int num_cells,
                                      std::vector< double > &vertices,
                                      std::vector< int > &tree_to_vertices) {
  assert(!vertices.empty());
  assert(!tree_to_vertices.empty());

  this->conn_vertices_.resize(0);
  this->conn_vertices_.insert(this->conn_vertices_.end(), vertices.begin(),
                              vertices.end());

  this->tree_to_vertices_.resize(0);
  this->tree_to_vertices_.insert(this->tree_to_vertices_.end(),
                                 tree_to_vertices.begin(),
                                 tree_to_vertices.end());
...
#ifdef WITH_P4EST
  if (this->p4est_forest_ != nullptr)
  {
    p4est_destroy(this->p4est_forest_);
  }
  
  if (this->p4est_conn_ != nullptr) 
  {
    p4est_connectivity_destroy(this->p4est_conn_);
  }
  if (this->p8est_forest_ != nullptr)
  {
    p8est_destroy(p8est_forest_);
  }
  if (this->p8est_conn_ != nullptr) {
    p8est_connectivity_destroy(p8est_conn_);
  }
#endif
  this->cell_to_mesh_mapping_.clear();
}

void MeshPXestDatabase::add_cell_to_mesh_maps(
    const std::vector< Id > &cells, std::vector< int > &history_indices) {
  assert(cells.size() == history_indices.size());
  LOG_DEBUG(2, "Add the following cell id to mesh index maps: "
                   << string_from_range(cells.begin(), cells.end()) << "  =>  "
                   << string_from_range(history_indices.begin(),
                                        history_indices.end()));
  ...
  for (int l = 0; l < cells.size(); ++l) {
    ...
    std::map< Id, int >::iterator it =
        this->cell_to_mesh_mapping_.find(cell_id);
...
void MeshPXestDatabase::set_mesh(Mesh *mesh_ptr, int history_index) {
  assert(history_index >= 0);
  LOG_DEBUG(2, "Existing MeshPtr at index "
                   << history_index << " is "
                   << this->mesh_history_[history_index]
                   << ", to be inserted MeshPtr is " << mesh_ptr);

  std::pair< std::map< int, Mesh * >::iterator, bool > ret =
      this->mesh_history_.insert(std::make_pair(history_index, mesh_ptr));

  // if element already exists -> update
  if (!ret.second) {
    this->mesh_history_[history_index] = mesh_ptr;
  }
  LOG_DEBUG(2, "New MeshPtr at index " << history_index << " is "
                                       << this->mesh_history_[history_index]);
}

Mesh *MeshPXestDatabase::get_mesh(int history_index) {
  assert(history_index >= 0);
  assert(history_index <= mesh_history_.size());
  assert(this->mesh_history_[history_index] != nullptr);
  return this->mesh_history_[history_index];
}

void MeshPXestDatabase::add_entity_to_quad_coord(int tdim, Id entity_id,
                                                 QuadCoord &coord,
                                                 int which_mapping) {
  assert(tdim >= 0);
  ...
  std::pair< EntityToQuadMap::iterator, bool > ret =
      map->insert(std::make_pair(entity_id, coord));
...
void MeshPXestDatabase::permute_entity_to_quad_map(
    const std::vector< size_t > &perm, int tdim, int which_mapping) {
...
QuadCoord MeshPXestDatabase::get_entity_to_quad_coord(int tdim, Id entity_id,
                                                      int which_mapping) const {
  assert(tdim >= 0);
  assert(tdim <= this->tdim());
  assert(entity_id >= 0);
  ...
  switch (which_mapping) {
  case 0: {
    assert(this->coarse_entity_to_quad_[tdim].find(entity_id) !=
           this->coarse_entity_to_quad_[tdim].end());
    return this->coarse_entity_to_quad_.at(tdim).at(entity_id);
  }
  case 1: {
    assert(this->entity_to_quad_[tdim].find(entity_id) !=
           this->entity_to_quad_[tdim].end());
    return this->entity_to_quad_.at(tdim).at(entity_id);
  }
  default: {
    assert(this->entity_to_quad_[tdim].find(entity_id) !=
           this->entity_to_quad_[tdim].end());
...
EntityToQuadMap *MeshPXestDatabase::get_entity_to_quad_map(int tdim,
                                                           int which_mapping) {
  assert(tdim >= 0);
  assert(tdim <= this->tdim());
  switch (which_mapping) {
...
void MeshPXestDatabase::set_entity_to_quad_map(int tdim, int which_mapping,
                                               EntityToQuadMap &map) {
  assert(tdim >= 0);
  assert(tdim <= this->tdim());
  switch (which_mapping) {
...
void MeshPXestDatabase::save(std::string filename, const MPI_Comm &comm) const {
  ...
  std::string connectivity_filename =
      filename_without_suffix + "_connectivity.pXest";

  if (tdim == 2) {
    assert(p4est_forest_->connectivity != nullptr);
    p4est_save(forest_filename.c_str(), p4est_forest_, 1);
  }

  if (tdim == 3) {
    assert(p8est_forest_->connectivity != nullptr);
  ...
  for (int i = 0; i < tdim + 1; ++i) {
    ...
    for (std::map< int, QuadCoord >::const_iterator it = m.begin();
         it != m.end(); ++it) {
      keys.push_back(it->first);
      QuadCoord temp_coord = it->second;
      values.push_back(temp_coord.tree);
      values.push_back(static_cast< int32_t >(temp_coord.level));
      values.push_back(temp_coord.x);
      values.push_back(temp_coord.y);
      values.push_back(temp_coord.z);
      values.push_back(static_cast< int32_t >(temp_coord.localId));
      values.push_back(static_cast< int32_t >(temp_coord.tdim));
      values.push_back(static_cast< int32_t >(temp_coord.coarseId));
    }
    ...
    for (std::map< int, QuadCoord >::const_iterator it = m2.begin();
         it != m2.end(); ++it) {
      keys.push_back(it->first);
    ...
    write_array_parallel< int32_t >(group_ptr, coarse_values_name.str(), values,
                                    comm);
  }

  // save p4est_connectivity related stuff
  write_array_parallel< double >(group_ptr, "conn_vertices", conn_vertices_,
                                 comm);
  write_array_parallel< int >(group_ptr, "tree_to_vertices", tree_to_vertices_,
                              comm);
  write_array_parallel< int >(group_ptr, "num_conn_vertices",
                              &num_conn_vertices_, 1, comm);
  write_array_parallel< int >(group_ptr, "num_conn_cells", &num_conn_cells_, 1,
                              comm);
  ...
  int counter = 0;
  for (std::map< Id, int >::const_iterator it = cell_to_mesh_mapping_.begin();
       it != cell_to_mesh_mapping_.end(); ++it) {
    mapping_keys[counter] = it->first;
    mapping_values[counter] = it->second;
    ++counter;
  }

  write_array_parallel< Id >(group_ptr, "cell_to_mesh_mapping_keys",
                             mapping_keys, comm);
  write_array_parallel< int >(group_ptr, "cell_to_mesh_mapping_values",
                              mapping_values, comm);

  // save individual values
  write_array_parallel< int >(group_ptr, "layer_width", &layer_width_, 1, comm);
  int quadrant_arrays_sorted_int = static_cast< int >(quadrant_arrays_sorted_);
  write_array_parallel< int >(group_ptr, "quadrant_arrays_sorted",
                              &quadrant_arrays_sorted_int, 1, comm);
#else
...
void MeshPXestDatabase::load(std::string filename, const MPI_Comm &comm) {
  ...
  std::string forest_filename = filename_without_suffix + "_forest.pXest";
  if (tdim == 2) {
    p4est_forest_ = p4est_load(forest_filename.c_str(), comm,
                               QuadData_size(tdim), 1, nullptr, &p4est_conn_);
  }

  if (tdim == 3) {
    p8est_forest_ = p8est_load(forest_filename.c_str(), comm,
                               QuadData_size(tdim), 1, nullptr, &p8est_conn_);
  }
#endif
  int rank, size;
  MPI_Comm_rank(comm, &rank);
  ...
  for (int i = 0; i < tdim + 1; ++i) {
    ...
    for (int j = 0; j < num_quads; ++j) {
      QuadCoord new_quad(values[8 * j], values[8 * j + 1], values[8 * j + 2],
                         values[8 * j + 3], values[8 * j + 4],
                         values[8 * j + 5], values[8 * j + 6],
                         values[8 * j + 7]);
    ...
    read_array_parallel< int32_t >(group_ptr, coarse_values_name.str(), values,
                                   comm);
    assert(8 * keys.size() == values.size());
  ...
  read_array_parallel< double >(group_ptr, "conn_vertices", conn_vertices_,
                                comm);
  read_array_parallel< int >(group_ptr, "tree_to_vertices", tree_to_vertices_,
                             comm);
  read_array_parallel< int >(group_ptr, "num_conn_vertices", temp_container,
                             comm);
  ...
  read_array_parallel< Id >(group_ptr, "cell_to_mesh_mapping_keys",
                            mapping_keys, comm);
  read_array_parallel< int >(group_ptr, "cell_to_mesh_mapping_values",
                             mapping_values, comm);

  cell_to_mesh_mapping_.clear();
  for (int j = 0; j < mapping_keys.size(); ++j) {
    cell_to_mesh_mapping_.insert(
        std::make_pair(mapping_keys[j], mapping_values[j]));
  }
  ...
  read_array_parallel< int >(group_ptr, "quadrant_arrays_sorted", buffer, dummy,
                             comm);
...
void MeshPXestDatabase::copy_from(const MeshPXestDatabasePtr db) {
...
#ifdef WITH_P4EST
  // boost::shared_ptr<MeshPXestDatabase> new_db_pXest =
  // boost::static_pointer_cast<MeshPXestDatabase> ( new_db );

  // copy connectivity
  if (tdim == 2) {
    assert(db->p4est_conn_ != nullptr);
    ...
    pXest_build_conn(num_conn_vertices, num_conn_cells, conn_vertices,
                     tree_to_vertices, conn);
    this->p4est_conn_ = conn;
  }

  if (tdim == 3) {
    assert(db->p8est_conn_ != nullptr);
  ...
  if (tdim == 2) {
    assert(db->get_p4est_forest() != nullptr);
  ...
  if (tdim == 3) {
    assert(db->get_p8est_forest() != nullptr);
