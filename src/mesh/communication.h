...
struct SharedVertex {
  ...
  bool operator==(const SharedVertex &v) const {
    return sub_domain == v.sub_domain && remote_vertex_id == v.remote_vertex_id;
  }
#if __cplusplus > 199711L
  SharedVertex(const SharedVertex &) = default;
  SharedVertex(SharedVertex &&) = default;
#endif

  SharedVertex &operator=(const SharedVertex &v) {
    if (this == &v) {
      return *this;
    }
    this->sub_domain = v.sub_domain;
    this->remote_vertex_id = v.remote_vertex_id;
    return *this;
  }
};
...
public:
  ...
  typedef UMAP_NAMESPACE::unordered_map< Id, SharedVertexArray > Table;
  typedef UMAP_NAMESPACE::unordered_map< Id, SharedVertexArray >::const_iterator
      const_iterator;
  typedef SortedArray< SubDomainId >::const_iterator NeighborIterator;

  SharedVertexTable() {
    this->shared_verts_.clear();
  ...
  void add_shared_vertex(Id local_vertex, const SharedVertex &shared_vertex);
...
struct EntityPackage {
  ...
  std::vector< AlignNumber > align_numbers;
};

/// \brief Packs all entities of a given dimension in a mesh.
template < typename EntityIteratorType >
void pack_entities(const EntityIteratorType &begin,
                   const EntityIteratorType &end, EntityPackage *entities);
...
/// \brief Identifies which entities in the local mesh are ghost entities in
/// neighboring sub-domains
typedef std::map< SubDomainId, SortedArray< EntityNumber > > GhostEntityMap;
void find_ghost_cells_entities_to_send(
    const Mesh &mesh, const SharedVertexTable &shared_bdy_vertices,
    GhostEntityMap &entities_to_send);

void find_ghost_facets_entities_to_send(
    const Mesh &mesh, const SharedVertexTable &shared_bdy_vertices,
    GhostEntityMap &entities_to_send);
...
///
/// \param mesh  pointer to mesh to broadcast. Must be non-zero on process root
/// only.
...
void pack_entities(const EntityIteratorType &begin,
  ...
  UMAP_NAMESPACE::unordered_map< Id, int > vertex_map;
  vertex_map.clear();
  ...
  for (EntityIteratorType it = begin; it != end; ++it) {
    ...
    for (int v = 0; v < it->num_vertices(); ++v) {
      ...
      if (vertex_map.find(v_id) == vertex_map.end()) {
        ...
        entities->coords.insert(entities->coords.end(), v_coords.begin(),
                                v_coords.end());
    ...
    entities->align_numbers.push_back(it->get_align_number());
  }
  // last offset
  entities->offsets.push_back(entities->connections.size());
}
