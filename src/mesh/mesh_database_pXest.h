...
public:
  ...
  virtual ~MeshPXestDatabase();

  virtual void build(TDim dim, const SortedArray< Id > &cells,
  ...
  /// @param[in] which_mapping flag indicating whether map should be added to
  /// coarse maps (0) or distributed maps(1)
  virtual void add_entity_to_quad_coord(int tdim, Id entity_id,
                                        QuadCoord &coord, int which_mapping);

  virtual void permute_entity_to_quad_map(const std::vector< size_t > &perm,
                                          int tdim, int which_mapping);

  virtual void clear_entity_to_quad_map(int tdim, int which_mapping);

  /// \brief Set complete entity_to_quad_mapping for specified topological
  /// dimension
  /// @param[in] tdim topological dimension
  /// @param[in] which_mapping flag indicating whether map should be see as
  /// coarse maps (0) or distributed maps(1)
  /// @param[in] maps vector of entity_to_quad maps to be set
  virtual void set_entity_to_quad_map(int tdim, int which_mapping,
                                      EntityToQuadMap &maps);

  virtual QuadCoord get_entity_to_quad_coord(int tdim, Id entity_id,
                                             int which_mapping) const;

  virtual EntityToQuadMap *get_entity_to_quad_map(int tdim, int which_mapping);

  /// \brief set data structures used for building p4est connectivity for coarse
  /// mesh
  virtual void set_conn_data(int num_vertices, int num_cells,
  ...
  virtual inline int get_layer_width() { return this->layer_width_; }

  virtual inline void set_layer_width(int width) { this->layer_width_ = width; }
  ...
  virtual Mesh *get_mesh(int history_index);

  virtual std::map< int, Mesh * > get_mesh_history() const {
  ...
  virtual void set_mesh(Mesh *mesh_ptr, int history_index);

  /// \brief add mesh history indices for given cells
  // RESTRUCTURING -> Database
  virtual void add_cell_to_mesh_maps(const std::vector< Id > &cells,
                                     std::vector< int > &history_indices);
  ...
  virtual int get_cell_ref_level(Id cell_id) const {
    return this->get_entity_to_quad_coord(this->tdim(), cell_id, 1).level;
  }
...
protected:
  ...
  // Mapping from hiflow entity to quadrant in forest for nondistributed coarse
  // mesh
  EntityToQuadMapping coarse_entity_to_quad_;

  // RESTRUCTURING -> Database
  std::map< Id, int > cell_to_mesh_mapping_;
  std::map< int, Mesh * > mesh_history_;
