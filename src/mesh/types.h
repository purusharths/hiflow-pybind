...
/// and imported types from boost libraries.
...
/// Type for representing entity Id:s. Id:s are arbitrary, non-negative integer
/// values.
typedef int Id;
...
/// Type for the align number.
typedef int AlignNumber;
...
typedef boost::function< void(const Entity &, std::vector< Coordinate > &) >
    RefinedGeometryFunction;

typedef boost::function< void(const Entity &, const RefinedCell &,
                              std::vector< Coordinate > &) >
    RecursiveRefinedGeometryFunction;
...
#define NOT_YET_IMPLEMENTED                                                    \
  {                                                                            \
    std::clog << "\nError: Function \"" << __FUNCTION__ << "\" in file \""     \
              << __FILE__ << "\" at line \"" << __LINE__                       \
              << "\" isn't implemented yet!" << std::endl;                     \
    assert(0);                                                                 \
  }

template < class T, class I > void range_check(const T &container, I i) {
  assert(i >= 0);
