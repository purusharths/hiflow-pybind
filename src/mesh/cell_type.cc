...
/// \returns reference to the unique CellType object corresponding to (dim,
/// num_vertices)
...
CellType::Tag CellType::tag() const { return tag_; }

/// \returns the topological dimension of the cell type.

TDim CellType::tdim() const { return tdim_; }
...
EntityCount CellType::num_regular_entities(TDim dim) const {
  ...
  // number of sub-entities is length of cell_sub_entities-vector for
  // root-cell
...
/// \param dim   toplogical dimension of the the sub-entities.
/// \pre  0 <= dim <= CellType::tdim() .
/// \returns  the total number of sub-entities of dimension dim for all refined
/// cells.
...
/// \returns  total number of vertices, including both regular and refined
/// vertices.

int CellType::num_vertices() const { return vertices_.size(); }
...
const std::vector< int > &CellType::local_vertices_of_entity(TDim d,
                                                             int i) const {
  assert(d >= 0);
  assert(d <= tdim_);
  assert(i >= 0);
...
/// \pre 0 <= d <= CellType::tdim() .
/// \pre 0 <= i < CellType::num_entities(d) .
/// \pre 0 <= cell_vertices[j] < CellType::num_regular_entities(0) if i <
/// CellType::num_regular_entites(i) . \pre 0 <= cell_vertices[j] <
/// CellType::num_vertices if i >= CellType::num_regular_entites(i) .

std::vector< int >
CellType::vertices_of_entity(TDim d, int i,
...
/// \pre 0 < d < CellType::tdim()
/// \returns     reference to vector containing the numbers of the sub-entities
/// of dimension d of the cell, in the current CellType.

const std::vector< int > &CellType::sub_entities_of_cell(TDim d,
                                                         int cell) const {
...
/// \pre   0 <= i < CellType::num_entities(d) .
///
/// \returns the unique regular parent entity of the entity, or -1 if no parent
/// exists.
...
/// \param gdim     the geometrical dimension of the cell (number of coordinates
/// per point in the coords vector). \pre   coords.size() / gdim =
/// CellType::num_regular_entities(0). \returns  true if the geometry of the
/// cell is correct.

template < typename T >
bool CellType::check_cell_geometry(const std::vector< T > &coords,
                                   GDim gdim) const {
  return true;
}
...
int CellType::add_refinement(const std::vector< int > &sub_cell_numbers) {
  ...
  assert(*std::min_element(sub_cell_numbers.begin(), sub_cell_numbers.end()) >
         0);
...
/// \param entity_number  output parameter for the number of the entity, if it
/// is found. \returns   true if the sought entity was found.
...
bool CellType::is_entity_contained(TDim dim, int entity,
                                   int contained_entity) const {
  ...
  const std::vector< int > &contained_entity_vertices =
      entities(dim)[contained_entity];
  ...
  for (std::vector< int >::const_iterator it =
           contained_entity_vertices.begin();
...
void CellType::create_sub_entities_of_refined_cell(int cell) {
  ...
  // TODO Basically the same algorithm as build() in MeshDatabase -> can we
  // merge?
...
std::ostream &operator<<(std::ostream &os, const CellType &cell_type) {
  os << "CellType: dim = " << cell_type.tdim() << ", tag = " << cell_type.tag()
     << "\n";

  for (int d = 0; d < cell_type.tdim(); ++d) {
    os << "Num entities of dimension " << d << " = "
       << cell_type.num_entities(d) << " of which "
       << cell_type.num_regular_entities(d) << " are regular\n";
  }
  ...
  for (int d = 1; d <= cell_type.tdim(); ++d) {
    for (int i = 0; i < cell_type.num_entities(d); ++i) {
      ...
      os << "\tparents = " << string_from_range(parents.begin(), parents.end())
         << "\n";
      os << "\tregular parent = " << cell_type.regular_parent(d, i) << "\n";
    }
  }

  for (int i = 0; i < cell_type.num_entities(cell_type.tdim()); ++i) {
    for (int d = 1; d < cell_type.tdim(); ++d) {
      const std::vector< int > &sub_entities =
          cell_type.cell_sub_entities(d)[i];
...
bool compare_as_sets(const std::vector< int > &v1,
  ...
  for (std::vector< int >::const_iterator it = v1.begin(); it != v1.end();
       ++it) {
...
template bool CellType::check_cell_geometry(const std::vector< double > &coords,
                                            GDim gdim) const;
template bool CellType::check_cell_geometry(const std::vector< float > &coords,
                                            GDim gdim) const;
} // namespace mesh
} // namespace hiflow
