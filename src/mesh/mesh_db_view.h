...
/// \brief Get material numbers on facets of a cell in a vector. If no facets
/// have any material numbers, the return vector is empty.
std::vector< MaterialNumber >
get_cell_facet_materials(const MeshDatabasePtr &db, const Entity &cell);
std::vector< AlignNumber > get_cell_facet_aligns(const MeshDatabasePtr &db,
    const Entity &cell);

/// \brief Transfer the material numbers from parent facets to  exterior child
/// facets, if they are non-negative.
void inherit_facet_material_numbers(
  const MeshDatabasePtr &db, const CellType &parent_cell_type,
  const std::vector< MaterialNumber > &parent_facet_materials,
  int sub_cell_number, const std::vector< Id > &sub_cell_vertices,
  const RefinementTree *tree);

void inherit_facet_align_numbers(
  const MeshDatabasePtr &db, const CellType &parent_cell_type,
  const std::vector< AlignNumber > &parent_facet_aligns, int sub_cell_number,
  const std::vector< Id > &sub_cell_vertices, const RefinementTree *tree);
...
public:
  ...
  virtual void get_vertex_ids(TDim entity_dim,
                              EntityNumber index,
  ...
  virtual std::vector< Coordinate > get_coordinates(TDim entity_dim,
      EntityNumber index) const;
      
  virtual void
  get_coordinates(TDim entity_dim, EntityNumber index, std::vector< Coordinate >& coord) const;
  ...
  get_children_cell_ids(EntityNumber cell_index) const;
  virtual std::vector< EntityNumber >
  get_sibling_cell_indices(EntityNumber cell_index) const;
  ...
  // if the boundary already has been extracted, it should not be extracted
  // again.
  virtual MeshPtr extract_boundary_mesh() const;

  virtual MeshPtr extract_facet_mesh() const;

  virtual void set_refined_geometry_function(RefinedGeometryFunction f);

  virtual void
  set_recursive_refined_geometry_function(RecursiveRefinedGeometryFunction f);
  ...
  virtual void copy_from(ConstMeshPtr mesh);

  virtual void deep_copy_from(ConstMeshPtr mesh);

  virtual ConstMeshPtr get_mesh(int history_index) const;

  virtual void set_mesh(ConstMeshPtr mesh_ptr, int history_index);

  virtual void set_mesh_history(std::map< int, ConstMeshPtr > &history) {
    this->mesh_history_ = history;
  }
  ...
  void transform_vertices (const std::vector< std::vector< double> >& A, 
...
protected:
  ...
  std::vector< Id >
  compute_refined_cells(const Entity &cell, const RefinementTree *tree,
  ...
  std::map< int, ConstMeshPtr > mesh_history_;
...
private:
  ...
  void convert_connectivity_to_indices(TDim d,
...
public:
  ...
  MeshDbViewBuilder(
    TDim tdim, GDim gdim, int flag,
    std::vector< MasterSlave > period = std::vector< MasterSlave >(0),
    bool is_rectangular = false);

  virtual VertexHandle add_vertex(const std::vector< Coordinate > &coordinates);
  virtual std::vector< VertexHandle >
  add_vertices(const std::vector< Coordinate > &coordinates);

  virtual EntityHandle add_entity(TDim tdim,
                                  const std::vector< VertexHandle > &vertices);
  virtual std::vector< EntityHandle >
  add_entities(TDim tdim, const std::vector< VertexHandle > &vertices,
               const std::vector< int > &sizes);

  virtual void set_material_number(TDim tdim, EntityHandle entity,
                                   MaterialNumber material);

  virtual void set_align_number(TDim tdim, EntityHandle entity,
                                AlignNumber align);
