...
void find_ghost_cells_entities_to_send(
  ...
  for (EntityIterator ent_it = mesh.begin(mesh.tdim());
       ent_it != mesh.end(mesh.tdim()); ++ent_it) {
    const Id ent_index = ent_it->index();
    for (VertexIdIterator v_it = ent_it->begin_vertex_ids(),
                          end_v_it = ent_it->end_vertex_ids();
         v_it != end_v_it; ++v_it) {
      const Id v_id = *v_it;
      for (int s = 0; s < shared_bdy_vertices.num_shared_vertices(v_id); ++s) {
        ...
        if (entities_to_send.find(sub_domain) != entities_to_send.end()) {
          entities_to_send[sub_domain].find_insert(ent_index);
        } else {
          entities_to_send.insert(std::make_pair(
              sub_domain, SortedArray< EntityNumber >(1, ent_index)));
        }
      }
    }
  }
...
void find_ghost_facets_entities_to_send(
  ...
  for (EntityIterator cell_it = mesh.begin(mesh.tdim()),
                      end_cell_it = mesh.end(mesh.tdim());
       cell_it != end_cell_it; ++cell_it) {

    for (VertexIdIterator v_it = cell_it->begin_vertex_ids(),
                          end_v_it = cell_it->end_vertex_ids();
         v_it != end_v_it; ++v_it) {
      ...
      for (int s = 0; s < shared_bdy_vertices.num_shared_vertices(v_id); ++s) {
        ...
        for (IncidentEntityIterator
                 facet_it = cell_it->begin_incident(mesh.tdim() - 1),
                 end_facet_it = cell_it->end_incident(mesh.tdim() - 1);
             facet_it != end_facet_it; ++facet_it) {
