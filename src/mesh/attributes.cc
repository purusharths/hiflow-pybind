...
void AttributeBase::set_all_to_zero() { throw AttributeTypeException(); }

void AttributeBase::set_all_to_one() { throw AttributeTypeException(); }

int AttributeBase::size() { throw AttributeTypeException(); }

//////////////// BoolAttribute ////////////////

BoolAttribute::BoolAttribute() {}

BoolAttribute::BoolAttribute(const std::vector< bool > &data) : data_(data) {}
...
void BoolAttribute::set_all_to_zero() {
  for (std::vector< bool >::iterator it = data_.begin(), e_it = data_.end();
       it != e_it; ++it) {
    *it = 0;
  }
}
...
void BoolAttribute::set_all_to_one() {
  for (std::vector< bool >::iterator it = data_.begin(); it != data_.end();
       ++it) {
    *it = 1;
  }
}
...
int BoolAttribute::size() { return data_.size(); }

//////////////// IntAttribute ////////////////

IntAttribute::IntAttribute() {}

IntAttribute::IntAttribute(const std::vector< int > &data) : data_(data) {}
...
void IntAttribute::set_all_to_zero() {
  for (std::vector< int >::iterator it = data_.begin(), e_it = data_.end();
       it != e_it; ++it) {
    *it = 0;
  }
}
...
int IntAttribute::size() { return data_.size(); }
...
void DoubleAttribute::set_all_to_zero() {
  for (std::vector< double >::iterator it = data_.begin(), e_it = data_.end();
       it != e_it; ++it) {
    *it = 0;
  }
}
...
int DoubleAttribute::size() { return data_.size(); }

//////////////// InheritedAttribute ////////////////

InheritedAttribute::InheritedAttribute() {}

InheritedAttribute::InheritedAttribute(AttributePtr data,
                                       AttributePtr parent_index)
...
int InheritedAttribute::size() { return data_->size(); }
...
void AttributeTable::add_attribute(const std::string &name,
...
std::vector< std::string > AttributeTable::get_attribute_names() const {
  ...
  for (std::map< std::string, AttributePtr >::const_iterator it =
           attributes_.begin();
...
void AttributeTable::get(const std::string &name, int index,
                         double *value) const {
  assert(value != 0);
