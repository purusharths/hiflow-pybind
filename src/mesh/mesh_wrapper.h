...
/// \file mesh_wrapper.h
/// \brief Wrapper class to faciliate individual implementation of abstract
/// base class mesh.
///
/// \author Teresa Beck
///
/// Wrapper class used to faciliate individualized implementations of the
/// abstract base class mesh. A MeshWrapper wraps a MeshPtr by forewarding
/// the respective functions of the mesh behind to its respective mesh
/// implementation. This enables the derivation of new mesh implementations
/// without completely redefineing every single pure virtual mesh function
/// of the base class. Instead, the functions are forewarded to the mesh
/// implementation of the mesh object behind the mesh pointer. Thus,
/// additional functionalities can easily be amended or added in the
...
public:
  ...
  virtual IncidentEntityIterator begin_incident(const Entity &entity,
    ...
    return IncidentEntityIterator(wrap_incident_entity,
                                  it.get_incidence_iterator());
  }
  ...
  virtual MeshPtr extract_facet_mesh() const {
    return mymesh_->extract_facet_mesh();
  }
