...
enum IMPL { IMPL_NONE = -1, IMPL_DBVIEW = 0, IMPL_REFINED = 1, IMPL_P4EST = 2 };
...
public:
  ...
  inline Mesh(TDim tdim, GDim gdim,
  ...
  /// \param tdim  Topological dimension of entities for which attribute is
  /// defined \param attribute A pointer to the attribute
  inline void add_attribute(const std::string &name, TDim tdim,
  ...
  /// \param name  Name of attribute
  /// \param tdim  Topological dimension of entities for which attribute is
  /// defined \return A pointer to the attribute
  ...
  /// \param name  Name of attribute
  /// \param tdim  Topological dimension of entities for which attribute is
  /// defined \param index Index of entity \param[out] value  Pointer where
  /// value should be stored
  template < typename T >
  void get_attribute_value(const std::string &name, TDim tdim,
                           EntityNumber index, T *value) const;

  /// \brief Set the value of an attribute
  ///
  /// \param name  Name of attribute
  /// \param tdim  Topological dimension of entities for which attribute is
  /// defined \param index Index of entity \param value New value of attribute
  /// for the entity
  template < typename T >
  void set_attribute_value(const std::string &name, TDim tdim,
                           EntityNumber index, const T &value) const;

  /// \brief Check whether a given attribute exists in the Mesh
  ///
  /// \param name  Name of attribute
  /// \param tdim  Topological dimension of entities for which attribute is
  /// defined \return true if an attribute with the given name and dimension
  /// exists in the Mesh
  ...
  ///
  /// \param entity_dim  The topological dimension of the entities to iterate
  /// over \return  An iterator to the first entity of dimension entity_dim in
  /// the mesh
  ...
  ///
  /// \param entity_dim  The topological dimension of the entities to iterate
  /// over \return  An iterator to one-past-the-last entity of dimension
  /// entity_dim in the mesh
  virtual EntityIterator end(TDim entity_dim) const = 0;

  /// \brief Obtain iterator to the first entity of dimension entity_dim which
  /// is incident to a given entity.
  virtual IncidentEntityIterator begin_incident(const Entity &entity,
      TDim entity_dim) const = 0;

  /// \brief Obtain iterator to one-past-the-last entity of dimension entity_dim
  /// which is incident to a given entity.
  virtual IncidentEntityIterator end_incident(const Entity &entity,
      TDim entity_dim) const = 0;
  ...
  virtual std::vector< Id > get_vertex_ids(TDim entity_dim,
      EntityNumber index) const = 0;

  /// \brief Access the material number of an entity
  virtual MaterialNumber get_material_number(TDim entity_dim,
      EntityNumber index) const = 0;

  /// \brief Change the material number of an entity
  virtual void set_material_number(TDim entity_dim, EntityNumber index,
                                   MaterialNumber material) = 0;

  /// \brief Access the material number of an entity
  virtual AlignNumber get_align_number(TDim entity_dim,
                                       EntityNumber index) const = 0;

  /// \brief Change the material number of an entity
  virtual void set_align_number(TDim entity_dim, EntityNumber index,
                                AlignNumber align) = 0;

  /// \brief Access the coordinates of an entity.z
  // TODO: replace Coordinate by typename T
  virtual std::vector< Coordinate >
  get_coordinates(TDim entity_dim, EntityNumber index) const = 0;

  virtual void
  get_coordinates(TDim entity_dim, EntityNumber index, std::vector< Coordinate >& coord) const = 0;
  ...
  get_children_cell_ids(EntityNumber cell_index) const = 0;

  virtual std::vector< EntityNumber >
  get_sibling_cell_indices(EntityNumber cell_index) const = 0;
  ...
  virtual EntityCount num_incident_entities(const Entity &entity,
      TDim entity_dim) const = 0;
  ...
  /// \return  An Entity object allowing access to the data associated with the
  /// entity
  virtual Entity get_entity(TDim entity_dim,
                            EntityNumber entity_number) const = 0;

  /// \brief Look up an entity in the mesh by its id
  ///
  /// \param entity_dim  The topological dimension of the desired entity
  /// \param id  The id of the entity
  /// \param entity_number  An optional pointer to a location where the entity
  /// number should be stored if it is found. \return  true if the mesh contains
  /// an entity with the given dimension and id
  virtual bool find_entity(TDim entity_dim, Id id,
                           EntityNumber *entity_number) const = 0;
  ...
  virtual bool find_vertex(const std::vector< Coordinate > &coords,
                           EntityNumber *v_index) const = 0;

  /// \brief Refine the mesh uniformly
  ///
  /// Refines all the cells of the mesh with the default type of refinement
  /// (refinement_type = 0). \return  A shared pointer to a Mesh object
  /// containing the refined cells.
  virtual MeshPtr refine() const = 0;

  /// \brief Refine the mesh with the provided refinement types.
  ///
  /// \param     refinements  refinement type of each cell, -1 for coarsening,
  /// other negative number for no action. \return  A shared pointer to a Mesh
  /// object containing the refined cells.
  ...
  /// \param   attribute_name  Name of attribute containing the desired
  /// refinement type for each cell. \return  A shared pointer to a Mesh object
  /// containing the refined cells.
  ...
  virtual void replace_vertex(const std::vector< Coordinate > &destination,
                              Id vertex_index) = 0;
  ...
  virtual void move_vertex(const std::vector< Coordinate > &displacement,
                           Id vertex_index) = 0;
  ...
  virtual void
  move_vertices(const std::vector< Coordinate > &displacements) = 0;


  /// \brief Tranforms all vertices of the mesh according to: x->Ax+b  
  virtual void transform_vertices (const std::vector< std::vector< double> >& A, 
  ...
  /// \return A new Mesh object of topological dimension tdim()-1 containing the
  /// boundary facets. Ownership of the object is transferred to the caller.
  virtual MeshPtr extract_boundary_mesh() const = 0;

  /// \brief Extracts the facet of the mesh
  ///
  /// Identifies the facets (entities of dimension this->tdim()-1)
  /// that lie on the each cell, and creates a new mesh with these entities.
  /// \return A new Mesh object of topological dimension tdim()-1 containing the
  /// cell facets. Ownership of the object is transferred to the caller.
  virtual MeshPtr extract_facet_mesh() const = 0;
  ...
  virtual void set_recursive_refined_geometry_function(
    RecursiveRefinedGeometryFunction f) = 0;

  /// \brief compute global number of cells in mesh, shared by all processes in
  /// given communicator
  ...
  /// \brief return true if cell is locally ownder, i.e. not contained in ghost
  /// layer
  virtual bool cell_is_local(EntityNumber index) const = 0;

  // get level of refinement of cell with given index
  virtual int get_cell_ref_level(EntityNumber cell_index) const = 0;
  ...
  inline virtual void copy_from(ConstMeshPtr mesh);

  /// \brief copy data from reference mesh, including underlying data
  /// structures, to which the mesh has a pointer
  inline virtual void deep_copy_from(ConstMeshPtr mesh);
  ...
  inline virtual void set_ghost_layer_width(int width);

  inline virtual bool is_uniformly_coarsenable() const;

  inline virtual bool is_rectangular() const;

  inline virtual void set_rectangular(bool flag);
  
  inline virtual int ref_count() const;

  inline IMPL mesh_impl() const 
  {
    return this->mesh_impl_;
  }
...
Mesh::Mesh(TDim tdim, GDim gdim, std::vector< MasterSlave > period,
           bool is_rectangular)
  : ref_count_(0), attributes_(new AttributeTable[tdim + 1]), period_(period),
    ghost_layer_width_(1), is_rectangular_(is_rectangular), tdim_(tdim),
    gdim_(gdim), mesh_impl_(IMPL_NONE) {
  // std::cout << " mesh cons" << ref_count_ << std::endl;
}
...
void Mesh::add_attribute(const std::string &name, TDim tdim,
                         AttributePtr attribute) {
  assert(tdim >= 0);
  assert(tdim <= tdim_);
...
template < typename T >
void Mesh::get_attribute_value(const std::string &name, TDim tdim,
                               EntityNumber index, T *value) const {
  assert(tdim >= 0);
  assert(tdim <= tdim_);
...
template < typename T >
void Mesh::set_attribute_value(const std::string &name, TDim tdim,
                               EntityNumber index, const T &value) const {
  assert(tdim >= 0);
  assert(tdim <= tdim_);
...
void Mesh::set_ghost_layer_width(int width) {
  this->ghost_layer_width_ = width;
}
...
bool Mesh::is_rectangular() const {
  return this->is_rectangular_;
}

void Mesh::set_rectangular(bool flag) {
  this->is_rectangular_ = flag;
}

int Mesh::ref_count() const {
  return this->ref_count_;
}

void Mesh::copy_from(ConstMeshPtr mesh) {
  assert(this->tdim() == mesh->tdim());
  assert(this->gdim() == mesh->gdim());

  this->is_rectangular_ = mesh->is_rectangular();
  ...
  for (int dim = 0; dim < mesh->tdim() + 1; ++dim) {
    std::vector< std::string > names(
      mesh->attributes_[dim].get_attribute_names());
    ...
    for (std::vector< std::string >::iterator it = names.begin();
      ...
      if ((int_attr = dynamic_cast< IntAttribute * >(attr)) != 0) {
        ...
        for (std::vector< double >::iterator it_att = double_att.begin();
             it_att < double_att.end(); ++it_att, ++i) {
          *it_att = double_attr->get_double_value(i);
        }

        AttributePtr new_attribute =
          AttributePtr(new DoubleAttribute(double_att));
        this->add_attribute(*it, dim, new_attribute);
      } else if ((bool_attr = dynamic_cast< BoolAttribute * >(attr)) != 0) {
        std::vector< bool > bool_att(this->num_entities(dim));
        for (std::vector< bool >::iterator it_att = bool_att.begin();
             it_att < bool_att.end(); ++it_att, ++i) {
...
void Mesh::deep_copy_from(ConstMeshPtr mesh) {
  this->copy_from(mesh);
}

// TODO: save rectangular
...
void Mesh::save(std::string filename, const MPI_Comm &comm) const {
  ...
  for (int dim = 0; dim < tdim() + 1; ++dim) {
    ...
    for (std::vector< std::string >::iterator it = names.begin();
      ...
      if ((int_attr = dynamic_cast< IntAttribute * >(attr)) != 0) {
        ...
        for (std::vector< int >::iterator it_att = bool_att.begin();
             it_att < bool_att.end(); ++it_att, ++i) {
          *it_att = static_cast< int >(bool_attr->get_bool_value(i));
        }
  ...
  if (period_.empty()) { // can't write empty vectors
...
void Mesh::load(std::string filename, const MPI_Comm &comm) {
  ...
  for (int dim = 0; dim < tdim() + 1; ++dim) {
    ...
    for (std::vector< std::string >::const_iterator it = names.begin();
      ...
      if (att_type == 'i') {
        ...
        read_array_parallel< double >(group_ptr, attribute_name, double_att,
                                      comm);
        attribute = AttributePtr(new DoubleAttribute(double_att));
      } else if (att_type == 'b') {
        std::vector< int > temp_bool_att(0);
        read_array_parallel< int >(group_ptr, attribute_name, temp_bool_att,
                                   comm);
  ...
  if (conv_period.size() == 1) {
    // case period is empty
