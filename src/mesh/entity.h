...
public:
  ...
  /// \brief Create an iterator to the first incident entity of dimension
  /// incident_dim.
  IncidentEntityIterator begin_incident(TDim incident_dim) const;

  /// \brief Create an iterator to one-past-the-last incident entity of
  /// dimension incident_dim.
  ...
  /// \brief Create an iterator to the id of the first child cell of a cell
  /// entity.
  ChildrenIdIterator begin_children_ids() const;

  /// \brief Create an iterator to the id of one-past-the-last child cell of a
  /// cell entity.
  ...
  template < typename T > 
  void get_coordinates(std::vector< T > &coords) const;
  
  /// \brief Get the coordinates of one vertex of the entity.
  // template<typename T> const std::vector<T> get_coordinates(int
  // vertex_number) const;
  template < typename T >
  void get_coordinates(std::vector< T > &coords, int vertex_number) const;
  
  /// \brief compute midpoint of entity
  template < typename T >
  void get_midpoint(std::vector< T > &coords) const;
  
  /// \brief Get the material number of an entity.
  MaterialNumber get_material_number() const;

  /// \brief Get the align number of an entity.
  AlignNumber get_align_number() const;
...
template < typename T >
void Entity::get_midpoint(std::vector< T > &coords) const {
  assert(is_valid());
  std::vector<T> vert_coords;
  this->get_coordinates(vert_coords);
  
  const int gdim = this->gdim();
  coords.clear();
  coords.resize(gdim,0.);
  const int num_vert = this->num_vertices();
  
  for (int v=0; v<num_vert; ++v)
  {
    for (int d=0; d<gdim; ++d)
    {
      coords[d] += vert_coords[v*gdim+d];
    }
  }
  for (int d=0; d<gdim; ++d)
  {
    coords[d] /= static_cast<T>(num_vert);
  }
}

template < typename T >
void Entity::get_coordinates(std::vector< T > &coords,
                             int vertex_number) const {
  assert(is_valid());
  this->compute_coordinates();
  const GDim d = gdim();

  std::vector< Coordinate >::iterator begin_sub =
      coordinates_.begin() + d * vertex_number;
  std::vector< Coordinate >::iterator end_sub =
      coordinates_.begin() + d * (vertex_number + 1);
