...
MeshPtr partition_and_distribute(const MeshPtr master_mesh,
                                 const int master_rank, const MPI_Comm &comm,
                                 int &uniform_ref_steps, IMPL impl) {
...
#ifdef WITH_P4EST
    MeshPtr local_mesh;
    local_mesh = partition_and_distribute_pXest(master_mesh, master_rank, comm,
                                                uniform_ref_steps);
    return local_mesh;
#else
    LOG_ERROR("Not compiled with p4ests support!");
  ...
  uniform_ref_steps = 0;
  return partition_and_distribute_dbview(master_mesh, master_rank, comm,
                                         &gpartitioner);
}
...
MeshPtr partition_and_distribute_dbview(const MeshPtr master_mesh,
  ...
  if ((gpartitioner->is_collective()) || (my_rank == master_rank)) {
    gpartitioner->partition_graph(mesh_graph, master_rank, 0, 0, comm,
                                  &partitioning);
  }
...
MeshPtr compute_ghost_cells_dbview(const Mesh &local_mesh, const MPI_Comm &comm,
                                   SharedVertexTable &shared_verts) {
  assert(comm != MPI_COMM_NULL);
