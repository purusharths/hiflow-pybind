...
public:
  ...
  inline ConstRefCellPtr<DataType, DIM> ref_cell() const;
  ...
  virtual void grad_N (const Coord &ref_pt, std::vector< Vec<DIM,DataType> > &gradients) const;
  virtual void hessian_N (const Coord &ref_pt, std::vector< Mat<DIM, DIM, DataType> > &hessians) const;
...
template < class DataType, int DIM > 
ConstRefCellPtr<DataType, DIM> FunctionSpace< DataType, DIM >::ref_cell() const 
{
...
template < class DataType, int DIM > 
std::vector< Vec<DIM, DataType> > FunctionSpace< DataType, DIM >::ref_cell_coords() const 
{
  assert(this->ref_cell_);
...
template < class DataType, int DIM > 
void FunctionSpace< DataType, DIM >::grad_N (const Coord &pt, std::vector< Vec<DIM,DataType> > &gradients) const
{
  ...
  for (size_t c = 0; c < DIM; ++c) 
  {
    ...
    {
      gradients[k][c] = gradient_component_[k];
    }
  }
}

template < class DataType, int DIM > 
void FunctionSpace< DataType, DIM >::hessian_N (const Coord &pt, std::vector< Mat<DIM, DIM, DataType> > &hessians) const
{
  ...
  for (size_t c1 = 0; c1 < DIM; ++c1) 
  {
    for (size_t c2 = c1; c2 < DIM; ++c2) 
    {
      ...
      for (size_t k=0; k<weight_size_; ++k)
      {
        hessians[k](c1,c2) = hessian_component_[k];
        if (c1 != c2)
        {
          hessians[k](c2,c1) = hessian_component_[k];
        }
      }
    }
  }
