...
template < class DataType, int DIM >
int FEManager< DataType, DIM >::get_fe_index(RefElement< DataType, DIM > const * ref_fe, int cell_index)  const
{
  assert (cell_index >= 0);
                                       ...
                                       const std::vector< std::vector< std::vector< int > > > &param) 
{
  ...
  RefCellType ref_cell_type = REF_CELL_NOT_SET;
  ...
  this->fe_conformity_.resize(nb_fe_, FE_CONFORM_NONE);
  ...
  for (size_t fe_ind = 0; fe_ind < nb_fe_; ++fe_ind) 
  {
    ...
    for (mesh::EntityIterator it = mesh_->begin(tdim_), e_it = mesh_->end(tdim_); it != e_it; ++it) 
    {
      ...
      if (!is_dg[fe_ind])
      {
        ...
        assert (this->fe_conformity_[fe_ind] == FE_CONFORM_NONE 
      ...
      {
        this->fe_conformity_[fe_ind] = FE_CONFORM_L2;
      }

      if (old_size != new_size)
      {
          LOG_INFO("added FE", "index = " << fe_ind << ", type = " << fe_types[fe_ind] << ", max possible conformity = " << this->fe_inst_.max_fe_conformity(fe_id)); 
      }
...
//    this->cell_transformation_[it->index()].push_back(cell_trafo);
      if (fe_ind == 0)
      {
        ...
        std::shared_ptr<CellTransformation<DataType, DIM> > cell_trafo 
            = this->fe_inst_.create_cell_trafo(fe_id, coord_vtx);
    ...
    RefElement< DataType, DIM > const * example_fe = this->fe_tank_[0][fe_ind];
    
    // ensure that all reference elements live on the same reference cell
    if (ref_cell_type == REF_CELL_NOT_SET)
    {
      assert (example_fe->ref_cell_type() != REF_CELL_NOT_SET);
