...
///
/// \todo rename coord_on_subentity_ to coord_on_entity_ => also coord can be
/// treated

template < class DataType > class FEType {
public:
  ...
  void set_id(int Id) { instance_id_ = Id; }
  ...
  /// Get information if this finite element was initialized (true) or not
  /// (false)
...
protected:
  ...
  /// check whether a point 'point' is located within a subentity (point, line,
  /// face), defined by 'points'
  bool is_point_on_subentity(Coord point,
...
typename FEType< DataType >::FiniteElement
FEType< DataType >::get_my_id() const {
  return my_id_;
}

template < class DataType > int FEType< DataType >::get_global_id() const {
  return instance_id_;
}

template < class DataType >
typename FEType< DataType >::FEAnsatz
FEType< DataType >::get_my_ansatz() const {
  return my_ansatz_;
}

template < class DataType > bool FEType< DataType >::get_init_status() const {
  return init_status_;
}

template < class DataType > int FEType< DataType >::get_fe_deg() const {
  return fe_deg_;
}
