...
CellTransformation< DataType >::CellTransformation(int gdim) : gdim_(gdim) {
  ...
  this->eps_ = 1.e3 * std::numeric_limits< DataType >::epsilon();
}
...
///          G([x_ref, y_ref]) := [x_phy - x(coord_ref) , y_phy - y(coord_ref)
///          ]<br> solve G([x_ref, y_ref]) = 0 via Newton-scheme
...
template < class DataType >
bool CellTransformation< DataType >::inverse_newton_2d(
    DataType x_phy, DataType y_phy, DataType &x_ref, DataType &y_ref,
    DataType x_ref_0, DataType y_ref_0) const {
  // Initialisation
  ...
  while (residual > 10. * std::numeric_limits< DataType >::epsilon()) {
    ...
    while ((iter_armijo <= iter_max_armijo) && (residual_armijo > residual)) {
      ...
      if ((ref_k1[0] >= -tol_eps) && (ref_k1[0] <= tol_eps)) {
        ref_k1[0] = 0.;
      }
      if ((ref_k1[1] >= -tol_eps) && (ref_k1[1] <= tol_eps)) {
        ref_k1[1] = 0.;
      }

      if ((ref_k1[0] - 1. >= -tol_eps) && (ref_k1[0] - 1. <= tol_eps)) {
        ref_k1[0] = 1.;
      }
      if ((ref_k1[1] - 1. >= -tol_eps) && (ref_k1[1] - 1. <= tol_eps)) {
        ref_k1[1] = 1.;
      }
...
template < class DataType >
bool CellTransformation< DataType >::inverse_newton_3d(
    DataType x_phy, DataType y_phy, DataType z_phy, DataType &x_ref,
    DataType &y_ref, DataType &z_ref, DataType x_ref_0, DataType y_ref_0,
    DataType z_ref_0) const {
  // Initialisation
  ...
  while (residual > 10. * std::numeric_limits< DataType >::epsilon()) {
    ...
    while ((iter_armijo <= iter_max_armijo) && (residual_armijo > residual)) {
      ...
      if ((ref_k1[2] >= -tol_eps) && (ref_k1[2] <= tol_eps)) {
        ref_k1[2] = 0.;
      }
      ...
      if ((ref_k1[2] - 1. >= -tol_eps) && (ref_k1[2] - 1. <= tol_eps)) {
        ref_k1[2] = 1.;
      }
...
template < class DataType >
bool CellTransformation< DataType >::contains_physical_point(
    const Coord &coord_phys, Coord *coord_ref) const {
  if (coord_phys.size() != this->gdim_) {
    throw "Incorrect geometrical dimension of coord_phys in "
          "CellTransformation::contains_physical_point!";
  }
  ...
  switch (this->gdim_) {
  case 1: {
    found_ref_point = this->inverse(coord_phys[0], cr[0]);
    LOG_DEBUG(2, "Physical Point " << coord_phys[0] << " has ref coords "
                                   << cr[0] << "? -> " << found_ref_point);
    break;
  }
  case 2: {
    found_ref_point = this->inverse(coord_phys[0], coord_phys[1], cr[0], cr[1]);
  ...
  case 3: {
    found_ref_point = this->inverse(coord_phys[0], coord_phys[1], coord_phys[2],
                                    cr[0], cr[1], cr[2]);
