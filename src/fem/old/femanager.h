...
template < class DataType > class FEManager {
public:
  ...
  /// Information about the used FEType on a given mesh cell for a specific
  /// variable
  ...
  inline CellTransformation< DataType > *
  get_cell_transformation(int cell_index) const;
  ...
  virtual void init_fe_tank(const typename FEType< DataType >::FEAnsatz &ansatz,
...
template < class DataType > int FEManager< DataType >::get_dim() const {
  return dim_;
}

template < class DataType > int FEManager< DataType >::get_nb_var() const {
  return nb_var_;
}

template < class DataType > bool FEManager< DataType >::get_ca(int var) const {
  interminable_assert(var >= 0 && var < nb_var_);
  return ca_[var];
}

template < class DataType >
CellTransformation< DataType > *
FEManager< DataType >::get_cell_transformation(int cell_index) const {
...
template < class DataType >
FEType< DataType > *FEManager< DataType >::get_fe_on_cell(int cell_index,
                                                          int var) const {
