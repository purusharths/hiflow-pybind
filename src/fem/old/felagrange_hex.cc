...
template < class DataType > FELagrangeHex< DataType >::FELagrangeHex() {
...
template < class DataType > void FELagrangeHex< DataType >::init_coord() {
...
///          condition degree < fe_deg_. All possible combinations are
///          multiplied and considered in the sum.

template < class DataType >
void FELagrangeHex< DataType >::N(const Coord &pt,
      ...
      for (int j = 0; j <= this->fe_deg_; ++j) {
        ...
        for (int i = 0; i <= this->fe_deg_; ++i) {
          weight[ijk2ind(i, j, k)] =
              this->lp_.poly(this->fe_deg_, i, pt[0]) * lp_j * lp_k;
        }
      }
    }
  } else
...
template < class DataType >
void FELagrangeHex< DataType >::N_x(const Coord &pt,
                                    std::vector< DataType > &weight) const {
  assert(weight.size() == this->get_nb_dof_on_cell());
  ...
  if (this->fe_deg_ > 0) {
    for (int k = 0; k <= this->fe_deg_; ++k) {
      ...
      for (int j = 0; j <= this->fe_deg_; ++j) {
        ...
        for (int i = 0; i <= this->fe_deg_; ++i) {
          weight[ijk2ind(i, j, k)] =
              this->lp_.poly_x(this->fe_deg_, i, pt[0]) * lp_j * lp_k;
        }
      }
    }
  } else {
...
template < class DataType >
void FELagrangeHex< DataType >::N_y(const Coord &pt,
                                    std::vector< DataType > &weight) const {
  assert(weight.size() == this->get_nb_dof_on_cell());
...
template < class DataType >
void FELagrangeHex< DataType >::N_z(const Coord &pt,
                                    std::vector< DataType > &weight) const {
  assert(weight.size() == this->get_nb_dof_on_cell());
...
template < class DataType >
void FELagrangeHex< DataType >::N_xx(const Coord &pt,
                                     std::vector< DataType > &weight) const {
  assert(weight.size() == this->get_nb_dof_on_cell());
  ...
  if (this->fe_deg_ > 0) {
    for (int k = 0; k <= this->fe_deg_; ++k) {
      ...
      for (int j = 0; j <= this->fe_deg_; ++j) {
        ...
        for (int i = 0; i <= this->fe_deg_; ++i) {
          weight[ijk2ind(i, j, k)] =
              this->lp_.poly_xx(this->fe_deg_, i, pt[0]) * lp_j * lp_k;
        }
      }
    }
  } else {
...
template < class DataType >
void FELagrangeHex< DataType >::N_xy(const Coord &pt,
                                     std::vector< DataType > &weight) const {
  assert(weight.size() == this->get_nb_dof_on_cell());
...
template < class DataType >
void FELagrangeHex< DataType >::N_xz(const Coord &pt,
                                     std::vector< DataType > &weight) const {
  assert(weight.size() == this->get_nb_dof_on_cell());
...
template < class DataType >
void FELagrangeHex< DataType >::N_yy(const Coord &pt,
                                     std::vector< DataType > &weight) const {
  assert(weight.size() == this->get_nb_dof_on_cell());
...
template < class DataType >
void FELagrangeHex< DataType >::N_yz(const Coord &pt,
                                     std::vector< DataType > &weight) const {
  assert(weight.size() == this->get_nb_dof_on_cell());
...
template < class DataType >
void FELagrangeHex< DataType >::N_zz(const Coord &pt,
                                     std::vector< DataType > &weight) const {
  assert(weight.size() == this->get_nb_dof_on_cell());
