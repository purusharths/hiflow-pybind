...
template < class DataType > FELagrangeQuad< DataType >::FELagrangeQuad() {
...
template < class DataType > void FELagrangeQuad< DataType >::init_coord() {
...
///          condition degree < this->fe_deg_. All possible combinations are
///          multiplied and considered in the sum.

template < class DataType >
void FELagrangeQuad< DataType >::N(const Coord &pt,
  ...
  if (this->fe_deg_ > 0) {
    for (int j = 0; j <= this->fe_deg_; ++j) {
      ...
      for (int i = 0; i <= this->fe_deg_; ++i) {
        weight[ij2ind(i, j)] = this->lp_.poly(this->fe_deg_, i, pt[0]) * lp_j;
      }
    }
  } else {
...
template < class DataType >
void FELagrangeQuad< DataType >::N_x(const Coord &pt,
                                     std::vector< DataType > &weight) const {
  assert(weight.size() == this->get_nb_dof_on_cell());
  ...
  if (this->fe_deg_ > 0) {
    for (int j = 0; j <= this->fe_deg_; ++j) {
      const DataType lp_j = this->lp_.poly(this->fe_deg_, j, pt[1]);
      for (int i = 0; i <= this->fe_deg_; ++i) {
        weight[ij2ind(i, j)] = this->lp_.poly_x(this->fe_deg_, i, pt[0]) * lp_j;
      }
    }
  } else {
...
template < class DataType >
void FELagrangeQuad< DataType >::N_y(const Coord &pt,
                                     std::vector< DataType > &weight) const {
  assert(weight.size() == this->get_nb_dof_on_cell());
...
template < class DataType >
void FELagrangeQuad< DataType >::N_z(const Coord &pt,
                                     std::vector< DataType > &weight) const {
...
template < class DataType >
void FELagrangeQuad< DataType >::N_xx(const Coord &pt,
                                      std::vector< DataType > &weight) const {
  assert(weight.size() == this->get_nb_dof_on_cell());
  ...
  if (this->fe_deg_ > 0) {
    for (int j = 0; j <= this->fe_deg_; ++j) {
      ...
      for (int i = 0; i <= this->fe_deg_; ++i) {
        weight[ij2ind(i, j)] =
            this->lp_.poly_xx(this->fe_deg_, i, pt[0]) * lp_j;
      }
    }
  } else {
    weight[ij2ind(0, 0)] = 0.0;
...
template < class DataType >
void FELagrangeQuad< DataType >::N_xy(const Coord &pt,
                                      std::vector< DataType > &weight) const {
  assert(weight.size() == this->get_nb_dof_on_cell());
...
template < class DataType >
void FELagrangeQuad< DataType >::N_xz(const Coord &pt,
                                      std::vector< DataType > &weight) const {
  weight.assign(weight.size(), 0.);
}
...
template < class DataType >
void FELagrangeQuad< DataType >::N_yy(const Coord &pt,
                                      std::vector< DataType > &weight) const {
  assert(weight.size() == this->get_nb_dof_on_cell());
...
template < class DataType >
void FELagrangeQuad< DataType >::N_yz(const Coord &pt,
                                      std::vector< DataType > &weight) const {
  weight.assign(weight.size(), 0.);
}
...
template < class DataType >
void FELagrangeQuad< DataType >::N_zz(const Coord &pt,
                                      std::vector< DataType > &weight) const {
  weight.assign(weight.size(), 0.);
}
