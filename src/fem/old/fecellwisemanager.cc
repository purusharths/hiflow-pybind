...
FECellwiseManager< DataType >::FECellwiseManager(int dim, int nb_var)
    : FEManager< DataType, DIM >(dim, nb_var) {}

template < class DataType >
void FECellwiseManager< DataType >::reinit_fe_tank(
    const std::vector< std::vector< int > > &fe_data) {
  assert(0);
  ...
  //  {
  //    for (MeshEntityIterator it = mesh_->begin(dim_); it != mesh_->end(dim_);
  //    ++it)
  //    {
  //      // 1st: Get Cell Type (Tet, Hex, Quad ...)
  //      CellType::Type cell_type = mesh::get_cell_type(dim_,
  //      it->num_vertices())->cell_type();
  ...
  //                if(fe_data[it->index() + var*nb_awake_cells][0] ==
  //                FEType::LAGRANGE)
  //                  fe_tank_.at(it->index() + var*nb_awake_cells) =
  //                    fe_inst_.get_lagrange_tri_element(fe_data[it->index() +
  //                    var*nb_awake_cells][1]);
  //                else
  //                  assert(0);
  //                break; }
  //
  ...
  //                FEType::LAGRANGE)
  //                  fe_tank_.at(it->index() + var*nb_awake_cells) =
  //                    fe_inst_.get_lagrange_quad_element(fe_data[it->index() +
  //                    var*nb_awake_cells][1]);
  //                else
  //                  assert(0);
  //                break; }
  ...
  //                FEType::LAGRANGE)
  //                  fe_tank_.at(it->index() + var*nb_awake_cells) =
  //                    fe_inst_.get_lagrange_tet_element(fe_data[it->index() +
  //                    var*nb_awake_cells][1]);
  //                else
  //                  assert(0);
  //                break; }
  ...
  //                {
  //                  fe_tank_.at(it->index() + var*nb_awake_cells) =
  //                    fe_inst_.get_lagrange_hex_element(fe_data[it->index() +
  //                    var*nb_awake_cells][1]);
  //                }
  //                else
  //                  assert(0);
...
template < class DataType >
void FECellwiseManager< DataType >::init_fe_tank(
    int var, const typename FEType< DataType, DIM >::FEAnsatz &ansatz,
  ...
  for (mesh::EntityIterator it = this->mesh_->begin(this->dim_),
                            e_it = this->mesh_->end(this->dim_);
       it != e_it; ++it) {
    // 1st: Get Cell Type (Tet, Hex, Quad ...)
  ...
  if (tri_representer != -1) {
    ...
    for (size_t i = 0; i < 3; ++i) {
      ltt.inverse(coordinates[2 * i], coordinates[2 * i + 1], ref_coords[i][0],
                  ref_coords[i][1]);
    }
  ...
  if (quad_representer != -1) {
    ...
    if (!this->mesh_->is_rectangular()) {
      ...
      for (size_t i = 0; i < 4; ++i) {
        blqt.inverse(coordinates[2 * i], coordinates[2 * i + 1],
  ...
  if (tet_representer != -1) {
    ...
    for (size_t i = 0; i < 4; ++i) {
      ltt.inverse(coordinates[3 * i], coordinates[3 * i + 1],
                  coordinates[3 * i + 2], ref_coords[i][0], ref_coords[i][1],
                  ref_coords[i][2]);
    }
  ...
  if (hex_representer != -1) {
    ...
    if (!this->mesh_->is_rectangular()) {
      ...
      for (size_t i = 0; i < 8; ++i) {
        tlht.inverse(coordinates[3 * i], coordinates[3 * i + 1],
                     coordinates[3 * i + 2], ref_coords[i][0], ref_coords[i][1],
                     ref_coords[i][2]);
      }
    } else {
  ...
  if (pyr_representer != -1) {
    ...
    for (size_t i = 0; i < 5; ++i) {
      lpr.inverse(coordinates[3 * i], coordinates[3 * i + 1],
                  coordinates[3 * i + 2], ref_coords[i][0], ref_coords[i][1],
                  ref_coords[i][2]);
    }
