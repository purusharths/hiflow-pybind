...
public:
  ...
  /// Default Constructor
  FETransformation(RefElement<DataType, DIM> const * ref_fe)
  : ref_fe_(ref_fe), index_func_set_(false)
  {} 
  ...
  virtual void map_shape_function_gradients (DataType const * detJ, 
                                             ...
                                             const std::vector< DataType> & mapped_vals,
                                             std::vector< Vec<DIM, DataType> > & mapped_grads) const = 0; 
  ...
  virtual void map_shape_function_hessians (Mat<DIM, DIM, DataType> const * JinvT,
                                            std::vector< Mat<DIM, DIM, DataType> > const * H,
                                            size_t func_offset, size_t num_func, size_t nb_comp,
                                            IndexFunction ind_func,
                                            const std::vector< Vec<DIM, DataType> > & shape_grads,
                                            ...
                                            std::vector< Mat<DIM, DIM, DataType> > & mapped_hessians) const = 0;
...
protected:
  ...
  // we need ref_fe_ for iv2ind mapping only
  RefElement<DataType, DIM> const * ref_fe_;
};
...
public:
  ...
  /// Default Constructor
  FETransformationStandard(RefElement<DataType, DIM> const * ref_fe)
  : FETransformation<DataType, DIM>(ref_fe)
  {} 
  ...
  void map_shape_function_hessians (Mat<DIM, DIM, DataType> const * JinvT,
                                    std::vector< Mat<DIM, DIM, DataType> > const * H,
                                    size_t func_offset, size_t num_func, size_t nb_comp,
                                    IndexFunction ind_func,
                                    const std::vector< Vec<DIM, DataType> > & shape_grads,
...
public:
  ...
  /// Default Constructor
  FETransformationContraPiola(RefElement<DataType, DIM> const * ref_fe)
  : FETransformation<DataType, DIM>(ref_fe)
  {} 
...
public:
  ...
  /// Default Constructor
  FETransformationCoPiola(RefElement<DataType, DIM> const * ref_fe)
  : FETransformation<DataType, DIM>(ref_fe)
  {} 
