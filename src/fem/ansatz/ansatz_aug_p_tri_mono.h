...
class AugPTriMono : public virtual AnsatzSpace< DataType, DIM > 
{
public:
  typedef Vec<DIM, DataType> Coord;

  /// Default Constructor
  AugPTriMono(ConstRefCellPtr<DataType, DIM> ref_cell);
...
template < class DataType, int DIM > 
AugPTriMono< DataType, DIM >::AugPTriMono(ConstRefCellPtr<DataType, DIM> ref_cell)
  : AnsatzSpace<DataType, DIM>(ref_cell)
{
  this->tdim_ = 2;
  this->name_ = "Aug_P_Tri_Mono";
  this->type_ = ANSATZ_P_AUG;
