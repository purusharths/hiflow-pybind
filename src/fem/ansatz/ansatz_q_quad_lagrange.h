...
class QQuadLag : public virtual AnsatzSpace< DataType, DIM > 
{
public:
  typedef Vec<DIM, DataType> Coord;

  /// Default Constructor
  QQuadLag (ConstRefCellPtr<DataType, DIM> ref_cell);
...
template < class DataType, int DIM > 
QQuadLag< DataType, DIM >::QQuadLag(ConstRefCellPtr<DataType, DIM> ref_cell)
  : AnsatzSpace<DataType, DIM>(ref_cell)
{
  ...
  assert (this->ref_cell_->type() == REF_CELL_QUAD_STD);
}
