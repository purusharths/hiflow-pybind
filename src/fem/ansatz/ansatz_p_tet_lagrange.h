...
class PTetLag : public virtual AnsatzSpace< DataType, DIM > 
{
public:
  typedef Vec<DIM, DataType> Coord;

  /// Default Constructor
  PTetLag(ConstRefCellPtr<DataType, DIM> ref_cell);
...
template < class DataType, int DIM > 
PTetLag< DataType, DIM >::PTetLag(ConstRefCellPtr<DataType, DIM> ref_cell)
  : AnsatzSpace<DataType, DIM>(ref_cell)
{
  ...
  assert (this->ref_cell_->type() == REF_CELL_TET_STD);
}
