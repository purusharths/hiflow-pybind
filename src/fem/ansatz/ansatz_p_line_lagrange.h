...
class PLineLag : public virtual AnsatzSpace< DataType, DIM > 
{
public:
  typedef Vec<DIM, DataType> Coord;

  /// Default Constructor
  PLineLag(ConstRefCellPtr<DataType, DIM> ref_cell);
...
protected:
  ...
  size_t ij2ind(size_t i, size_t j, size_t comp) const;
...
template < class DataType, int DIM > 
PLineLag<DataType, DIM>::PLineLag (ConstRefCellPtr<DataType, DIM> ref_cell)
 : AnsatzSpace<DataType, DIM>(ref_cell)
{
  this->tdim_ = 1;
  this->name_ = "P_Line_Lagrange";
  this->type_ = ANSATZ_P_LAGRANGE;

  assert (this->ref_cell_->type() == REF_CELL_LINE_STD);
}
