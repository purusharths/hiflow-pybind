...
class SkewAugPTriMono : public virtual AnsatzSpace< DataType, DIM > 
{
public:
  typedef Vec<DIM, DataType> Coord;

  /// Default Constructor
  SkewAugPTriMono(ConstRefCellPtr<DataType, DIM> ref_cell);
...
template < class DataType, int DIM > 
SkewAugPTriMono< DataType, DIM >::SkewAugPTriMono(ConstRefCellPtr<DataType, DIM> ref_cell)
  : AnsatzSpace<DataType, DIM>(ref_cell)
{
  this->tdim_ = 2;
  this->name_ = "Skew_Aug_P_Tri_Mono";
  this->type_ = ANSATZ_SKEW_P_AUG;
