...
class PTriLag : public virtual AnsatzSpace< DataType, DIM > 
{
public:
  typedef Vec<DIM, DataType> Coord;

  /// Default Constructor
  PTriLag(ConstRefCellPtr<DataType, DIM> ref_cell);
...
template < class DataType, int DIM > 
PTriLag< DataType, DIM >::PTriLag(ConstRefCellPtr<DataType, DIM> ref_cell)
  : AnsatzSpace<DataType, DIM>(ref_cell)
{
  ...
  assert (this->ref_cell_->type() == REF_CELL_TRI_STD);
}
