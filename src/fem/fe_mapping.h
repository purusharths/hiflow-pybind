...
public:
  ...
  MappingPhys2Ref ( Functor const * func,
                    ...
                    ConstCellTrafoPtr<DataType, DIM> cell_trafo)
...
private:
  ...
  ConstCellTrafoPtr<DataType, DIM> cell_trafo_;
...
public:
  ...
  void evaluate (const mesh::Entity& entity, const Vec<DIM, DataType>& pt, std::vector<DataType>& vals) const
  {
    assert (this->func_ != nullptr);
    assert (this->cell_trafo_ != nullptr);
    assert (this->fe_trafo_ != nullptr);
