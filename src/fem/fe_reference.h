...
class RefElement : virtual public FunctionSpace<DataType, DIM>
{
public:  
  ...
  inline DofContainer<DataType, DIM> const * dof_container() const;

  inline AnsatzSpace<DataType, DIM> const * ansatz_space () const;

  inline FETransformation<DataType, DIM> const * fe_trafo() const;
  ...
  inline size_t iv2ind(size_t i, size_t var) const
  {
    ...
    return this->space_->iv2ind(i, var);
  }
...
template < class DataType, int DIM > 
inline DofContainer<DataType, DIM> const * RefElement< DataType, DIM >::dof_container() const
{
  return this->dofs_;
}

template < class DataType, int DIM > 
inline AnsatzSpace<DataType, DIM> const * RefElement< DataType, DIM >::ansatz_space () const
{
  return this->space_;
}

template < class DataType, int DIM >
inline FETransformation<DataType, DIM> const * RefElement< DataType, DIM >::fe_trafo() const 
{
