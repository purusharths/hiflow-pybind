...
class LinearPyramidTransformation : public CellTransformation< DataType, DIM > {
public:
  typedef Vec<DIM, DataType> Coord;

  explicit LinearPyramidTransformation(ConstRefCellPtr<DataType, DIM> ref_cell);
...
template < class DataType, int DIM >
LinearPyramidTransformation< DataType, DIM >::LinearPyramidTransformation(ConstRefCellPtr<DataType, DIM> ref_cell)
    : CellTransformation< DataType, DIM >(ref_cell) 
{
  this->order_ = 1;  
  this->fixed_ref_cell_type_ = REF_CELL_PYR_STD;
  this->name_ = "Pyramid";
}

template < class DataType, int DIM >
bool LinearPyramidTransformation< DataType, DIM >::inverse(Coord co_phy, Coord &co_ref ) const 
{
  assert (DIM == 3);
