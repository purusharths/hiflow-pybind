...
public:
  ...
  inline std::vector< RefElement< DataType, DIM > const *> get_fe(int cell_index) const;
  inline RefElement< DataType, DIM > const * get_fe(int cell_index, size_t fe_ind) const; 
  inline RefElement< DataType, DIM > const * get_fe_for_var(int cell_index, size_t var) const; 

  int get_fe_index (RefElement< DataType, DIM > const * ref_fe, int cell_index) const;
  ...
  std::shared_ptr<CellTransformation< DataType, DIM > > get_cell_transformation(int cell_index) const;
...
protected:
  ...
  std::vector< std::vector<RefElement< DataType, DIM > const * > > fe_tank_;
  ...
  std::vector<std::shared_ptr<CellTransformation< DataType, DIM > > >  cell_transformation_;
...
template < class DataType, int DIM >
std::shared_ptr< CellTransformation< DataType, DIM > > FEManager< DataType, DIM >::get_cell_transformation(int cell_index) const 
{
...
/*
template < class DataType, int DIM >
inline CellTransformation< DataType, DIM > * FEManager< DataType, DIM >::get_cell_transformation_for_var(int cell_index) const 
{
  assert(cell_index >= 0 && cell_index < cell_transformation_.size());
  return cell_transformation_[cell_index];
}
...
inline std::vector< RefElement< DataType, DIM > const *> FEManager< DataType, DIM >::get_fe(int cell_index) const 
{
  ...
  std::vector< RefElement< DataType, DIM > const * > tank (this->fe_tank_[cell_index].size());
...
{
  if (cell_index >= this->fe_tank_.size()) std::cout << cell_index << " " << this->fe_tank_.size() << std::endl;
  assert (cell_index >= 0);
  assert (cell_index < this->fe_tank_.size());
...
template < class DataType, int DIM >
inline RefElement< DataType, DIM > const * FEManager< DataType, DIM >::get_fe_for_var(int cell_index, size_t var) const 
{
