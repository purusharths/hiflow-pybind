...
public:
  ...
  RefElement<DataType, DIM> const * get_fe (size_t fe_id) const ;
  ...
  //CellTrafoPtr<DataType, DIM> create_cell_trafo (size_t fe_id, int align_number) const;
  CellTrafoPtr<DataType, DIM> create_cell_trafo (size_t fe_id, std::vector<DataType> coord_vtx) const;
...
private:
  std::vector< RefElement<DataType, DIM> const * > ref_elements_;
  std::vector< AnsatzSpace<DataType, DIM> const * > ansatz_spaces_;
  std::vector< DofContainer<DataType, DIM> const * > dof_containers_;
  std::vector< FETransformation<DataType, DIM> const * > fe_trafos_;
  std::vector< ConstRefCellPtr<DataType, DIM> > ref_cells_;
