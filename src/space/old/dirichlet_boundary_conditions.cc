...
void find_dofs_and_coords_on_face(
  mesh::Id boundary_id, const VectorSpace< DataType > &space, int var,
...
template < class DataType >
void find_dofs_and_coords_on_face(
  const mesh::EntityIterator &boundary_face,
  const VectorSpace< DataType > &space, int var,
  std::vector< doffem::DofID > &dof_ids,
  std::vector< std::vector< DataType > > &dof_points) {
  find_dofs_and_coords_on_face(boundary_face->id(), space, var, dof_ids,
                               dof_points);
}

template < class DataType >
void find_dofs_and_coords_on_face(
  const mesh::IncidentEntityIterator &boundary_face,
  const VectorSpace< DataType > &space, int var,
  std::vector< doffem::DofID > &dof_ids,
  std::vector< std::vector< DataType > > &dof_points) {
  find_dofs_and_coords_on_face(boundary_face->id(), space, var, dof_ids,
...
template void find_dofs_and_coords_on_face< double >(
  mesh::Id boundary_id, const VectorSpace< double > &space, int var,
  std::vector< doffem::DofID > &dof_ids,
  std::vector< std::vector< double > > &dof_points);
template void find_dofs_and_coords_on_face< float >(
  mesh::Id boundary_id, const VectorSpace< float > &space, int var,
  std::vector< doffem::DofID > &dof_ids,
