...
void PeriodicBoundaryConditionsCartesian< DataType >::compute_conditions(
    VectorSpace< DataType > &space) {
  ...
  for (mesh::EntityIterator it_boundary = boundary_mesh->begin(tdim - 1);
    ...
    if (subdomain == rank) {
      ...
      typename PeriodicBoundaryConditionsCartesian< DataType >::MaterialNumber
          mat_num = face.get_material_number();
      ...
      for (int var = 0; var < space.get_nb_var(); ++var) {
        ...
        find_dofs_and_coords_on_face< DataType >(it_boundary, space, var,
                                                 dofs_on_face, coords_on_face);
        ...
        for (int i = 0; i < dofs_on_face.size(); ++i) {
          if (is_periodic_boundary(mat_num)) {
            ...
            if (new_entry) {
              x_coord_.insert(
                  std::make_pair(dofs_on_face[i], coords_on_face[i][0]));
              y_coord_.insert(
                  std::make_pair(dofs_on_face[i], coords_on_face[i][1]));
              if (gdim == 3) {
                z_coord_.insert(
                    std::make_pair(dofs_on_face[i], coords_on_face[i][2]));
              }
            }
          } // if (is_periodic_boundary(mat_num))
        }   // for (int i=0; i<dofs_on_face.size(); ++i)
  ...
  typename std::map<
      typename PeriodicBoundaryConditionsCartesian< DataType >::MaterialNumber,
      typename PeriodicBoundaryConditionsCartesian<
          DataType >::MaterialNumber >::const_iterator mat_tuple_it =
      this->boundary_descriptor_.begin();
  while (mat_tuple_it != this->boundary_descriptor_.end()) {

    typename PeriodicBoundaryConditionsCartesian< DataType >::MaterialNumber
        mat_a = mat_tuple_it->first;
    typename PeriodicBoundaryConditionsCartesian< DataType >::MaterialNumber
        mat_b = mat_tuple_it->second;
    ...
    if (mat_a < mat_b) {
      ...
      // for (int var=0; var<space.get_nb_var()-1; ++var) //no pressure DOFs for
      // Navier-Stokes
      for (int var = 0; var < space.get_nb_var(); ++var) {

        if (verbatim > 0) {
          std::cout << std::endl;
          std::cout << "MAT_A " << mat_a << "\t MAT_B " << mat_b << "\t var "
                    << var << std::endl;
        }
        ...
        typename std::map<
            std::pair< typename PeriodicBoundaryConditionsCartesian<
                           DataType >::MaterialNumber,
                       int >,
        ...
        if (z_coord_.size() == 0) {
          ...
          for (int i = 1; i < dofs_a.size(); ++i) {
            if ((x_coord_[dofs_a[i]] != x_coord_[dofs_a[0]]) && (!x_changing)) {
              x_changing = true;
              ++number_changings;
            }
            if ((y_coord_[dofs_a[i]] != y_coord_[dofs_a[0]]) && (!y_changing)) {
              y_changing = true;
              ++number_changings;
            }
            if ((z_coord_[dofs_a[i]] != z_coord_[dofs_a[0]]) && (!z_changing)) {
              z_changing = true;
              ++number_changings;
            }
        ...
        for (size_t i = 0; i < dofs_a.size(); ++i) {
          ...
          if (z_coord_.size() == 0) {
            ...
            for (size_t j = 0; j < dofs_b.size(); ++j) {
          ...
          this->corresponding_dof_.insert(
              std::make_pair(dofs_a[i], dof_vector));

          if (verbatim > 0) {
            std::cout << dofs_a[i] << ": \t x = " << x_coord_[dofs_a[i]]
                      << "\t"
                         "y = "
                      << y_coord_[dofs_a[i]] << "\t" << dof_vector.at(0)
                      << ": \t x = " << x_coord_[dof_vector.at(0)]
                      << "\t"
                         "y = "
                      << y_coord_[dof_vector.at(0)] << std::endl;
          } // if (verbatim > 0)
