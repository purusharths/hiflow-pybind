...
public:
  ...
  void evaluate (const mesh::Entity& face, const Vec<DIM, DataType>& pt, std::vector<DataType>& vals) const
  {
                                        ...
                                        std::vector< DataType > &dirichlet_values) 
{
       ...
       it_boundary != boundary_mesh->end(tdim - 1); ++it_boundary) 
  {
    ...
    doffem::RefElement<DataType, DIM> const * ref_fe = space.fe_manager().get_fe(cell->index(), fe_ind);
    
    // Get dof container
    doffem::DofContainer<DataType, DIM> const * dofs = ref_fe->dof_container();
