...
template < class DataType > class Element;
...
template < class DataType > class GlobalAssembler {
public:
  ...
  typedef boost::function3< void, const Element< DataType > &,
  ...
  /// boundary.
  typedef boost::function4< void, const Element< DataType > &, int,
                            const Quadrature< DataType > &, LocalVector & >
  ...
  typedef boost::function2< void, const Element< DataType > &,
                            Quadrature< DataType > & >
  ...
  ///
  void
  assemble_multiple_scalar(const VectorSpace< DataType > &space,
                           MultipleScalarAssemblyFunction local_asm,
                           int num_scalars,
  ...
  ///
  /// \param[in] space           the VectorSpace for which the assembly is
...
private:
  ...
  /// functions.
  virtual void compute_sparsity_structure_impl(
      const VectorSpace< DataType > &space, SparsityStructure &sparsity,
      std::vector< std::vector< bool > > *coupling_vars) const = 0;

  virtual void
  assemble_scalar_impl(const VectorSpace< DataType > &space,
                       ScalarAssemblyFunction local_asm,
                       std::vector< DataType > &vec,
                       QuadratureSelectionFunction q_select) const = 0;

  virtual void
  assemble_multiple_scalar_impl(const VectorSpace< DataType > &space,
                                MultipleScalarAssemblyFunction local_asm,
                                int num_scalars,
  ...
  virtual void
  assemble_vector_impl(const VectorSpace< DataType > &space,
                       VectorAssemblyFunction local_asm, GlobalVector &vec,
                       QuadratureSelectionFunction q_select) const = 0;

  virtual void
  assemble_matrix_impl(const VectorSpace< DataType > &space,
                       MatrixAssemblyFunction local_asm, GlobalMatrix &mat,
                       QuadratureSelectionFunction q_select) const = 0;

  virtual void assemble_scalar_boundary_impl(
      const VectorSpace< DataType > &space,
      BoundaryScalarAssemblyFunction local_asm, std::vector< DataType > &vec,
      FacetQuadratureSelectionFunction fq_select) const = 0;

  virtual void assemble_vector_boundary_impl(
      const VectorSpace< DataType > &space,
      BoundaryVectorAssemblyFunction local_asm, GlobalVector &vec,
      FacetQuadratureSelectionFunction fq_select) const = 0;

  virtual void assemble_matrix_boundary_impl(
      const VectorSpace< DataType > &space,
      BoundaryMatrixAssemblyFunction local_asm, GlobalMatrix &mat,
      FacetQuadratureSelectionFunction fq_select) const = 0;
...
class StandardGlobalAssembler : public GlobalAssembler< DataType > {
  ...
  virtual void assemble_scalar_impl(
      const VectorSpace< DataType > &space,
      typename GlobalAssembler< DataType >::ScalarAssemblyFunction local_asm,
      std::vector< DataType > &vec,
      typename GlobalAssembler< DataType >::QuadratureSelectionFunction
          q_select) const;

  virtual void assemble_multiple_scalar_impl(
      const VectorSpace< DataType > &space,
      typename GlobalAssembler< DataType >::MultipleScalarAssemblyFunction
          local_asm,
      int num_scalars, std::vector< std::vector< DataType > > &vec,
      typename GlobalAssembler< DataType >::QuadratureSelectionFunction
          q_select) const;
  ...
  virtual void assemble_scalar_boundary_impl(
      const VectorSpace< DataType > &space,
      typename GlobalAssembler< DataType >::BoundaryScalarAssemblyFunction
          local_asm,
      std::vector< DataType > &vec,
      typename GlobalAssembler< DataType >::FacetQuadratureSelectionFunction
          fq_select) const;

  virtual void assemble_vector_boundary_impl(
      const VectorSpace< DataType > &space,
      typename GlobalAssembler< DataType >::BoundaryVectorAssemblyFunction
          local_asm,
      typename GlobalAssembler< DataType >::GlobalVector &vec,
      typename GlobalAssembler< DataType >::FacetQuadratureSelectionFunction
          fq_select) const;
  ...
  virtual void assemble_matrix_boundary_impl(
      const VectorSpace< DataType > &space,
      typename GlobalAssembler< DataType >::BoundaryMatrixAssemblyFunction
          local_asm,
      typename GlobalAssembler< DataType >::GlobalMatrix &mat,
      typename GlobalAssembler< DataType >::FacetQuadratureSelectionFunction
          fq_select) const;
...
void interpolate_constrained_vector(
    const VectorSpace< typename LAD::DataType > &space,
    typename LAD::VectorType &vector);
