...
class StandardScalarAssembly
    : public AssemblyAlgorithmBase<
          AlgorithmType, StandardScalarAssembly< AlgorithmType, DataType >,
          DataType > {
public:
  ...
  StandardScalarAssembly(const VectorSpace< DataType > &space,
  ...
  const Element< DataType > &next() {
    ...
    this->elem_ =
        Element< DataType >(this->space_, this->traversal_[this->curr_]);
  ...
  void add(const Element< DataType > &element,
...
class StandardMultipleScalarAssembly
    : public AssemblyAlgorithmBase<
          AlgorithmType,
          StandardMultipleScalarAssembly< AlgorithmType, DataType >,
          DataType > {
public:
  typedef std::vector< DataType > LocalObjectType;
  typedef hiflow::Quadrature< DataType > QuadratureType;
  typedef AssemblyAlgorithmBase<
      AlgorithmType, StandardMultipleScalarAssembly< AlgorithmType, DataType >,
      DataType >
      Base;
  ...
  StandardMultipleScalarAssembly(const VectorSpace< DataType > &space,
...
class StandardVectorAssembly
    : public AssemblyAlgorithmBase<
          AlgorithmType, StandardVectorAssembly< AlgorithmType, DataType >,
          DataType > {
public:
  typedef std::vector< DataType > LocalObjectType;
  typedef hiflow::Quadrature< DataType > QuadratureType;
...
class StandardMatrixAssembly
    : public AssemblyAlgorithmBase<
          AlgorithmType, StandardMatrixAssembly< AlgorithmType, DataType >,
          DataType > {
public:
    ...
    if (!row_indices.empty() && num_dofs > 0) {
      ...
      for (size_t i = 0, i_e = row_indices.size(); i != i_e; ++i) {
        ...
        for (size_t j = 0, j_e = num_dofs; j != j_e; ++j) {
          local_mat_sorted_reduced(i, j) =
              local_mat(row_ind, dofs_sort_permutation[j]);
        }
      }
...
class StandardBoundaryScalarAssembly
    : public AssemblyAlgorithmBase<
          AlgorithmType,
          StandardBoundaryScalarAssembly< AlgorithmType, DataType >,
          DataType > {
public:
  typedef std::vector< DataType > LocalObjectType;
  typedef hiflow::Quadrature< DataType > QuadratureType;
  typedef AssemblyAlgorithmBase<
      AlgorithmType, StandardBoundaryScalarAssembly< AlgorithmType, DataType >,
      DataType >
      Base;
  ...
  StandardBoundaryScalarAssembly(const VectorSpace< DataType > &space,
                                 std::vector< DataType > &values)
      : Base(space), values_(values) {
...
void StandardGlobalAssembler< DataType >::compute_sparsity_structure_impl(
  ...
  InitStructure(space, &sparsity.diagonal_rows, &sparsity.diagonal_cols,
                &sparsity.off_diagonal_rows, &sparsity.off_diagonal_cols,
                coupling_vars);
}
...
void StandardGlobalAssembler< DataType >::assemble_scalar_impl(
    ...
    typename GlobalAssembler< DataType >::QuadratureSelectionFunction q_select)
    const {
  StandardScalarAssembly< InteriorAssemblyAlgorithm, DataType > assembly(space,
                                                                         vec);
...
void StandardGlobalAssembler< DataType >::assemble_multiple_scalar_impl(
    ...
    typename GlobalAssembler< DataType >::QuadratureSelectionFunction q_select)
    const {
  StandardMultipleScalarAssembly< InteriorAssemblyAlgorithm, DataType >
      assembly(space, vec, num_scalars);
  assembly.assemble(local_asm, q_select);
}
...
void StandardGlobalAssembler< DataType >::assemble_vector_impl(
    ...
    typename GlobalAssembler< DataType >::QuadratureSelectionFunction q_select)
    const {
  StandardVectorAssembly< InteriorAssemblyAlgorithm, DataType > assembly(space,
                                                                         vec);
  assembly.assemble(local_asm, q_select);
}
...
void StandardGlobalAssembler< DataType >::assemble_matrix_impl(
    ...
    typename GlobalAssembler< DataType >::QuadratureSelectionFunction q_select)
    const {
  StandardMatrixAssembly< InteriorAssemblyAlgorithm, DataType > assembly(space,
                                                                         mat);
  assembly.assemble(local_asm, q_select);
}
...
void StandardGlobalAssembler< DataType >::assemble_scalar_boundary_impl(
  ...
  StandardBoundaryScalarAssembly< BoundaryAssemblyAlgorithm, DataType >
      assembly(space, vec);
...
void StandardGlobalAssembler< DataType >::assemble_vector_boundary_impl(
    ...
    typename GlobalAssembler< DataType >::FacetQuadratureSelectionFunction
        fq_select) const {
  StandardVectorAssembly< BoundaryAssemblyAlgorithm, DataType > assembly(space,
                                                                         vec);
  assembly.assemble(local_asm, fq_select);
}
...
void StandardGlobalAssembler< DataType >::assemble_matrix_boundary_impl(
    ...
    typename GlobalAssembler< DataType >::FacetQuadratureSelectionFunction
        fq_select) const {
  StandardMatrixAssembly< BoundaryAssemblyAlgorithm, DataType > assembly(space,
                                                                         mat);
  assembly.assemble(local_asm, fq_select);
}
