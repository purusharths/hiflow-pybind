...
///                          resulting in a sparser structure. If this argument
///                          is not passed or is empty, full coupling of all
///                          variables is assumed. All rows and columns need to
///                          have the size of space.get_nb_var().
...
inline void InitStructure(const VectorSpace< DataType > &space,
  ...
  size_t ndof_total = space.dof().ndofs_on_sd(space.dof().get_my_subdomain());
  ...
  typename VectorSpace< DataType >::MeshEntityIterator mesh_it =
      space.mesh().begin(space.get_dim());
  typename VectorSpace< DataType >::MeshEntityIterator e_mesh_it =
      space.mesh().end(space.get_dim());
  while (mesh_it != e_mesh_it) {
    // loop over test variables
    for (int test_var = 0, tv_e = space.get_nb_var(); test_var != tv_e;
         ++test_var) {
      // get dof indices for test variable
      space.GetDofIndices(test_var, *mesh_it, &dof_ind_test);

      // loop over trial variables
      for (int trial_var = 0, vt_e = space.get_nb_var(); trial_var != vt_e;
           ++trial_var) {
  ...
  for (size_t i = 0; i != ndof_total; ++i) {
    ...
    for (SortedArray< int >::const_iterator it = raw_struct_diag[i].begin(),
  ...
  for (size_t i = 0; i != ndof_total; ++i) {
    ...
    for (SortedArray< int >::const_iterator it = raw_struct_offdiag[i].begin(),
