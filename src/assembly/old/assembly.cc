...
void initialize_linear_algebra_objects(
  const VectorSpace< DataType > &space,
  const GlobalAssembler< DataType > &global_asm, la::LaCouplings &couplings,
  ...
  space.GetLaCouplings(global_offsets, ghost_dofs, ghost_offsets);

  couplings.InitializeCouplings(global_offsets, ghost_dofs, ghost_offsets);
...
template < class DataType >
void interpolate_constrained_std_vector(const VectorSpace< DataType > &space,
  ...
  for (doffem::DofInterpolation::const_iterator it = interp.begin(),
       end = interp.end();
       it != end; ++it) {
...
template <>
void interpolate_constrained_vector< la::LADescriptorCoupledD >(
  const VectorSpace< la::LADescriptorCoupledD::DataType > &space,
  la::LADescriptorCoupledD::VectorType &vector) {
  interpolate_constrained_std_vector(space, vector);
}

template <>
void interpolate_constrained_vector< la::LADescriptorCoupledS >(
  const VectorSpace< la::LADescriptorCoupledS::DataType > &space,
  la::LADescriptorCoupledS::VectorType &vector) {
  interpolate_constrained_std_vector(space, vector);
}

template <>
void interpolate_constrained_vector<
la::LADescriptorBlock< la::LADescriptorCoupledD > >(
  const VectorSpace< la::LADescriptorCoupledD::DataType > &space,
  la::LADescriptorBlock< la::LADescriptorCoupledD >::VectorType &vector) {
  interpolate_constrained_std_vector(space, vector);
}
...
la::LADescriptorBlock< la::LADescriptorCoupledS > >(
  const VectorSpace< la::LADescriptorCoupledS::DataType > &space,
  la::LADescriptorBlock< la::LADescriptorCoupledS >::VectorType &vector) {
  interpolate_constrained_std_vector(space, vector);
}
...
la::LADescriptorPCE< la::LADescriptorCoupledD > >(
  const VectorSpace< la::LADescriptorCoupledD::DataType > &space,
  la::LADescriptorPCE< la::LADescriptorCoupledD >::VectorType &vector) {
  interpolate_constrained_std_vector(space, vector);
}
...
la::LADescriptorPCE< la::LADescriptorCoupledS > >(
  const VectorSpace< la::LADescriptorCoupledS::DataType > &space,
  la::LADescriptorPCE< la::LADescriptorCoupledS >::VectorType &vector) {
  interpolate_constrained_std_vector(space, vector);
}

#ifdef WITH_HYPRE
template <>
void interpolate_constrained_vector< la::LADescriptorHypreD >(
  const VectorSpace< la::LADescriptorHypreD::DataType > &space,
  la::LADescriptorHypreD::VectorType &vector) {
  interpolate_constrained_std_vector(space, vector);
}

template <>
void interpolate_constrained_vector<
la::LADescriptorBlock< la::LADescriptorHypreD > >(
  const VectorSpace< la::LADescriptorHypreD::DataType > &space,
  la::LADescriptorBlock< la::LADescriptorHypreD >::VectorType &vector) {
  interpolate_constrained_std_vector(space, vector);
}

template <>
void interpolate_constrained_vector<
la::LADescriptorPCE< la::LADescriptorHypreD > >(
  const VectorSpace< la::LADescriptorHypreD::DataType > &space,
  la::LADescriptorPCE< la::LADescriptorHypreD >::VectorType &vector) {
  interpolate_constrained_std_vector(space, vector);
}
#endif

template <>
void interpolate_constrained_vector< la::LADescriptorPolynomialChaosD >(
  const VectorSpace< la::LADescriptorPolynomialChaosD::DataType > &space,
  la::LADescriptorPolynomialChaosD::VectorType &vector) {
  for (int l = 0; l < vector.NModes(); ++l) {
    interpolate_constrained_std_vector(space, *vector.Mode(l));
  }
}
...
template < class DataType > GlobalAssembler< DataType >::~GlobalAssembler() {}

template < class DataType >
void GlobalAssembler< DataType >::compute_sparsity_structure(
  const VectorSpace< DataType > &space, SparsityStructure &sparsity,
...
template < class DataType >
void GlobalAssembler< DataType >::integrate_scalar(
  const VectorSpace< DataType > &space, ScalarAssemblyFunction local_asm,
...
template < class DataType >
void GlobalAssembler< DataType >::integrate_multiple_scalar(
  const VectorSpace< DataType > &space,
  ...
  assemble_multiple_scalar_impl(space, local_asm, num_scalars, cell_values,
                                q_select_);
...
template < class DataType >
void GlobalAssembler< DataType >::assemble_scalar(
  const VectorSpace< DataType > &space, ScalarAssemblyFunction local_asm,
...
template < class DataType >
void GlobalAssembler< DataType >::assemble_multiple_scalar(
  const VectorSpace< DataType > &space,
  MultipleScalarAssemblyFunction local_asm, const int num_scalars,
...
template < class DataType >
void GlobalAssembler< DataType >::assemble_vector(
  const VectorSpace< DataType > &space, VectorAssemblyFunction local_asm,
...
template < class DataType >
void GlobalAssembler< DataType >::assemble_matrix(
  const VectorSpace< DataType > &space, MatrixAssemblyFunction local_asm,
...
template < class DataType >
void GlobalAssembler< DataType >::integrate_scalar_boundary(
  const VectorSpace< DataType > &space,
  BoundaryScalarAssemblyFunction local_asm, DataType &integral) const {
...
template < class DataType >
void GlobalAssembler< DataType >::maximize_scalar_boundary(
  const VectorSpace< DataType > &space,
  BoundaryScalarAssemblyFunction local_asm, DataType &maximum) const {
  // TODO: actually the number of boundary facets is needed
  const size_t num_facets = space.mesh().num_entities(space.mesh().tdim() - 1);
  std::vector< DataType > cell_values(num_facets, 0.);
...
template < class DataType >
void GlobalAssembler< DataType >::assemble_scalar_boundary(
  const VectorSpace< DataType > &space,
...
template < class DataType >
void GlobalAssembler< DataType >::assemble_vector_boundary(
  const VectorSpace< DataType > &space,
  BoundaryVectorAssemblyFunction local_asm, GlobalVector &vec) const {
  if (should_reset_assembly_target_) {
    vec.Zeros();
  }
...
template < class DataType >
void GlobalAssembler< DataType >::assemble_matrix_boundary(
  const VectorSpace< DataType > &space,
  BoundaryMatrixAssemblyFunction local_asm, GlobalMatrix &matrix) const {
  if (should_reset_assembly_target_) {
    matrix.Zeros();
  }
...
template < class DataType >
void GlobalAssembler< DataType >::set_quadrature_selection_function(
...
template < class DataType >
void GlobalAssembler< DataType >::should_reset_assembly_target(
  bool should_reset) {
...
template < class DataType > struct DefaultQuadratureSelection {

  DefaultQuadratureSelection()
    : last_fe_id_(FEType< DataType >::NOT_SET), last_order_(-1) {}

  void operator()(const Element< DataType > &elem,
                  Quadrature< DataType > &quadrature) {
    const typename FEType< DataType >::FiniteElement fe_id =
    ...
    if (quadrature.size() > 0 && fe_id == last_fe_id_ &&
        desired_order == last_order_) {
      return;
    }
...
template < class DataType > struct DefaultFacetQuadratureSelection {
  ...
  void operator()(const Element< DataType > &elem,
    ...
    // typically sorted before traversal, which means that we
    // can minimize the number of quadrature switches through this.
    if (!(base_quadrature_.size() > 0 && fe_id == last_fe_id_ &&
          desired_order == last_order_) ||
        fe_id == 6) {
      switch (fe_id) {
      case FEType< DataType >::LAGRANGE_LINE:
      ...
      case FEType< DataType >::LAGRANGE_TET:
        ...
        base_quadrature_.set_quadrature_by_order("GaussTriangle",
            desired_order);
        break;
      case FEType< DataType >::LAGRANGE_HEX:
        base_quadrature_.set_cell_type(5);
        base_quadrature_.set_quadrature_by_order("GaussQuadrilateral",
            desired_order);
        break;
      case FEType< DataType >::LAGRANGE_PYR:
    ...
    quadrature.set_facet_quadrature(
      base_quadrature_, base_quadrature_.get_cell_type(), facet_number);
  }
...
GlobalAssembler< double >::default_select_ =
  DefaultQuadratureSelection< double >();
template <>
const GlobalAssembler< double >::FacetQuadratureSelectionFunction
GlobalAssembler< double >::default_facet_select_ =
  DefaultFacetQuadratureSelection< double >();

template <>
const GlobalAssembler< float >::QuadratureSelectionFunction
GlobalAssembler< float >::default_select_ =
  DefaultQuadratureSelection< float >();
template <>
const GlobalAssembler< float >::FacetQuadratureSelectionFunction
GlobalAssembler< float >::default_facet_select_ =
  DefaultFacetQuadratureSelection< float >();

template class GlobalAssembler< double >;
template class GlobalAssembler< float >;

template void interpolate_constrained_std_vector< double >(
  const VectorSpace< double > &space,
  GlobalAssembler< double >::GlobalVector &vector);
template void interpolate_constrained_std_vector< float >(
  const VectorSpace< float > &space,
  GlobalAssembler< float >::GlobalVector &vector);

/*template void initialize_linear_algebra_objects<double>(const VectorSpace&
space, const GlobalAssembler<double>& global_asm, la::LaCouplings& couplings,
                                                        typename
GlobalAssembler<double>::GlobalMatrix& matrix); template void
initialize_linear_algebra_objects<float>(const VectorSpace& space, const
GlobalAssembler<float>& global_asm, la::LaCouplings& couplings, typename
GlobalAssembler<float>::GlobalMatrix& matrix);*/
