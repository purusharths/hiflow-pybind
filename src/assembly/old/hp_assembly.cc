...
void compute_hp_sparsity_structure(
  ...
  const int num_total_dofs =
      space.dof().ndofs_on_sd(space.dof().get_my_subdomain());
  int local_row_dof;
  ...
  for (CellIterator cell_it = mesh.begin(tdim), cell_end = mesh.end(tdim);
    ...
    for (size_t test_var = 0, e_test_var = space.get_nb_var();
      ...
      for (DofIterator it_i = dofs_test.begin(), end_i = dofs_test.end();
        ...
        if (dof_i != interpolation.end()) {
          ...
          for (ConstraintIterator ci_it = dof_i->second.begin(),
            ...
            if (space.dof().is_dof_on_sd(ci_it->first)) {
              ...
              for (size_t trial_var = 0, e_trial_var = space.get_nb_var();
                ...
                if ((*coupling_vars)[test_var][trial_var]) {
                  ...
                  for (DofIterator it_j = dofs_trial.begin(),
                    ...
                    DofInterpolation::const_iterator dof_j =
                        interpolation.find(*it_j);
                    ...
                    if (dof_j != interpolation.end()) {
                      ...
                      for (ConstraintIterator cj_it = dof_j->second.begin(),
                        ...
                        LOG_DEBUG(
                            2, "[" << space.dof().get_my_subdomain()
                                   << "]   Constrained row = " << local_row_dof
                                   << ", constrained col = " << cj_it->first
                                   << ", diagonal ? "
                                   << space.dof().is_dof_on_sd(cj_it->first));
                      }
                    } else {
                      ...
                      LOG_DEBUG(2,
                                "[" << space.dof().get_my_subdomain()
                                    << "]   Constrained row = " << local_row_dof
                                    << ", unconstrained col = " << *it_j
                                    << ", diagonal ? "
                                    << space.dof().is_dof_on_sd(*it_j));
                    }
                  }
                }
              }
        ...
        if (space.dof().is_dof_on_sd(*it_i)) {
          ...
          LOG_DEBUG(2, "[" << space.dof().get_my_subdomain()
  ...
  for (size_t i = 0; i != num_total_dofs; ++i) {
    ...
    for (SortedArray< int >::const_iterator
             it = off_diagonal_couplings[i].begin(),
             end = off_diagonal_couplings[i].end();
         it != end; ++it) {
...
public:
  ...
  ~HpMatrixAssembly() {
    ...
    for (DofInterpolation::const_iterator it = interp_.begin(),
                                          end = interp_.end();
         it != end; ++it) {
        ...
        for (ConstraintIterator c_it = it_i->second.begin(),
          ...
          if (this->space_.dof().is_dof_on_sd(c_it->first)) {
            ...
            for (size_t j = 0; j != num_dofs; ++j) {
              DofInterpolation::const_iterator it_j =
...
template < class DataType >
void HpFemAssembler< DataType >::compute_sparsity_structure_impl(
    const VectorSpace< DataType > &space, SparsityStructure &sparsity,
...
template < class DataType >
void HpFemAssembler< DataType >::assemble_scalar_impl(
...
template < class DataType >
void HpFemAssembler< DataType >::assemble_multiple_scalar_impl(
    const VectorSpace< DataType > &space,
...
template < class DataType >
void HpFemAssembler< DataType >::assemble_vector_impl(
    const VectorSpace< DataType > &space,
...
template < class DataType >
void HpFemAssembler< DataType >::assemble_matrix_impl(
    const VectorSpace< DataType > &space,
...
void HpFemAssembler< DataType >::assemble_vector_boundary_impl(
  ...
  NOT_YET_IMPLEMENTED;
  // StandardGlobalAssembler<DataType> assembly;
  // assembly.set_quadrature_selection_function ( fq_select );
  // assembly.assemble_vector_boundary ( space, local_asm, vec );
}
...
void HpFemAssembler< DataType >::assemble_matrix_boundary_impl(
  ...
  // StandardGlobalAssembler<DataType> assembly;
  // assembly.set_quadrature_selection_function ( fq_select );
  // assembly.assemble_matrix_boundary ( space, local_asm, mat );
}
