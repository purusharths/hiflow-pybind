...
void create_sparsity_struct(const VectorSpace<DataType, DIM> &space, 
                            ...
                            std::vector< SortedArray< int > >& raw_diag,
                            std::vector< SortedArray< int > >& raw_offdiag)
{
                                ...
                                const std::vector< std::vector< bool > >& pre_coupling_vars)
{
  ...
  const int num_total_dofs = space.dof().nb_dofs_on_subdom(space.dof().my_subdom());
  int local_row_dof;
  ...
  {
    const int master_index = it->master_index();
    
    // loop over test variables
    for (size_t test_var = 0, e_test_var = space.nb_fe(); test_var != e_test_var; ++test_var) 
    {
      // Get dof id:s on cell
      space.get_dof_indices(test_var, master_index, &dof_list);

      // loop over trial variables
      for (size_t trial_var = 0, e_trial_var = space.nb_fe();trial_var != e_trial_var; ++trial_var) 
      {
        // check if coupling exists
        if (coupling_vars[test_var][trial_var]) 
        {
          // dont consider continuous case
          if ((space.fe_conformity(trial_var) != doffem::FE_CONFORM_H1) 
              || (space.fe_conformity(test_var) != doffem::FE_CONFORM_H1))
          {
      ...
      { // rows
        ...
        { // cols
          ...
          if (space.dof().is_dof_on_subdom(dof_list[j])) 
          {
            ...
            diagonal_couplings[local_dof_i].find_insert(dof_list[j]);
          } 
          else 
          {
            off_diagonal_couplings[local_dof_i].find_insert(dof_list[j]);
          }
        }
      } // for (int i=0;...
    }
        ...
        end_it = if_list.end(); it != end_it; ++it) 
  {
    ...
    {
      // Get dof id:s on cell
      space.get_dof_indices(test_var, master_index, &dofs_test);
               ...
               ci_it != ci_end; ++ci_it) 
          {
            ...
            if (space.dof().is_dof_on_subdom(ci_it->first)) 
            {
              ...
              for (size_t trial_var = 0, e_trial_var = space.nb_fe(); trial_var != e_trial_var; ++trial_var) 
              {
                ...
                {
                  // trial dofs from master cell
                  space.get_dof_indices(trial_var, master_index, &dofs_trial);

                  // trial dofs from slave cell in case of not fully continuous 
                    ...
                    || (space.fe_conformity(test_var) != doffem::FE_CONFORM_H1))
                  {
                    for (int s = 0, s_e = it->num_slaves(); s < s_e; ++s) 
                    {
                      ...
                      space.get_dof_indices(trial_var, it->slave_index(s), &slave_dofs);
                      dofs_trial.insert(dofs_trial.end(), slave_dofs.begin(), slave_dofs.end());
                    }
                  }
                                   ...
                                   it_j != end_j; ++it_j) 
                  {
                    ...
                    typename doffem::DofInterpolation<DataType>::const_iterator dof_j = interpolation.find(*it_j);
              ...
              for (size_t trial_var = 0, e_trial_var = space.nb_fe();
                ...
                if (coupling_vars[test_var][trial_var]) {
                  ...
                  for (DofIterator it_j = dofs_trial.begin(),
                    ...
                    // search current dof it_j in DofInterpolation map
                    typename doffem::DofInterpolation<DataType>::const_iterator dof_j =
                            ...
                            Quadrature< DataType > &master_quad) 
{
  ...
  doffem::ConstCellTrafoPtr<DataType, DIM> Ts = slave_elem.get_cell_transformation();
  doffem::ConstCellTrafoPtr<DataType, DIM> Tm = master_elem.get_cell_transformation();

  const size_t num_q = slave_quad.size();
  std::vector< Vec<DIM, DataType> > pts(num_q);
  std::vector<DataType> wgt(num_q, 0.);

  const int dim = slave_elem.get_cell().tdim();

  Vec<DIM, DataType> ref_pt, phys_pt;
