...
struct ElementCmp 
{
  ...
  bool operator()(int i, int j) const 
  {
    ...
    {
      const RefElement< DataType, DIM > *fe_i = fe_man_.get_fe(i, l);
      const RefElement< DataType, DIM > *fe_j = fe_man_.get_fe(j, l);
...
public:
  ...
  bool has_next() { return this->curr_ < this->traversal_.size(); }

  void reset(DataType &local_val) { local_val = 0.; }
  ...
  void reset(typename GlobalAssembler< DataType, DIM >::LocalMatrix &local_mat) {
