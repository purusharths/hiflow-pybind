...
public:
  ...
  virtual void init(ConstVectorSpacePtr< DataType, DIM > input_space);
  ...
  virtual VectorSpacePtr< DataType, DIM > get_patch_space() 
  { 
    return this->space_; 
  }
...
protected:
  ...
  ConstVectorSpacePtr< DataType, DIM > input_space_;
  MeshPtr input_mesh_;

  /// interpolating space
  VectorSpacePtr< DataType, DIM > space_;
...
template < class LAD, int DIM >
void SpacePatchInterpolation< LAD, DIM >::init(ConstVectorSpacePtr< DataType, DIM > input_space) 
{
  this->clear();
  ...
  this->space_.reset();
  this->space_ = VectorSpacePtr<DataType, DIM> (new VectorSpace<DataType, DIM> ());
  ...
  VectorType in_vec;
  VectorType out_vec;
  in_vec.Init(comm, input_space->la_couplings());
  out_vec.Init(comm, this->space_->la_couplings());
    
  this->fe_mapping_.init(input_space.get(), this->space_.get(), in_vec, out_vec, in_fe_ind, out_fe_ind);
