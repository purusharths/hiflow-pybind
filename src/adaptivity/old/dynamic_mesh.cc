...
DynamicMeshHandler< LAD, MESH_IMPL, DIM >::DynamicMeshHandler(
    DynamicMeshProblem< LAD, MESH_IMPL, DIM > *problem, MPI_Comm comm)
    : adapt_type_("None"), problem_(problem), mesh_(nullptr), initial_mesh_(nullptr),
      time_mesh_(nullptr), space_(nullptr), space_dual_(nullptr), tdim_(DIM), gdim_(DIM),
      active_mesh_index_(-1), comm_(comm) {
  MPI_Comm_rank(this->comm_, &rank_);
  fe_inter_type_ = this->problem_->get_fe_inter_type();
}
...
void DynamicMeshHandler< LAD, MESH_IMPL, DIM >::clear() {
  ...
  for (int l = 0; l < this->space_list_dual_.size(); ++l) {
    if (this->space_list_dual_[l] != nullptr) {
  ...
  {
      if (this->space_list_tmp_[l] != nullptr)
      {
  ...
  for (int l = 0; l < this->couplings_list_.size(); ++l) {
    if (this->couplings_list_[l] != nullptr) {
  ...
  for (int l = 0; l < this->couplings_list_dual_.size(); ++l) {
    if (this->couplings_list_dual_[l] != nullptr) {
  ...
  for (int l = 0; l < this->block_manager_list_.size(); ++l) {
    if (this->block_manager_list_[l] != nullptr) {
      delete this->block_manager_list_[l];
    }
  }
  this->block_manager_list_.clear();

  for (int l = 0; l < this->block_manager_list_dual_.size(); ++l) {
    if (this->block_manager_list_dual_[l] != nullptr) {
      delete this->block_manager_list_dual_[l];
    }
  }
...
template < class LAD, IMPL MESH_IMPL, int DIM >
void DynamicMeshHandler< LAD, MESH_IMPL, DIM >::read_mesh_change_list(
  ...
  this->set_initial_mesh_change_times(mesh_change_times);
}

template < class LAD, IMPL MESH_IMPL, int DIM >
void DynamicMeshHandler< LAD, MESH_IMPL, DIM >::write_mesh_change_list(
...
template < class LAD, IMPL MESH_IMPL, int DIM >
void DynamicMeshHandler< LAD, MESH_IMPL, DIM >::set_update_vectors(
    int mode, int type, std::vector< VectorType * > vectors) {
  if (mode == 1) {
    if (type == 1) {
...
template < class LAD, IMPL MESH_IMPL, int DIM >
int DynamicMeshHandler< LAD, MESH_IMPL, DIM >::first_step_for_mesh(
...
template < class LAD, IMPL MESH_IMPL, int DIM >
int DynamicMeshHandler< LAD, MESH_IMPL, DIM >::last_step_for_mesh(
    int mesh_index) const {
  assert(mesh_index >= 0);
  assert(mesh_index < this->num_mesh());
...
template < class LAD, IMPL MESH_IMPL, int DIM >
int DynamicMeshHandler< LAD, MESH_IMPL, DIM >::num_cells_by_step(
    int time_step) const {
...
template < class LAD, IMPL MESH_IMPL, int DIM >
int DynamicMeshHandler< LAD, MESH_IMPL, DIM >::num_cells_by_index(
    int mesh_index) const {
  if (mesh_index < 0) {
    return 0;
  }
...
template < class LAD, IMPL MESH_IMPL, int DIM >
VectorSpace< typename LAD::DataType > *
DynamicMeshHandler< LAD, MESH_IMPL, DIM >::get_space_by_step(int time_step,
                                                             int mode) const {
  if (time_step < 0) {
    return nullptr;
  }
...
template < class LAD, IMPL MESH_IMPL, int DIM >
VectorSpace< typename LAD::DataType > *
DynamicMeshHandler< LAD, MESH_IMPL, DIM >::get_space_by_index(int mesh_index,
                                                              int mode) const {
  assert(mesh_index >= 0);
  assert(mesh_index < this->num_mesh());
...
template < class LAD, IMPL MESH_IMPL, int DIM >
LaCouplings *DynamicMeshHandler< LAD, MESH_IMPL, DIM >::get_couplings_by_index(
    int mesh_index, int mode) const {
  assert(mesh_index >= 0);
  assert(mesh_index < this->num_mesh());
...
template < class LAD, IMPL MESH_IMPL, int DIM >
BlockManager *
DynamicMeshHandler< LAD, MESH_IMPL, DIM >::get_block_manager_by_index(
    int mesh_index, int mode) const {
  assert(mesh_index >= 0);
  assert(mesh_index < this->num_mesh());

  if (mode == 1) {
    return this->block_manager_list_[mesh_index];
  } else if (mode == -1) {
...
template < class LAD, IMPL MESH_IMPL, int DIM >
MeshPtr DynamicMeshHandler< LAD, MESH_IMPL, DIM >::get_mesh_by_step(
    int time_step) const {
  if (time_step < 0) {
    return nullptr;
  }
...
template < class LAD, IMPL MESH_IMPL, int DIM >
MeshPtr DynamicMeshHandler< LAD, MESH_IMPL, DIM >::get_mesh_by_index(
    int mesh_index) const {
  assert(mesh_index >= 0);
  assert(mesh_index < this->num_mesh());
...
template < class LAD, IMPL MESH_IMPL, int DIM >
bool DynamicMeshHandler< LAD, MESH_IMPL, DIM >::need_to_change_mesh(
    DataType last_time, DataType cur_time) const {
...
template < class LAD, IMPL MESH_IMPL, int DIM >
bool DynamicMeshHandler< LAD, MESH_IMPL, DIM >::need_to_change_mesh(
    int last_step, int cur_step) const {
  if (last_step < 0) {
    return true;
  }
...
template < class LAD, IMPL MESH_IMPL, int DIM >
bool DynamicMeshHandler< LAD, MESH_IMPL, DIM >::need_to_change_mesh_interval(
    int low_step, int high_step) {
...
void DynamicMeshHandler< LAD, MESH_IMPL, DIM >::fill_mesh_list() {
  ...
  for (int l = 0; l < num_mesh; ++l) {
    if (MESH_IMPL == mesh::IMPL_P4EST) {
      ...
      // std::cout << this->mesh_list_[l] << " ref count " <<
      // this->mesh_list_[l]->ref_count() << " --- " <<
      // this->mesh_list_[l]->ref_count_ << std::endl; exit(-1);
    } else if (MESH_IMPL == mesh::IMPL_DBVIEW) {
      this->mesh_list_[l] = new MeshDbView(this->tdim_, this->gdim_);
    }
    // std::cout << this->initial_mesh_ << " initial before copy ref count " <<
    // this->initial_mesh_->ref_count() << std::endl;

    this->mesh_list_[l]->deep_copy_from(this->initial_mesh_);

    // std::cout << this->mesh_list_[l] << " ref count after copy " <<
    // this->mesh_list_[l]->ref_count() << std::endl; std::cout <<
    // this->initial_mesh_ << " initial after copy ref count " <<
    // this->initial_mesh_->ref_count() << std::endl;
  }
}
...
void DynamicMeshHandler< LAD, MESH_IMPL, DIM >::init_fe_spaces() {
  ...
  this->block_manager_list_.resize(num_mesh);
  ...
  for (int l = 0; l < num_mesh; ++l) {
    ...
    this->couplings_list_[l] = new LaCouplings();
    this->couplings_list_dual_[l] = new LaCouplings();

    this->block_manager_list_[l] = new BlockManager();
    this->block_manager_list_dual_[l] = new BlockManager();

    this->problem_->setup_space(*this->space_list_[l],
                                this->coupling_vars_list_[l],
                                this->mesh_list_[l], 1);
...
template < class LAD, IMPL MESH_IMPL, int DIM >
bool DynamicMeshHandler< LAD, MESH_IMPL, DIM >::update(int time_step, int mode,
                                                       bool initial_call) {
  ...
  LOG_DEBUG(1, "Change from mesh " << this->active_mesh_index_ << " to mesh "
                                   << new_mesh_index);
  ...
  if (!initial_call) {
    if (mode == 1) {
      ...
      for (int l = 0; l < p_vectors.size(); ++l) {
        ...
        p_vectors[l]->Update();
        interpolate_constrained_vector< LAD >(*this->get_active_space(1),
                                              *p_vectors[l]);
        p_vectors[l]->Update();
      }
    }
  ...
  int num_global_cells =
      this->space_->meshPtr()->num_global_cells(this->space_->get_mpi_comm());
  ...
  LOG_DEBUG(2, "  Active space has " << this->space_->dof().ndofs_global()
                                     << " total dofs and " << num_global_cells
                                     << " cells ");

  // 3. reinit LA objects
  if (mode == 1) {
    this->problem_->setup_LA_primal(
        *this->space_, *this->space_dual_,
        this->coupling_vars_list_[c_mesh_index],
        this->coupling_vars_list_dual_[c_mesh_index],
        *this->couplings_list_[c_mesh_index],
        *this->couplings_list_dual_[c_mesh_index],
        *this->block_manager_list_[c_mesh_index],
  ...
  if (mode == 1) {
    for (int l = 0; l < p_vectors.size(); ++l) {
      LOG_DEBUG(2, "  Interpolate primal vector "
                       << l << " from mesh " << old_mesh_index << " to mesh "
                       << new_mesh_index);
      this->interpolate_vector(*p_vectors[l], old_mesh_index,
                               *this->vec_primal_primal_[l], new_mesh_index);
    }
  } else if (mode == -1) {
    for (int l = 0; l < p_vectors.size(); ++l) {
      LOG_DEBUG(2, "  Interpolate primal vector "
                       ...
                       << new_mesh_index);
      this->interpolate_vector(*p_vectors[l], old_mesh_index,
                               *this->vec_dual_primal_[l], new_mesh_index);
    }
    for (int l = 0; l < d_vectors.size(); ++l) {
      LOG_DEBUG(2, "  Interpolate dual vector " << l << " from mesh "
                                                << old_mesh_index << " to mesh "
                                                << new_mesh_index);
      this->interpolate_vector(*d_vectors[l], old_mesh_index,
                               *this->vec_dual_dual_[l], new_mesh_index);
    }
  } else if (mode == 0) {
    for (int l = 0; l < p_vectors.size(); ++l) {
      LOG_DEBUG(2, "  Interpolate primal vector "
    ...
    for (int l = 0; l < d_vectors.size(); ++l) {
      LOG_DEBUG(2, "  Interpolate dual vector " << l << " from mesh "
                                                ...
                                                << new_mesh_index);
      this->interpolate_vector(*d_vectors[l], old_mesh_index,
                               *this->vec_est_dual_[l], new_mesh_index);
    }
...
template < class LAD, IMPL MESH_IMPL, int DIM >
void DynamicMeshHandler< LAD, MESH_IMPL, DIM >::create_fe_interpolator(
    int num_mesh) {
  this->fe_inter_type_ = this->problem_->get_fe_inter_type();
    ...
    for (int k = 0; k < num_mesh; ++k) {
      if (this->fe_interpolator_[l][k] != nullptr) {
        ...
        if (this->fe_inter_type_ == FE_INTER_LAGRANGE) {
          this->fe_interpolator_[l][k] =
              new FEInterpolationLagrange< LAD, DIM >();
...
template < class LAD, IMPL MESH_IMPL, int DIM >
void DynamicMeshHandler< LAD, MESH_IMPL, DIM >::interpolate_vector( const VectorType &in_vec, int in_index, 
                                                                    VectorType &out_vec, int out_index) 
{
  if (!this->fe_interpolator_.at(in_index).at(out_index)->is_initialized()) 
  {
    if (this->fe_inter_type_ == FE_INTER_INNERPROD) 
    {
      FEInterpolationInnerProduct< LAD, DIM > *tmp_inter = dynamic_cast< FEInterpolationInnerProduct< LAD, DIM > * >(
                                                            this->fe_interpolator_.at(in_index).at(out_index));

      assert(tmp_inter != nullptr);
      assert(this->problem_->get_fe_inter_asm() != nullptr);

      tmp_inter->set_inner_product_assembler( this->problem_->get_fe_inter_asm(), this->problem_->get_fe_inter_coupling_vars());
      tmp_inter->set_dirichlet_bc( this->problem_->get_fe_inter_dirichlet_dofs(), this->problem_->get_fe_inter_dirichlet_values());
      tmp_inter->set_solver(this->problem_->get_fe_inter_solver());
    }
    this->fe_interpolator_.at(in_index).at(out_index)->init(this->get_space_by_index(in_index, 1), this->get_space_by_index(out_index, 1), in_vec);
  }

  LOG_INFO ("", "Interpolate vector from " << in_index << " to " << out_index);
  this->fe_interpolator_.at(in_index).at(out_index)->interpolate(in_vec, out_vec);
}

template < class LAD, IMPL MESH_IMPL, int DIM >
void DynamicMeshHandler< LAD, MESH_IMPL, DIM >::add_mesh_change_times(
    const std::vector< DataType > &additional_changes,
  ...
  for (int t = 0; t < new_change_list.size(); ++t) {
    is_new_change.insert(
        std::pair< DataType, bool >(new_change_list.data().at(t), false));
  }
  ...
  for (int t = 0; t < additional_changes.size(); ++t) {
    ...
    is_new_change.insert(
        std::pair< DataType, bool >(additional_changes[t], true));
  }
  ...
  LOG_DEBUG(2,
            " old indicator 2 mesh index: " << string_from_range(
                indicator_mesh_indices.begin(), indicator_mesh_indices.end()));

  indicator_mesh_indices = new_mesh_indices;

  LOG_DEBUG(2,
            " new indicator 2 mesh index: " << string_from_range(
                indicator_mesh_indices.begin(), indicator_mesh_indices.end()));
...
template < class LAD, IMPL MESH_IMPL, int DIM >
void DynamicMeshHandler< LAD, MESH_IMPL, DIM >::refine(
    int mesh_index, std::vector< int > &markers) {
  ...
  if (MESH_IMPL == IMPL_P4EST) {
    this->mesh_list_[mesh_index] = compute_ghost_cells(
        *active_mesh, this->comm_, shared_verts, mesh::IMPL_P4EST, 2);
  } else if (MESH_IMPL == IMPL_DBVIEW) {
    this->mesh_list_[mesh_index] = compute_ghost_cells(
        *active_mesh, this->comm_, shared_verts, mesh::IMPL_DBVIEW, 1);
  }
...
template < class LAD, IMPL MESH_IMPL, int DIM >
void DynamicMeshHandler< LAD, MESH_IMPL, DIM >::load_mesh_from_file(
    int adapt_counter, int num_mesh, std::string &prefix) {
  this->mesh_list_.clear();
  this->mesh_list_.resize(num_mesh);
...
template < class LAD, IMPL MESH_IMPL, int DIM >
void DynamicMeshHandler< LAD, MESH_IMPL, DIM >::visualize_mesh(
    int adapt_counter, std::string &prefix) {
  for (int m = 0; m < this->num_mesh(); ++m) {
    ...
    a_pre << this->root_ << "/mesh/adapt_mesh." << this->ac_ << "."
    << m;
    ...
    a_name << this->root_ << "/mesh/adapt_mesh." << this->ac_ << "."
...
template < class LAD, IMPL MESH_IMPL, int DIM >
void DynamicMeshHandler< LAD, MESH_IMPL, DIM >::save_mesh_to_file(
    int adapt_counter, std::string &prefix) {
  for (int l = 0; l < this->mesh_list_.size(); ++l) {
    std::stringstream pre;
    pre << prefix << "." << adapt_counter << "." << l << ".h5";
...
template class DynamicMeshHandler< LADescriptorBlock< LADescriptorCoupledD >,
                                   IMPL_DBVIEW, 2 >;
template class DynamicMeshHandler< LADescriptorBlock< LADescriptorCoupledD >,
                                   IMPL_DBVIEW, 3 >;
//#ifdef WITH_P4EST
...
template class DynamicMeshHandler< LADescriptorBlock< LADescriptorHypreD >,
                                   IMPL_P4EST, 2 >;
template class DynamicMeshHandler< LADescriptorBlock< LADescriptorHypreD >,
                                   IMPL_P4EST, 3 >;
#endif
...
template < class LAD, IMPL MESH_IMPL, int DIM >
void DynamicMeshProblem< LAD, MESH_IMPL, DIM >::init_dmh(
...
void DynamicMeshProblem< LAD, MESH_IMPL, DIM >::setup_auxiliary_interpolation(
    VectorSpace< DataType > *old_space, VectorSpace< DataType > *new_space,
  ...
  */
  ...
  if (this->fe_inter_type_ == FE_INTER_INNERPROD) {
    ...
    tmp_inter->set_inner_product_assembler(local_asm, this->get_fe_inter_coupling_vars());

    tmp_inter->set_dirichlet_bc(this->get_fe_inter_dirichlet_dofs(), this->get_fe_inter_dirichlet_values());
...
template < class LAD, IMPL MESH_IMPL, int DIM >
void DynamicMeshProblem< LAD, MESH_IMPL, DIM >::interpolate_vector(const typename LAD::VectorType &in_vec, 
                                                                   typename LAD::VectorType &out_vec) 
{
...
void DynamicMeshProblem< LAD, MESH_IMPL, DIM >::interpolate_vector( const typename LAD::VectorType &in_vec, int in_index,
                                                                    typename LAD::VectorType &out_vec, int out_index, int mode) 
{
  if (in_index == -1) 
  {
    return;
...
template < class LAD, IMPL MESH_IMPL, int DIM >
void DynamicMeshProblem< LAD, MESH_IMPL, DIM >::adapt_mesh_change_list(std::string adapt_type, 
                                                                       int adapt_counter, 
                                                                       int min_steps_for_mesh,
                                                                       int max_mesh_number,
                                                                       int rate,
                                                                       typename LAD::DataType tol,
                                                                       ...
                                                                       std::vector< int > &indicator_mesh_indices) 
{
  ...
  // Allreduce
  MPI_Allreduce(&local_diff[0], &global_diff[0], num_steps - 1, MPI_DOUBLE,
                MPI_SUM, this->dmh_->get_mpi_comm());
  MPI_Allreduce(&local_norm[0], &global_norm[0], num_steps - 1, MPI_DOUBLE,
                MPI_SUM, this->dmh_->get_mpi_comm());
  ...
  LOG_DEBUG(1, "Old mesh change steps " << string_from_range(
                   mesh_change_steps.begin(), mesh_change_steps.end()));
      ...
      if (change_steps.size() + num_mesh < max_mesh_number) {
        ...
        if (trial_step > 0) {
          ...
          if (!mesh_change_steps.find(trial_step, &pos)) {
            ...
            if (prev_step >= 0 &&
                (trial_step - prev_step) <= min_steps_for_mesh) {
            ...
            if (next_step >= 0 &&
                (next_step - trial_step) <= min_steps_for_mesh) {
              mesh_change_steps.erase(pos);
              continue;
            }
  ...
  LOG_DEBUG(1, "New mesh change steps " << string_from_range(
                   mesh_change_steps.begin(), mesh_change_steps.end()));
...
template class DynamicMeshProblem< LADescriptorBlock< LADescriptorCoupledD >, IMPL_DBVIEW, 2 >;
template class DynamicMeshProblem< LADescriptorBlock< LADescriptorCoupledD >, IMPL_DBVIEW, 3 >;
...
template class DynamicMeshProblem< LADescriptorBlock< LADescriptorHypreD >, IMPL_P4EST, 2 >;
template class DynamicMeshProblem< LADescriptorBlock< LADescriptorHypreD >, IMPL_P4EST, 3 >;
#endif
//#   else
...
template class DynamicMeshProblem< LADescriptorBlock< LADescriptorCoupledD >, IMPL_P4EST, 2 >;
template class DynamicMeshProblem< LADescriptorBlock< LADescriptorCoupledD >, IMPL_P4EST, 3 >;
//#   endif
//#else
#ifdef WITH_HYPRE
...
template class DynamicMeshProblem< LADescriptorBlock< LADescriptorHypreD >, IMPL_DBVIEW, 2 >;
template class DynamicMeshProblem< LADescriptorBlock< LADescriptorHypreD >, IMPL_DBVIEW, 3 >;
#endif
//#endif
} // namespace hiflow
