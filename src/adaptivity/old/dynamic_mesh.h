...
namespace hiflow {
template < class LAD, IMPL MESH_IMPL, int DIM > class DynamicMeshProblem;
...
template < class LAD, IMPL MESH_IMPL, int DIM > class DynamicMeshHandler {
  typedef typename LAD::DataType DataType;
  typedef typename LAD::VectorType VectorType;

public:
  DynamicMeshHandler(DynamicMeshProblem< LAD, MESH_IMPL, DIM > *problem,
                     MPI_Comm comm);

  ~DynamicMeshHandler() { this->clear(); }
  ...
  virtual void interpolate_vector(const VectorType &in_vec, int in_index,
  ...
  virtual void load_mesh_from_file(int adapt_counter, int num_mesh,
                                   std::string &prefix);
  ...
  /// index
  virtual void
  add_mesh_change_times(const std::vector< DataType > &additional_changes,
  ...
  virtual void
  set_initial_mesh_change_times(const std::vector< DataType > &times) {
  ...
  virtual void set_adapt_type(std::string type) { this->adapt_type_ = type; }
  ...
  virtual void set_update_vectors(int mode, int type,
                                  std::vector< VectorType * > vectors);

  /// \brief read in list of mesh chang times
  /// @param[in] path path to mesh change list
  virtual void read_mesh_change_list(const std::string &path);

  /// \brief write out list of mesh chang times
  /// @param[in] path path to mesh change list
  virtual void write_mesh_change_list(const std::string &path) const;
  ...
  virtual int get_active_mesh_index() { return this->active_mesh_index_; }
  ...
  virtual VectorSpace< DataType > *get_space_by_step(int time_step,
                                                     int mode) const;
  ...
  /// @param[in] mode 1: primal -1: dual
  /// @return pointer to vector space
  virtual VectorSpace< DataType > *get_space_by_index(int mesh_index,
                                                      int mode) const;
  ...
  virtual std::vector< MeshPtr > get_mesh_list() const {
  ...
  virtual MeshPtr get_active_mesh() const { return this->mesh_; }

  /// \brief get underlying MPI communicator
  /// @return communicator

  virtual MPI_Comm &get_mpi_comm() { return this->comm_; }

  /// \brief get LA couplings for active mesh
  /// @param[in] mode 1: primal -1: dual
  /// @return couplings

  virtual LaCouplings *get_active_couplings(int mode) const {
  ...
  virtual BlockManager *get_active_block_manager(int mode) const {
  ...
  /// @param[in] mode 1: primal -1: dual
  /// @return couplings
  virtual LaCouplings *get_couplings_by_index(int mesh_index, int mode) const;

  /// \brief get LA couplings for specified mesh index
  /// @param[in] mesh_index index of considered mesh
  /// @param[in] mode 1: primal -1: dual
  /// @return couplings
  virtual BlockManager *get_block_manager_by_index(int mesh_index,
                                                   int mode) const;

  /// \brief get number of different meshes
  /// @return number

  virtual int num_mesh() const { return this->mesh_list_.size(); }
  ...
  /// @return bool
  virtual bool need_to_change_mesh_interval(int low_step, int high_step);
...
protected:
  ...
  FEInterpolationType fe_inter_type_;
  std::vector< std::vector< FEInterpolation< LAD, DIM > * > > fe_interpolator_;

  std::vector< LaCouplings * > couplings_list_;
  std::vector< LaCouplings * > couplings_list_dual_;

  std::vector< BlockManager * > block_manager_list_;
  std::vector< BlockManager * > block_manager_list_dual_;
...
public:
  ...
  DynamicMeshProblem(MPI_Comm comm)
      : dmh_(new DynamicMeshHandler< LAD, MESH_IMPL, DIM >(this, comm)),
        active_mesh_index_(-1), aux_fe_interpolator_(nullptr) {}
  ...
  virtual void setup_space(VectorSpace< typename LAD::DataType > &space,
                           std::vector< std::vector< bool > > &coupling_vars,
                           MeshPtr mesh, int mode) = 0;
  ...
  virtual void
  setup_LA_primal(VectorSpace< typename LAD::DataType > &space,
                  VectorSpace< typename LAD::DataType > &space_dual,
                  std::vector< std::vector< bool > > &coupling_vars,
                  std::vector< std::vector< bool > > &coupling_vars_dual,
                  LaCouplings &couplings, LaCouplings &couplings_dual,
                  BlockManager &block_manager,
                  BlockManager &block_manager_dual) = 0;
  ...
  /// @param[out] couplings_dual LA couplings for dual LA objects
  virtual void
  setup_LA_dual(VectorSpace< typename LAD::DataType > &space,
                VectorSpace< typename LAD::DataType > &space_dual,
                std::vector< std::vector< bool > > &coupling_vars,
                std::vector< std::vector< bool > > &coupling_vars_dual,
                LaCouplings &couplings, LaCouplings &couplings_dual,
  ...
  /// @param[out] couplings_dual LA couplings for dual LA objects
  virtual void
  setup_LA_est(VectorSpace< typename LAD::DataType > &space,
               VectorSpace< typename LAD::DataType > &space_dual,
               std::vector< std::vector< bool > > &coupling_vars,
               std::vector< std::vector< bool > > &coupling_vars_dual,
               LaCouplings &couplings, LaCouplings &couplings_dual,
  ...
  virtual void set_active_space(VectorSpace< typename LAD::DataType > *space,
                                int mode) = 0;
  ...
  /// \brief routine which is called in init_dmh() of DynamicMeshProblem to get
  /// initial points for mesh changes and pass them to dmh object
  virtual void init_mesh_change_list() = 0;

  /// \brief routine which is called in init_dmh() of DynamicMeshProblem to pass
  /// those vectors to dmh object, which have to be interpolated from \br one
  /// space to another one inside of update() of DynamicMeshHandler
  ...
  virtual int active_mesh_index() { return this->active_mesh_index_; }

  virtual InnerProductAssembler< LAD, DIM > *get_fe_inter_asm() = 0;

  virtual std::vector< std::vector< bool > > get_fe_inter_coupling_vars() = 0;

  virtual std::vector< int > get_fe_inter_dirichlet_dofs() = 0;

  virtual std::vector< DataType > get_fe_inter_dirichlet_values() = 0;

  virtual LinearSolver< LAD > *get_fe_inter_solver() = 0;

  FEInterpolationType get_fe_inter_type() const { return this->fe_inter_type_; }

  /// \brief main intialization routine for dmh object. This has to be called in
  /// the very beginning of an application
  /// @paam[in] adapt_type type of mesh change points adaption. see
  /// set_adapt_type() of DynamiMeshHandler
  ...
  void interpolate_vector(const typename LAD::VectorType &in_vec, int in_index,
                          typename LAD::VectorType &out_vec, int out_index,
                          int mode);

  void interpolate_vector(const typename LAD::VectorType &in_vec,
                          typename LAD::VectorType &out_vec);

  // TODO use const pointers
  void setup_auxiliary_interpolation(VectorSpace< DataType > *old_space,
                                     VectorSpace< DataType > *new_space,
                                     const VectorType &old_vec);

  virtual void adapt_boundary(MeshPtr mesh){};

  virtual bool need_dual_solution() const { return false; }

  virtual bool need_higher_order_interpolation() const { return false; }

  virtual bool do_error_estimation() const { return false; }
...
protected:
  ...
  virtual void adapt_mesh_change_list(
      std::string adapt_type, int adapt_counter, int min_steps_for_mesh,
      int max_mesh_number, int fixed_fraction_rate,
      typename LAD::DataType fixed_error_tol,
      const std::vector< std::vector< typename LAD::DataType > >
          &space_indicator,
      std::vector< int > &indicator_mesh_indices);
  ...
  FEInterpolation< LAD, DIM > *aux_fe_interpolator_;
