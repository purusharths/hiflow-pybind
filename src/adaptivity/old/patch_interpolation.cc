...
SpacePatchInterpolation< LAD, MESH_IMPL >::SpacePatchInterpolation()
    : eps_(1e-12), num_var_(0), initialized_(false), mesh_(nullptr),
...
template < class LAD, IMPL MESH_IMPL >
void SpacePatchInterpolation< LAD, MESH_IMPL >::init(
    VectorSpace< DataType > const *input_space) {
  this->clear();
  ...
  for (int v = 0; v < this->num_var_; ++v) {
    ...
    this->degrees_[v] =
        input_space->fe_manager().get_fe_on_cell(0, v)->get_fe_deg() * 2;
  }
  ...
  this->couplings_.Init(comm);
  std::vector< int > global_offsets;
  std::vector< int > ghost_dofs;
  std::vector< int > ghost_offsets;
  this->space_.GetLaCouplings(global_offsets, ghost_dofs, ghost_offsets);

  this->couplings_.InitializeCouplings(global_offsets, ghost_dofs,
                                       ghost_offsets);
...
bool SpacePatchInterpolation< LAD, MESH_IMPL >::check_space() const {
  ...
  for (int v = 0; v < this->num_var_; ++v) {
    int ref_deg =
        this->input_space_->fe_manager().get_fe_on_cell(0, v)->get_fe_deg();
    for (EntityNumber jc = 1; jc < this->input_mesh_->num_entities(this->tdim_);
         ++jc) {
...
void SpacePatchInterpolation< LAD, MESH_IMPL >::create_dof_mapping() {
  ...
  for (EntityIterator cell_it = this->input_mesh_->begin(this->tdim_);
       cell_it != this->input_mesh_->end(this->tdim_); cell_it++) {
    Id child_id = cell_it->id();
    EntityNumber child_index = cell_it->index();
    Id parent_id = this->input_mesh_->get_parent_cell_id(child_index);

    if (parent_id >= 0) {
      std::map< Id, std::vector< Id > >::iterator it =
          descendants.find(parent_id);
  ...
  for (EntityIterator cell_it = this->mesh_->begin(this->tdim_);
       cell_it != this->mesh_->end(this->tdim_); cell_it++) {
    ...
    for (int l = 0; l < children_ids.size(); ++l) {
      int child_index = -1;
      for (int index = 0; index < this->input_mesh_->num_entities(this->tdim_);
           ++index) {
      ...
      Entity child_cell =
          this->input_mesh_->get_entity(this->tdim_, child_index);
      ...
      this->input_space_->dof().get_coord_on_cell(0, child_cell.index(),
                                                  low_coords);
      ...
      for (int i = 0; i < num_high_dofs; i++) {
        if (rank_ !=
            this->input_space_->dof().owner_of_dof(high_dof_ids.at(i))) {
          continue;
        }
        ...
        for (int j = 0; j < num_low_dofs; ++j) {
          ...
          for (int d = 0; d < this->gdim_; ++d) {
            dist += (high_coords[i][d] - low_coords[j][d]) *
                    (high_coords[i][d] - low_coords[j][d]);
          }
          dist = std::sqrt(dist);
...
template < class LAD, IMPL MESH_IMPL >
void SpacePatchInterpolation< LAD, MESH_IMPL >::interpolate(
    const VectorType &input_vector, VectorType &vector) const {
  if (!this->initialized_) {
    std::cout << "SpacePatchInterpolation is not initialized ! " << std::endl;
    exit(-1);
  }

  // Loop over all higher order dofs and copy df values from low order vector to
  // higher order vector
  for (std::map< int, int >::const_iterator it = this->high2low_.begin();
       it != this->high2low_.end(); ++it) {
...
void SpacePatchInterpolation< LAD, MESH_IMPL >::copy_mesh() {
  if (MESH_IMPL == mesh::IMPL_P4EST) {
    ...
    std::cout
        << "SpacePatchInterpolation::copy_mesh() does only work for P4est mesh"
        << std::endl;
    exit(-1);
  }
...
void SpacePatchInterpolation< LAD, MESH_IMPL >::compute_ghost() {
  ...
  this->mesh_ = compute_ghost_cells(*this->mesh_, comm, shared_verts, MESH_IMPL,
...
template class SpacePatchInterpolation< LADescriptorCoupledD,
                                        mesh::IMPL_P4EST >;
#ifdef WITH_HYPRE
template class SpacePatchInterpolation< LADescriptorHypreD, IMPL_P4EST >;
#endif
template class SpacePatchInterpolation<
    LADescriptorBlock< LADescriptorCoupledD >, mesh::IMPL_P4EST >;
#ifdef WITH_HYPRE
template class SpacePatchInterpolation< LADescriptorBlock< LADescriptorHypreD >,
                                        IMPL_P4EST >;
#endif
...
template < typename T, class DataType >
T TimePatchInterpolation< T, DataType >::patch_dt_linear(
    const DataType c, const T &val_prev, const T &val,
    const T &val_next) const {
  return (this->b_c1_ * val + this->b_n1_ * val_next);
}

template < typename T, class DataType >
T TimePatchInterpolation< T, DataType >::patch_quadratic(
    const DataType c, const T &val_prev, const T &val,
    const T &val_next) const {
  DataType t = this->get_absolut_time(c);
  DataType tt = t * t;
...
template < typename T, class DataType >
T TimePatchInterpolation< T, DataType >::patch_dt_quadratic(
    const DataType c, const T &val_prev, const T &val,
    const T &val_next) const {
  DataType t = this->get_absolut_time(c);
...
template < typename T, class DataType >
DataType
TimePatchInterpolation< T, DataType >::get_absolut_time(DataType c) const {
  switch (this->rel_time_) {
  case 0:
