...
public:
  ...
  VectorSpacePtr< DataType, DIM > get_space_by_step(int time_step, int mode) const;

  /// \brief get pointer to vector space corresponding to specified mesh
  /// @param[in] mesh_index index of considered mesh
  /// @param[in] mode 1: primal -1: dual
  /// @return pointer to vector space
  VectorSpacePtr< DataType, DIM > get_space_by_index(int mesh_index, int mode) const;

  /// \brief get pointer to vector space corresponding to active mesh
  /// @param[in] mode 1: primal -1: dual
  /// @return pointer to vector space

  VectorSpacePtr< DataType, DIM > get_active_space(int mode) const 
  {
  ...
  BlockManagerPtr get_active_block_manager(int mode) const {
  ...
  /// @param[in] mode 1: primal -1: dual
  /// @return couplings
  BlockManagerPtr get_block_manager_by_index(int mesh_index, int mode) const;
...
protected:
  ...
  VectorSpacePtr< DataType, DIM > space_primal_;
  VectorSpacePtr< DataType, DIM > space_dual_;
  ...
  std::vector< VectorSpacePtr< DataType, DIM > > space_list_primal_;

  std::vector< VectorSpacePtr< DataType, DIM > > space_list_dual_;

  std::vector< VectorSpacePtr< DataType, DIM > > space_list_tmp_;

  std::vector< std::vector< FEInterpolator * > > fe_interpolator_;

  std::vector< BlockManagerPtr > block_manager_list_primal_;
  std::vector< BlockManagerPtr > block_manager_list_dual_;
...
template < class LAD, int DIM, class FEInterpolator >
VectorSpacePtr< typename LAD::DataType, DIM > 
...
void DynamicMeshHandler< LAD, DIM, FEInterpolator >::init_fe_spaces() 
{
  ...
  for (int l = 0; l < num_mesh; ++l) 
  {
    this->space_list_primal_[l] = VectorSpacePtr<DataType, DIM>(new VectorSpace< DataType, DIM >());
    this->space_list_dual_[l] = VectorSpacePtr<DataType, DIM>(new VectorSpace< DataType, DIM >());

    this->block_manager_list_primal_[l] = BlockManagerPtr(new BlockManager());
    this->block_manager_list_dual_[l] = BlockManagerPtr(new BlockManager());
...
{  
  ...
  MeshPtr old_mesh = this->get_mesh_by_index(c_mesh_index);
  assert(old_mesh != 0);
  if (this->mesh_impl_ == mesh::IMPL_P4EST) 
  {
    boost::intrusive_ptr< MeshPXest > mesh_pXest =
        boost::static_pointer_cast< MeshPXest >(old_mesh);
  ...
  MeshPtr new_mesh = old_mesh->refine(markers);
  this->problem_->adapt_boundary(new_mesh);

  if (this->mesh_impl_ == IMPL_P4EST) 
  {
    this->mesh_list_[c_mesh_index] = compute_ghost_cells(*new_mesh, 
                                                         this->parcom_->comm(), 
                                                         shared_verts, 
                                                         mesh::IMPL_P4EST, 2);
  } 
  ...
  VectorSpacePtr<DataType, DIM> old_space_primal = this->space_list_primal_[0];
  VectorSpacePtr<DataType, DIM> old_space_dual = this->space_list_dual_[0];
  
  BlockManagerPtr old_block_manager_primal = this->block_manager_list_primal_[0];
  BlockManagerPtr old_block_manager_dual = this->block_manager_list_dual_[0];
  
  // 3. Setup FE space for new mesh
  VectorSpacePtr<DataType, DIM> new_space_primal 
    = VectorSpacePtr<DataType, DIM> (new VectorSpace<DataType, DIM> ());
  VectorSpacePtr<DataType, DIM> new_space_dual 
    = VectorSpacePtr<DataType, DIM> (new VectorSpace<DataType, DIM> ());
    
  BlockManagerPtr new_block_manager_primal = BlockManagerPtr(new BlockManager());
  BlockManagerPtr new_block_manager_dual = BlockManagerPtr(new BlockManager());
  ...
  fe_interpolator.init(old_space_primal.get(), new_space_primal.get(), *p_vectors[0], *this->vec_primal_primal_[0]);
