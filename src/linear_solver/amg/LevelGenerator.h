 ...
 */
template < class MatrixArg, template < class > class CoarseningArg,
           template < class > class GridTransferArg,
           template < class > class CoarseMatrixConstructionArg >
class LevelGenerator {
public:
  // Due to the fact that the local and coupled matrices are not copyable or
  // movable, non-const matrices must be handled as SharedPtr, whereas const
  // matrices will be handled as const reference to avoid SharedPtr<const
  // MatrixType>.
  typedef MatrixArg MatrixType;
  typedef hiflow::shared_ptr< MatrixType > PtrMatrixType;
  typedef LevelGenerator< MatrixType, CoarseningArg, GridTransferArg,
                          CoarseMatrixConstructionArg >
      Self;
  ...
  struct ResultType {
    ...
    hiflow::shared_ptr< typename GridTransferType::Output >
        ptr_grid_transfer_output;
    hiflow::shared_ptr< typename CoarseMatrixConstructionType::Output >
        ptr_coarse_matrix_construction_output;
  };
  ...
  typedef typename CoarseMatrixConstructionType::Settings
      CoarseMatrixConstructionSettings;

  LevelGenerator(
      CoarseningSettings const &coarsening_settings = CoarseningSettings(),
      GridTransferSettings const &grid_transfer_settings =
          GridTransferSettings(),
      CoarseMatrixConstructionSettings const
          &coarse_matrix_construction_settings =
              CoarseMatrixConstructionSettings())
      : coarsening_settings_(coarsening_settings),
        grid_transfer_settings_(grid_transfer_settings),
        coarse_matrix_construction_settings_(
            coarse_matrix_construction_settings) {}

  ResultType operator()(MatrixType const &Af) const {
    ResultType result;

    typename CoarseningType::ConnectionType connection =
        CoarseningType(this->coarsening_settings_)(result, Af);
    GridTransferType(this->grid_transfer_settings_)(result, connection, Af);
    CoarseMatrixConstructionType(this->coarse_matrix_construction_settings_)(
        result, Af);
