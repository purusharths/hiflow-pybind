...
void PreconditionerVanka< LAD, DIM >::InitParameter(
    const hiflow::VectorSpace< DataType, DIM > &space, const DataType damping_param,
    const int num_iter, const bool use_block_of_cells,
...
void PreconditionerVanka< LAD, DIM >::InitIluppPrecond(
    int prepro_type, int precond_no, int max_levels, DataType mem_factor,
    DataType threshold, DataType min_pivot) {
  this->precond_.InitParameter(prepro_type, precond_no, max_levels, mem_factor,
                               threshold, min_pivot);
  this->use_preconditioner_ = true;
}

template < class LAD, int DIM > void PreconditionerVanka< LAD, DIM >::Factorize() {
...
LinearSolverState PreconditionerVanka< LAD, DIM >::SolveImpl(const VectorType &b,
  ...
  for (int k = 0, k_e = this->maxits_; k != k_e; ++k) {
    ...
    for (size_t i = 0, i_e = sorted_dofs_diag_.size(); i != i_e; ++i) {
      ...
      res_loc.resize(sorted_dofs_diag_[i].size(), 0.);
      res_loc_2.resize(sorted_dofs_diag_[i].size(), 0.);
      x_temp_loc.resize(sorted_dofs_diag_[i].size(), 0.);

      this->op_->VectorMult_submatrix_vanka(vec2ptr(sorted_dofs_diag_[i]),
                                            sorted_dofs_diag_[i].size(), *x,
                                            vec2ptr(res_loc));

      x->GetValues(vec2ptr(sorted_dofs_diag_[i]), sorted_dofs_diag_[i].size(),
                   vec2ptr(x_temp_loc));
      ...
      if (!this->use_block_of_cells_) {
        ...
        local_mat_diag_block_mode_.Resize(sorted_dofs_diag_[i].size(),
                                          sorted_dofs_diag_[i].size());

        // Get local matrix from global operator
        this->op_->GetValues(
            vec2ptr(sorted_dofs_diag_[i]), sorted_dofs_diag_[i].size(),
            vec2ptr(sorted_dofs_diag_[i]), sorted_dofs_diag_[i].size(),
      ...
      // Get local rhs
      b_loc.resize(sorted_dofs_diag_[i].size(), 0.);
      b.GetValues(vec2ptr(sorted_dofs_diag_[i]), sorted_dofs_diag_[i].size(),
                  vec2ptr(b_loc));
      ...
      // Solve local problem
      x_loc.resize(sorted_dofs_diag_[i].size(), 0.);
      if (!this->use_block_of_cells_) {
      ...
      for (size_t j = 0, j_e = x_loc.size(); j != j_e; ++j) {
        x_temp_loc[j] = (1. - this->damping_param_) * x_temp_loc[j] +
                        this->damping_param_ * x_loc[j];
      }

      x->SetValues(vec2ptr(sorted_dofs_diag_[i]), sorted_dofs_diag_[i].size(),
                   vec2ptr(x_temp_loc));
    }
...
template < class LAD, int DIM > 
void PreconditionerVanka< LAD, DIM >::CreateLocalMatrices() {
  ...
  for (hiflow::mesh::EntityIterator it = mesh->begin(mesh->tdim()),
    ...
    for (int fe_ind = 0, var_e = space_->nb_fe(); fe_ind != var_e; ++fe_ind) {
      ...
      sorted_dofs_diag_[it->index()].insert(
          sorted_dofs_diag_[it->index()].end(), var_indices.begin(),
          var_indices.end());
    }

    // get dofs of all adjacent cells in block-mode
    if (this->use_block_of_cells_) {
      for (hiflow::mesh::IncidentEntityIterator
               it_inc = it->begin_incident(mesh->tdim()),
    ...
    sorted_dofs_diag_[it->index()].erase(
        unique(sorted_dofs_diag_[it->index()].begin(),
               sorted_dofs_diag_[it->index()].end()),
        sorted_dofs_diag_[it->index()].end());
    ...
    while (found) {
      ...
      for (int i = 0, i_e = sorted_dofs_diag_[it->index()].size();
           i != i_e && !found; ++i) {
        if (!this->space_->dof().is_dof_on_subdom(
    ...
    for (int i = 0, i_e = sorted_dofs_diag_.size(); i != i_e && !found; ++i) {
      if (sorted_dofs_diag_[i].size() == 0) {
        if (!this->use_block_of_cells_) {
          local_mat_diag_[i]->Clear();
          delete local_mat_diag_[i];
          local_mat_diag_.erase(local_mat_diag_.begin() + i);
        }
  ...
  if (!this->use_block_of_cells_) {
    for (int i = 0, i_e = local_mat_diag_.size(); i != i_e; ++i) {
      // Resize local matrix to correct size
      local_mat_diag_[i]->Resize(sorted_dofs_diag_[i].size(),
                                 sorted_dofs_diag_[i].size());
