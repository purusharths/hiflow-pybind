...
template < class LAD >
DofIdConverter< LAD >::DofIdConverter(const Element< DataType, DIM > *element)
    : element_(element) {
  assert(element);
  ...
  l2ijk_map_.resize(element->get_num_variables(), std::vector< ijk_dof_id >());
...
template < class LAD >
void NearestKnownDofsFinder< LAD >::find_nearest_known_dofs(
...
template < class LAD >
int NearestKnownDofsFinder< LAD >::find_nearest_known_dof_on_subent(
  ...
  if (tdim < element->get_cell().tdim()) {
    element->space().dof().get_coord_on_subentity(
        var, element->get_cell_index(), tdim, sindex, coordinates);

    element->space().dof().get_dofs_on_subentity(var, element->get_cell_index(),
                                                 tdim, sindex, dofs);
  } else {
    element->get_dof_indices(var, dofs);
    element->space().dof().get_coord_on_cell(var, element->get_cell_index(),
                                             coordinates);
  }
...
template < class LAD >
void NearestKnownDofsFinder< LAD >::q_proximity_search(
...
template < class LAD >
void LinearCellInterpolator< LAD >::
operator()(const Element< DataType, DIM > &element) {
  ...
  for (int var = 0; var < element.get_num_variables(); ++var) {
    ...
    if (interpolate_from_interpolated_) {
      ...
      for (int local_dof_id = 0; local_dof_id < element.get_num_dofs(var);
           ++local_dof_id) {
        int global_id = -1;
    ...
    for (int dim = 0; dim < cell_dimension; ++dim) {
      ...
      for (IncidentEntityIterator ent = element.get_cell().begin_incident(dim);
        ...
        // obtain dofs
        dof_partition.get_dofs_on_subentity(var, element.get_cell_index(), dim,
                                            sindex, dofs);
...
template < class LAD >
void LinearCellInterpolator< LAD >::interpolate_entity(
    int var, const Element< DataType, DIM > &element, const Entity &ent,
    const std::vector< int > &dofs, const DofIdConverter< LAD > &dof_converter,
  ...
  for (std::size_t i = 0; i < dofs.size(); ++i) {
    ...
    if (dof_partition.is_dof_on_sd(dofs[i]) && !is_dof_known(dofs[i])) {
      ...
      if (ent.tdim() < element.get_cell().tdim()) {
        ...
        for (IncidentEntityIterator cell = ent.begin_incident(cell_dimension);
          ...
          Element< DataType, DIM > neighboring_cell(element.space(), cell->index());

          DofIdConverter< LAD > neighboring_converter(&neighboring_cell);

          OverlayBasedIsDofKnownLookup< LAD > neighboring_lookup(
              dof_ident_, &neighboring_converter);

          neighboring_lookup.use_overlay(interpolate_from_interpolated_);

          OverlayBasedIsDofKnownLookup< LAD > *used_lookup =
              &neighboring_lookup;
          ...
          NearestKnownDofsFinder< LAD > neighboring_finder(
