...
public:
  ...
  DataTransferInformation(const BasicLevel< LAD, DIM > *coarse_level,
                          const BasicLevel< LAD, DIM > *fine_level)
    : num_coarse_procs_(0), num_fine_procs_(0), fine_rank_(-1),
      coarse_rank_(-1), coarse_procs_ranks_(), coarse_procs_list_(),
      fine_procs_list_(), fine_level_(fine_level), coarse_level_(coarse_level) {
  ...
  int num_coarse_procs(void) const { return num_coarse_procs_; }

  /// @return How many processes exist in the communicator of the fine grid.

  int num_fine_procs(void) const { return num_fine_procs_; }

  /// @return The rank of the calling process in the communicator of the
  /// fine grid

  int fine_rank(void) const { return fine_rank_; }

  /// @return The rank of the calling process in the communicator of the
  /// coarse grid

  int coarse_rank(void) const { return coarse_rank_; }

  /// Returns a vector of the ranks in the fine communicator of the coarse
  /// processes (only working if the calling process is at least in the fine
  /// grid)
  ...
  /// Returns a vector of the ranks in the fine communicator of the fine
  /// processes (only working if the calling process is at least in the fine
  /// grid)
  ...
  const BasicLevel< LAD, DIM > *fine_level(void) const { return fine_level_; }

  /// @return A pointer to the coarse grid that has been used to
  /// construct the DataTransferInformation object

  const BasicLevel< LAD, DIM > *coarse_level(void) const { return coarse_level_; }
...
private:
  ...
  void fetch_coarse_process_information(void) {
    if (fine_level_->is_scheduled_to_this_process()) {
      ...
      MPI_Allgather(&transmitted_rank, 1, MPI_INT,
                    util::raw_array(coarse_procs_ranks_), 1, MPI_INT,
                    fine_level_->comm());
  ...
  const BasicLevel< LAD, DIM > *fine_level_;
  const BasicLevel< LAD, DIM > *coarse_level_;
};
...
public:
  ...
  AsymmetricDataTransfer(MPI_Comm comm,
    ...
    : comm_(comm), rank_(-1), comm_size_(0), process_is_sender_(false),
      process_is_receiver_(false), process_is_participating_(false),
      sending_processes_(source_process_list),
      receiving_processes_(dest_process_list), sizes_are_known_(false),
      transfer_sizes_() 
  {
    if (comm != MPI_COMM_NULL) {
      ...
      std::vector< int >::iterator it =
          std::find(sending_processes_.begin(), sending_processes_.end(), rank_);

      process_is_sender_ = (it != sending_processes_.end());

      it = std::find(receiving_processes_.begin(), receiving_processes_.end(),
                     rank_);
  ...
  /// performance advantage to pass the existing data as argument to this
  /// function.
  ...
  template < typename T >
  void transfer_data(const std::vector< T > &in, std::vector< T > &out) const {
    if (process_is_participating_) {
      std::vector< MPI_Request > send_requests(receiving_processes_.size(),
                                               MPI_Request());
      ...
      if (process_is_sender_) {
        ...
        for (unsigned i = 0; i < receiving_processes_.size(); ++i) {
          ...
          //           MPI_Isend(reinterpret_cast<void*> (const_cast<T*>
          //           (&(in[dest_process]))),
          //                     1, mpi_data_type<T>::get_type(),
          //                     dest_process, 0, comm_,
          //                     &(send_requests[i]));
          MPI_Isend(reinterpret_cast< void * >(const_cast< T * >(&(in[i]))), 1,
                    mpi_data_type< T >::get_type(), dest_process, 0, comm_,
                    &(send_requests[i]));
        }
      }
      ...
      if (process_is_receiver_) {
        ...
        std::vector< MPI_Request > recv_requests(sending_processes_.size(),
                                                 MPI_Request());
        ...
        for (unsigned i = 0; i < sending_processes_.size(); ++i) {
          ...
          MPI_Irecv(&(out[i]), 1, mpi_data_type< T >::get_type(),
                    source_process, 0, comm_, &(recv_requests[i]));
        }

        std::vector< MPI_Status > recv_statuses(recv_requests.size(),
                                                MPI_Status());

        MPI_Waitall(recv_requests.size(), util::raw_array(recv_requests),
      ...
      if (process_is_sender_) {
        ...
        MPI_Waitall(send_requests.size(), util::raw_array(send_requests),
  ...
  void perform_vector_transfer(const std::vector< std::vector< T > > &in,
    ...
    if (process_is_participating_) {
      ...
      if (process_is_sender_) {
        ...
        for (unsigned i = 0; i < receiving_processes_.size(); ++i) {
          ...
          //           if (in[i].size() != 0)
          {
            ...
            /*
                        MPI_Isend(reinterpret_cast<void*> (const_cast<T*>
               (util::raw_array(in[dest_process]))), in[dest_process].size(),
                                  mpi_data_type<T>::get_type(),
                                  dest_process, 0, comm_,
                                  &(send_requests.back()));*/
            MPI_Isend(reinterpret_cast< void * >(
                          const_cast< T * >(util::raw_array(in[i]))),
                      in[i].size(), mpi_data_type< T >::get_type(),
                      dest_process, 0, comm_, &(send_requests.back()));
          }
        }
      }
      ...
      if (process_is_receiver_) {
        ...
        //                 comm_size_,
        //                 std::vector<T>());
        out = std::vector< std::vector< T > >(sending_processes_.size(),
                                              std::vector< T >());
        ...
        //                       transfer_sizes_[source_process],
        //                       mpi_data_type<T>::get_type(), source_process,
        //                       0, comm_,
        //                       &(recv_requests.back()));
        //           }
        //         }
        ...
        for (unsigned i = 0; i < sending_processes_.size(); ++i) {
          ...
          if (transfer_sizes_[i] > 0) {
            ...
            MPI_Irecv(util::raw_array(out[i]), transfer_sizes_[i],
                      mpi_data_type< T >::get_type(), source_process, 0, comm_,
                      &(recv_requests.back()));
          }
        }
        ...
        MPI_Waitall(rcv_ctr, util::raw_array(recv_requests),
                    util::raw_array(recv_statuses));
      }

      if (process_is_sender_) {
        std::vector< MPI_Status > send_statuses(snd_ctr, MPI_Status());

        MPI_Waitall(snd_ctr, util::raw_array(send_requests),
                    util::raw_array(send_statuses));
      }
    }
    if (!process_is_receiver_) {
...
public:
  ...
  explicit DataTransferFineToCoarse(const DataTransferInformation< LAD, DIM > &info)
...
public:
  ...
  explicit DataTransferCoarseToFine(const DataTransferInformation< LAD, DIM > &info)
      : AsymmetricDataTransfer(info.fine_level()->comm(),
                               info.coarse_procs_list(),
                               info.fine_procs_list()) {}
};
