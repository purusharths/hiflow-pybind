...
public:
  ...
  IMPORT_FROM_BASECLASS(BasicHierarchy< ConnectedLevelType >,
                        IteratorFromFinest);
  IMPORT_FROM_BASECLASS(BasicHierarchy< ConnectedLevelType >,
  ...
  ConnectedHierarchy(
    ...
    // connections.
    std::vector< boost::shared_ptr< ConnectionType > > connections(
        levels + 1, boost::shared_ptr< ConnectionType >());
    ...
    if (levels > 1) {
      ...
      for (; coarser_grid != this->end_at_coarsest_level();
        ...
        connections[position] = boost::shared_ptr< ConnectionType >(
    ...
    for (IteratorFromCoarsest level_iterator =
             this->begin_from_coarsest_level();
...
struct HierarchyTypes {
  typedef ConnectedHierarchy< GMGLevel< LAD, DIM, GMGConnection< LAD, DIM > > > GMGHierarchy;
};
