...
#else
#define IMPORT_FROM_BASECLASS(baseclass, type) using typename baseclass::type
#endif

#define TYPE_FROM_CLASS(class, type) typedef typename class ::type type
...
template < typename T > inline T *raw_array(std::vector< T > &v) {
...
template < typename T > inline const T *raw_array(const std::vector< T > &v) {
#if __cplusplus >= 201103L
  return v.data();
#else
  return hiflow::vec2ptr(v);
...
public:
  ...
  /// E.g, to construct a 2x3 array, \c sizes has to contain the elements {2,
  /// 3}.
  ...
  multi_array(size_type size_x, size_type size_y) : data_(NULL) {
    sizes_.reserve(2);
  ...
  size_type get_dimension() const { return sizes_.size(); }
  ...
  iterator end() { return data_ + buffer_size_; }

  /// @return An iterator type pointing to one element beyond the
  /// last element of the array

  const_iterator end() const { return data_ + buffer_size_; }

  /// @return The total number of elements in the array

  size_type get_num_elements() const { return buffer_size_; }

  /// @return The total number of elements in the array

  size_type size() const { return get_num_elements(); }
  ...
  template < size_type N > T &operator[](const index_type (&position)[N]) {
