...
public:
  ...
  IjkIterator *get(int var) const {
    ...
    const RefElement< DataType, DIM > *fetype = element_->get_fe_for_var(var);
...
public:
  ...
  explicit DofIdConverter(const Element< DataType, DIM > *element) {
    ...
    g2lmap_ = UMAP_NAMESPACE::unordered_map< int, int >(1.2 * total_num_dofs);
  ...
  void rebind_to_element(const Element< DataType, DIM > *element) {
    ...
    for (int var = 0; var < this->element_->nb_fe(); ++var)
      if (element->get_fe(var)->get_my_deg() !=
          element_->get_fe(var)->get_my_deg())
...
private:
  ...
  void init_g2l_map() {
    ...
    for (int var = 0; var < element_->aaa_get_num_variables(); ++var) {
      for (int local_dof = 0; local_dof < element_->aaa_get_num_dofs(var);
           ++local_dof) {

        int global_dof =
            element_->space().dof().aaa_mapl2g(var, cell_index, local_dof);
...
int NearestKnownDofsFinder< LAD, DIM >::find_nearest_known_dof_on_subent(
  ...
  if (tdim < element->get_cell().tdim()) {
    element->space().dof().aaa_get_coord_on_subentity(
        var, element->get_cell_index(), tdim, sindex, coordinates);

    element->space().dof().aaa_get_dofs_on_subentity(var, element->get_cell_index(),
                                                 tdim, sindex, dofs);
  } else {
