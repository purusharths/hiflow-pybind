...
/// Implements a hierarchy for smoothers. Memory mangement of the stored
/// smoothers is the responsibility of the user.
...
public:
  ...
  IMPORT_FROM_BASECLASS(AbstractHierarchy< SmootherType >,
                        IteratorFromCoarsest);
...
/// A functor to be used in conjunction with
/// GeometricMultriGrid::for_each_level() that sets up Jacobi smoothers on each
/// level. This class also deals with memory management of the smoothers. It is
/// therefore important that this object lives at least as long as the
/// GeometricMultiGrid solver.

template < class LAD > class SetupJacobiSmoother {
public:
  ...
  SetupJacobiSmoother()
      : solve_mode_(3), global_iter_(3), inner_iter_(1), w_(0.5),
        async_(false) {}

  SetupJacobiSmoother(const int solve_mode, const int global_iter,
                      const int inner_iter, const DataType w, const bool async)
      : solve_mode_(solve_mode), global_iter_(global_iter),
        inner_iter_(inner_iter), w_(w), async_(async) {}

  void set_parameters(const int solve_mode, const int global_iter,
                      const int inner_iter, const DataType w, const bool async) 
  {
    assert(solve_mode > 1);
    assert(solve_mode < 4);
...
public:
  ...
  SetupAsyncIterGPUSmoother()
      : use_gpus_(1), procs_per_node_(1), solve_mode_(3), global_iter_(3),
        block_iter_(1), inner_iter_(5), w_(0.5), cuda_block_size_(128) {}

  SetupAsyncIterGPUSmoother(const int use_gpus, const int procs_per_node,
                            const int solve_mode, const int global_iter,
                            const int block_iter, const int inner_iter,
                            const DataType w, const int cuda_block_size)
      : use_gpus_(use_gpus), procs_per_node_(procs_per_node),
        solve_mode_(solve_mode), global_iter_(global_iter),
        block_iter_(block_iter), inner_iter_(inner_iter), w_(w),
        cuda_block_size_(cuda_block_size) {}

  void set_parameters(const int use_gpus, const int procs_per_node,
                      const int solve_mode, const int global_iter,
                      const int block_iter, const int inner_iter,
                      const DataType w, const int cuda_block_size) 
  {
  ...
  void operator()(LevelType &lvl, LinearSolver< LAD > *&smoother) {
    ...
    new_smoother->Prepare(*lvl.matrix(), *lvl.rhs(), *lvl.sol(), false,
                          use_gpus_, procs_per_node_);
...
public:
  ...
  SetupIndividualSmoother(const std::vector< int > &smoother_types)
      : lvl_idx_(0), smoother_types_(smoother_types), jacobi_(), async_gpu_() {}

  void SetJacobiParameters(const int solve_mode, const int global_iter,
                           const int inner_iter, const DataType w,
                           const bool async) {
    jacobi_.set_parameters(solve_mode, global_iter, inner_iter, w, async);
  }

  void SetAsyncIterGPUParameters(const int use_gpus, const int procs_per_node,
                                 const int solve_mode, const int global_iter,
                                 const int block_iter, const int inner_iter,
                                 const DataType w, const int cuda_block_size) {
    async_gpu_.set_parameters(use_gpus, procs_per_node, solve_mode, global_iter,
                            block_iter, inner_iter, w, cuda_block_size);
  }
...
public:
  ...
  //TYPE_FROM_CLASS(gmg::HierarchyTypes< LAD, DIM >, GMGHierarchy);  
  typedef typename gmg::HierarchyTypes< LAD, DIM >::GMGHierarchy GMGHierarchy ;
  ...
  GeometricMultiGrid(
      unsigned master_rank, unsigned levels, const MeshPtr &master_mesh,
      const std::vector< int > &fe_degrees, const gmg::Settings &settings,
      gmg::communicator_hierarchy_generators::BasicGenerator *comm_generator)

      : LinearSolver< LAD >(),
        hierarchy_(new GMGHierarchy(master_rank, levels, master_mesh,
  ...
  void SetSolveMode(const int solve_mode) {
    ...
    if ((solve_mode_ != 0) && (solve_mode_ != 1)) {
      LOG_ERROR(
          "GeometricMultiGrid::SetSolveMode called with invalid solve mode "
  ...
  void use_sleep(const bool use) { use_sleep_ = use; }
  ...
  void
  build_restriction_matrix(const bool use_transposed_of_interpolation_matrix) {
    for (typename GMGHierarchy::IteratorFromFinest lvl =
  ...
  void
  build_interpolation_matrix(const bool use_transposed_of_restriction_matrix) {
    for (typename GMGHierarchy::IteratorFromFinest lvl =
             hierarchy_->begin_from_finest_level();
         lvl != hierarchy_->end_at_coarsest_level() - 1; ++lvl) {
  ...
  /// Assembles all vectors and matrices neccessary to perform the multigrid
  /// algorithms (In more detail: Assembles the right hand side on the finest
  /// level and the matrices of all levels of the hierarchy)
  /// @param vector_assembler The vector assembly function
  /// @param matrix_assembler The matrix assembly funcion.

  void
  assemble_system(typename GlobalAssembler< DataType, DIM >::VectorAssemblyFunction
                      vector_assembler,
                  typename GlobalAssembler< DataType, DIM >::MatrixAssemblyFunction
                      matrix_assembler) {
    hierarchy_->begin_from_finest_level()->assemble_system(vector_assembler,
                                                           matrix_assembler);

    for (typename GMGHierarchy::IteratorFromFinest lvl =
             hierarchy_->begin_from_finest_level() + 1;
  ...
  VectorPtr sol() const { return hierarchy_->begin_from_finest_level()->sol(); }

  /// @return A pointer to the right hand side vector on the finest grid

  VectorPtr rhs() const { return hierarchy_->begin_from_finest_level()->rhs(); }
  ...
  int get_total_coarse_grid_iter() const { return total_coarse_grid_iter_; }

  enum CycleType { V_CYCLE, W_CYCLE };

  /// Set the mode of operation
  /// @param use_full_multigrid_start_cycle whether to start with a full
  /// multigrid iteration (default: no)
  /// @param type The cycle type that shall be used (V- or W-cycles. default:
  /// V-cycles)

  void set_mode(bool use_full_multigrid_start_cycle, CycleType type) {
    use_start_cycle_ = use_full_multigrid_start_cycle;
    cycle_type_ = type;
    LOG_INFO("GeometricMultiGrid",
             "set mode: full MG = "
                 << (use_full_multigrid_start_cycle ? "yes" : "NO")
                 << ", cycle type = " << (cycle_type_ == V_CYCLE ? "V" : "W"));
  }

  /// Setup Operator.
  /// @param op The matrix on which the solver operates. \c op must be the
  /// matrix return by \c matrix()
  ...
  virtual LinearSolverState SolveNormal() {
    ...
    LOG_INFO("GeometricMultiGrid",
             "starting with residual norm " << this->res_);
    for (this->iter_ = 0; this->control().Check(this->iter_, this->res_) ==
                          IterateControl::kIterate;
         ++(this->iter_)) {
      ...
      this->res_ = calculate_finest_level_residual_norm();
      LOG_INFO("GeometricMultiGrid", "cycle " << this->iter_ + 1
                                              << " ended with residual norm "
                                              << this->res_);
    }
  ...
  void set_dirichlet_bc(DirichletEvaluator &eval) {
    hierarchy_->begin_from_finest_level()->init_boundary_conditions(eval,
                                                                    false);
  ...
  /// @param solver The linear solver that will be used to solve the coarsest
  /// level.

  void setup_solver(LinearSolver< LAD > *solver) { solver_ = solver; }
  ...
  /// Perform a V cycle.
  /// @param stop_level An iterator to the level that represents the coarsest
  /// level, i.e. the level at which the V cycle is supposed to stop descending
  /// through the hierarchy.
  /// @return The linear solver state

  LinearSolverState
  v_cycle(typename GMGHierarchy::IteratorFromFinest stop_level) {
    assert(solver_);

    return cycle_process_level(
        hierarchy_->begin_from_finest_level(),
        hierarchy_->begin_from_finest_level(), stop_level,
        smoother_hierarchy_.begin_from_finest_level(), 1);
  }
  ...
  /// Perform a W cycle
  /// @param stop_level An iterator to the level that represents the coarsest
  /// level, i.e. the level at which the W cycle is supposed to stop descending
  /// through the hierarchy.
  /// @return The linear solver state

  LinearSolverState
  w_cycle(typename GMGHierarchy::IteratorFromFinest stop_level) {
    assert(solver_);

    return cycle_process_level(
        hierarchy_->begin_from_finest_level(),
        hierarchy_->begin_from_finest_level(), stop_level,
        smoother_hierarchy_.begin_from_finest_level(), 2);
  }
  ...
  LinearSolverState full_multigrid() {
    typename GMGHierarchy::IteratorFromCoarsest lvl =
        hierarchy_->begin_from_coarsest_level();
    typename GMGHierarchy::IteratorFromFinest lvl_reverse =
        hierarchy_->end_at_coarsest_level() - 1;
    ...
    for (; lvl != hierarchy_->end_at_finest_level(); ++lvl) {
      ...
      if (lvl != hierarchy_->begin_from_coarsest_level()) {
        if (lvl->is_scheduled_to_this_process()) {
          lvl->get_connection_to_next_coarser_grid()
              ->transfer_coarse_sol_to_fine_sol();
          linear_interpolation_(*lvl, *lvl->sol());
        }
      }

      // V cycle starting from each level
      cycle_process_level(lvl_reverse, lvl_reverse,
                          hierarchy_->end_at_coarsest_level() - 1, smoother, 1);
  ...
  /// to a pointer to a smoother of type LinearSolver<LAD> as arguments. This
  /// provides for example the means to conveniently setup all smoothers of the
  /// smoother hierarchy if they require access to the matrices of their
  /// respective level.
  /// @param f The function that will be executed on each level. It will need to
  /// match the signature \c f(GeometricMultigrid<LAD>::LevelType&,
  /// LinearSolver<LAD>*&). The \c LinearSolver will be the smoother for the \c
  /// LevelType.

  template < class Function > void for_each_level(Function &f) {
    typename GMGHierarchy::IteratorFromFinest lvl_it =
        hierarchy_->begin_from_finest_level();
    typename gmg::SmootherHierarchy< LAD >::IteratorFromFinest smoother_it =
        smoother_hierarchy_.begin_from_finest_level();

    for (; lvl_it != hierarchy_->end_at_coarsest_level();
         ++lvl_it, ++smoother_it) {
  ...
  template < class Function > void for_each_but_coarsest_level(Function &f) {
    typename GMGHierarchy::IteratorFromFinest lvl_it =
        hierarchy_->begin_from_finest_level();
    typename gmg::SmootherHierarchy< LAD >::IteratorFromFinest smoother_it =
        smoother_hierarchy_.begin_from_finest_level();

    for (; lvl_it != hierarchy_->end_at_coarsest_level() - 1;
         ++lvl_it, ++smoother_it) {
  ...
  virtual LinearSolverState Solve() { return this->SolveImpl(); }

protected:
  /// Solve the system. Requires a previous call to \c setup_solvers(), and, for
  /// any meaningful calculation, a call to \c assemble_system() and \c
  /// set_dirichlet_bc()
  /// @param b The right hand side vector. Must be the vector returned by \c
  /// rhs()
  ...
  virtual LinearSolverState SolveImpl() {
    switch (solve_mode_) {
    case 0:
      return SolveNormal();
      break;
    ...
    default:
      LOG_ERROR("GeometricMultriGrid: unknown solve mode "
                << solve_mode_ << ", must be 0 [solver] or 1 [preconditioner]");
      exit(-1);
      break;
    }
...
private:
  ...
  inline void
  calculate_residual(typename GMGHierarchy::IteratorFromFinest lvl) {
    if (lvl->is_scheduled_to_this_process()) {
  ...
  inline void calculate_finest_level_residual() {
    typename GMGHierarchy::IteratorFromFinest finest_lvl =
        hierarchy_->begin_from_finest_level();
  ...
  /// @param stop_lvl The level at which the algorithm shall cease to descend
  /// through the hierarchy.
  /// @param smoother An iterator to the current smoother in the smoother
  /// hierarchy
  /// @param num_recursive_calls The number of recursive calls. 1 corresponds to
  /// a V cycle, 2 to a W cycle.
  LinearSolverState cycle_process_level(
      typename GMGHierarchy::IteratorFromFinest lvl,
  ...
  {
    //   gmg::util::master_ostream mcout(std::cout,0);
    //   unsigned level_idx = hierarchy_->get_levels() - 1 -
    //   std::distance(start_lvl, lvl);
    //
    ...
    if (lvl->is_scheduled_to_this_process()) {
      ...
      if (lvl != stop_lvl) {
        ...
        for (unsigned i = 0; i != num_iter; ++i) {
          ...
          lvl->get_connection_to_next_coarser_grid()
              ->transfer_fine_res_to_coarse_rhs();

          cycle_process_level(lvl + 1, start_lvl, stop_lvl, smoother + 1,
                              num_recursive_calls);
          ...
          lvl->get_connection_to_next_coarser_grid()
              ->transfer_coarse_sol_to_fine_res();
  ...
  gmg::LinearInterpolation< LAD, DIM > linear_interpolation_;
  gmg::LinearRestriction< LAD, DIM > linear_restriction_;
