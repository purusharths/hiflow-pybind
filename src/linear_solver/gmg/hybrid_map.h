...
public:
  ...
  dof_locality_index_space_divider(
      const doffem::DofPartition< DataType, DIM > *dof_partition)
  ...
  dof_locality_index_space_divider() : dof_partition_(NULL) {}
  ...
  bool is_scattered(int global_dof) const {
    ...
    return !dof_partition_->is_dof_on_subdom(global_dof);
  }
...
private:
  const doffem::DofPartition< DataType, DIM > *dof_partition_;
};
...
template < class IndexSpaceDivider > class hybrid_bitmap {
...
public:
  /// Initialize object
  /// @param expected_num_scattered_entries The expected number of
  /// non-contiguous entries
  /// @param num_contiguous_entries The number of contiguous entries
  /// @param space_divider A Index space divider object that specifies if an
  /// index is contiguous (i.e. will be stored in the bitmap) or scattered (i.e.
  ...
  hybrid_bitmap(std::size_t expected_num_scattered_entries,
    ...
    continous_entries_ = std::vector< bool >(num_contiguous_entries, false);
  }
  ...
  bool query_bitmap(int idx) const {
    if (!divider_.is_scattered(idx)) {
      ...
      return scattered_entries_.find(idx) != scattered_entries_.end();
  }
...
template < class DataType, int DIM >
class HybridDofBitmap
    : public hybrid_bitmap< dof_locality_index_space_divider< DataType, DIM > > {
public:
  /// Construct object
  /// @param num_ghosts The estimated number of ghost dofs on the local
  /// subdomain.
  /// @param dof_partition A pointer to a \c dof_partition object to look up
  /// dofs

  HybridDofBitmap(std::size_t num_ghosts,
                  const doffem::DofPartition< DataType, DIM > *dof_partition)
      : hybrid_bitmap< dof_locality_index_space_divider< DataType, DIM > >(
            num_ghosts,
            dof_partition->nb_dofs_on_subdom(dof_partition->my_subdom()),
            dof_locality_index_space_divider< DataType, DIM >(dof_partition)) {}
  ...
  HybridDofBitmap()
      : hybrid_bitmap< dof_locality_index_space_divider< DataType, DIM > >(
            0, 0, dof_locality_index_space_divider< DataType, DIM >()) {}
};
