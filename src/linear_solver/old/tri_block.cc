...
#include "linear_solver/tri_block.h"

namespace hiflow {
namespace la {

template < class LAD > TriBlock< LAD >::TriBlock() : SchurComplement< LAD >() {
  this->precond_ = nullptr;
  this->solver_E_ = nullptr;
  this->scaling_E_ = 0.0;
  ...
  this->precond_two_type_ = externalPrecond;
}

template < class LAD > TriBlock< LAD >::~TriBlock() { this->Clear(); }

template < class LAD > void TriBlock< LAD >::Clear() {
...
template < class LAD >
void TriBlock< LAD >::Init(const hiflow::VectorSpace< ValueType > &space,
  ...
  SchurComplement< LAD >::Init(space, block_one_variables, block_two_variables,
                               this->precond_two_type_);

  E_.Init(this->comm_, this->la_c_two_, this->la_c_two_);
}

template < class LAD >
void TriBlock< LAD >::SetupOperator(OperatorType &op, const int block_id) {
  if (this->precond_two_type_ == PCDPrecond && block_id == 3) {
    std::cout << "TriBlock: Don't SetupOperator D if you want to use "
                 "ApproximateSchur. "
              << std::endl;
    quit_program();
  }
  ...
  if (block_id == 0) {
    ...
    for (typename std::map< int, SortedArray< int > >::const_iterator
      ...
      int block_index_system =
          row_index_block + this->offsets_block_one_[this->my_rank_];
      this->A_.SetValues(&block_index_system, 1,
                         vec2ptr(this->sparsity_A_[row_index_block]),
                         it->second.size(), vec2ptr(vals));
      ++row_index_block;
    }
  ...
  if (block_id == 1) {
    ...
    for (typename std::map< int, SortedArray< int > >::const_iterator
      ...
      int block_index_system =
          row_index_block + this->offsets_block_one_[this->my_rank_];
      this->B_.SetValues(&block_index_system, 1,
  ...
  if (block_id == 2) {
    ...
    for (typename std::map< int, SortedArray< int > >::const_iterator
      ...
      int block_index_system =
          row_index_block + this->offsets_block_two_[this->my_rank_];
      this->C_.SetValues(&block_index_system, 1,
                         vec2ptr(this->sparsity_C_[row_index_block]),
                         it->second.size(), vec2ptr(vals));
      ++row_index_block;
    }
  }
  ...
  if (block_id == 3) {
    ...
    for (typename std::map< int, SortedArray< int > >::const_iterator
      ...
      int block_index_system =
          row_index_block + this->offsets_block_two_[this->my_rank_];
      this->D_.SetValues(&block_index_system, 1,
...
template < class LAD > void TriBlock< LAD >::SetupOperatorE(OperatorType &op) {
  if (!this->E_is_initialized_) {
    ...
    for (int i = 0; i < this->sparsity_D_.size(); ++i) {
      for (int j = 0; j < this->sparsity_D_[i].size(); ++j) {
        if ((this->sparsity_D_[i][j] >=
             this->offsets_block_two_[this->my_rank_]) &&
            (this->sparsity_D_[i][j] <
             this->offsets_block_two_[this->my_rank_ + 1])) {
    ...
    E_.InitStructure(vec2ptr(rows_diag), vec2ptr(cols_diag), cols_diag.size(),
  ...
  for (typename std::map< int, SortedArray< int > >::const_iterator
    ...
    int block_index_system =
        row_index_block + this->offsets_block_two_[this->my_rank_];
    this->E_.SetValues(&block_index_system, 1,
                       vec2ptr(this->sparsity_D_[row_index_block]),
                       it->second.size(), vec2ptr(vals));
    ++row_index_block;
  }
...
template < class LAD >
void TriBlock< LAD >::FilterSubVector(VectorType *y, VectorType *rhs_temp,
                                      std::vector< int > *indexset_two) {
  ...
  val_temp.clear();
  val_temp.resize(this->mapb2s_two_.size(), 0.);
  rhs_temp->GetValues(vec2ptr(this->mapb2s_two_), this->mapb2s_two_.size(),
                      vec2ptr(val_temp));
  y->SetValues(vec2ptr(*indexset_two), indexset_two->size(), vec2ptr(val_temp));
}

template < class LAD > void TriBlock< LAD >::FilterVector(VectorType *y) {
  this->non_lin_op_->ApplyFilter(*y);
}

template < class LAD >
void TriBlock< LAD >::BuildImpl(VectorType const *b, VectorType *x) {
...
template < class LAD >
LinearSolverState TriBlock< LAD >::SolveImpl(const VectorType &b,
                                             VectorType *x) {
  this->x_.Zeros();
  this->y_.Zeros();
  ...
  if (block_op_[1] == 0 && block_op_[2] > 0) {
    if (this->print_level_ > 1)
      LOG_INFO("[" << this->nested_level_
                   << "] Solve lower block triangular system                   "
                      "     ",
               1);
    ...
    if (block_op_[0] == 0) {
      // operator of solver A is full matrix -> give global rhs to solver A with
      // zeros in block two
      rhs_temp.Zeros();
      sol_temp.Zeros();
      val_temp.clear();
      ...
      b.GetValues(vec2ptr(this->mapb2s_one_), this->mapb2s_one_.size(),
                  vec2ptr(val_temp));
      rhs_temp.SetValues(vec2ptr(this->mapb2s_one_), this->mapb2s_one_.size(),
                         vec2ptr(val_temp));
      ...
      val_temp.clear();
      val_temp.resize(this->mapb2s_one_.size(), 0.);
      sol_temp.GetValues(vec2ptr(this->mapb2s_one_), this->mapb2s_one_.size(),
                         vec2ptr(val_temp));
      x->SetValues(vec2ptr(this->mapb2s_one_), this->mapb2s_one_.size(),
                   vec2ptr(val_temp));
      this->x_.SetValues(vec2ptr(indexset_one), indexset_one.size(),
                         vec2ptr(val_temp));

    } else {
      // operator of A is submatrix A    -> extract block one values and store
      // in vector f_
      val_temp.resize(this->mapb2s_one_.size(), 0.);
      b.GetValues(vec2ptr(this->mapb2s_one_), this->mapb2s_one_.size(),
                  vec2ptr(val_temp));
      this->f_.SetValues(vec2ptr(indexset_one), indexset_one.size(),
                         vec2ptr(val_temp));
      ...
      val_temp.clear();
      val_temp.resize(this->mapb2s_one_.size(), 0.);
      this->x_.GetValues(vec2ptr(indexset_one), indexset_one.size(),
                         vec2ptr(val_temp));
      x->SetValues(vec2ptr(this->mapb2s_one_), this->mapb2s_one_.size(),
                   vec2ptr(val_temp));
    }
    ...
    b.GetValues(vec2ptr(this->mapb2s_two_), this->mapb2s_two_.size(),
                vec2ptr(val_temp));
    this->g_.SetValues(vec2ptr(indexset_two), indexset_two.size(),
                       vec2ptr(val_temp));
    ...
    if (block_op_[3] == 0 && this->precond_two_type_ == externalPrecond) {

      // operator of solver D is full matrix -> give global rhs to solver D with
      // zeros in block one
      rhs_temp.Zeros();
      sol_temp.Zeros();
      val_temp.clear();
      ...
      val_temp.resize(this->mapb2s_two_.size(), 0.);
      this->g_.GetValues(vec2ptr(indexset_two), indexset_two.size(),
                         vec2ptr(val_temp));
      rhs_temp.SetValues(vec2ptr(this->mapb2s_two_), this->mapb2s_two_.size(),
                         vec2ptr(val_temp));
      ...
      val_temp.clear();
      val_temp.resize(this->mapb2s_two_.size(), 0.);
      this->y_.GetValues(vec2ptr(indexset_two), indexset_two.size(),
                         vec2ptr(val_temp));
      x->SetValues(vec2ptr(this->mapb2s_two_), this->mapb2s_two_.size(),
                   vec2ptr(val_temp));
    }
  ...
  if (block_op_[1] > 0 && block_op_[2] == 0) {
    if (this->print_level_ > 1)
      LOG_INFO("[" << this->nested_level_
                   << "] Solve upper block triangular system                   "
                      "     ",
               1);
    ...
    if (block_op_[3] == 0 && this->precond_two_type_ == externalPrecond) {
      ...
      x->SetValues(vec2ptr(this->mapb2s_two_), this->mapb2s_two_.size(),
                   vec2ptr(val_temp));
      this->y_.SetValues(vec2ptr(indexset_two), indexset_two.size(),
                         vec2ptr(val_temp));

    } else {
      // operator of D is submatrix D    -> extract block two values and store
      // in vector g_
      val_temp.resize(this->mapb2s_two_.size(), 0.);
      b.GetValues(vec2ptr(this->mapb2s_two_), this->mapb2s_two_.size(),
                  vec2ptr(val_temp));
    ...
    if (block_op_[0] == 0) {
      ...
      val_temp.resize(this->mapb2s_one_.size(), 0.);
      this->f_.GetValues(vec2ptr(indexset_one), indexset_one.size(),
                         vec2ptr(val_temp));
      rhs_temp.SetValues(vec2ptr(this->mapb2s_one_), this->mapb2s_one_.size(),
                         vec2ptr(val_temp));
  ...
  if (block_op_[1] > 0 && block_op_[2] > 0) {
    LOG_ERROR(
        "["
        << this->nested_level_
        << "] Matrix is neither block triangular nor block diagonal, idiot!!!");
    quit_program();
  }
...
template < class LAD >
void TriBlock< LAD >::SolveBlock(const VectorType &b, VectorType &x,
                                 const int block_id) {
...
template < class LAD >
void TriBlock< LAD >::ApplyPressureConvDiff(const VectorType &b,
                                            VectorType *x) {
  VectorType y, z;
  x->Zeros();
  ...
  if (this->scaling_E_ != 0.0 && this->solver_E_ != nullptr) {
    // y = E^{-1} b
    y.Zeros();
...
#endif
template class TriBlock< LADescriptorCoupledD >;
template class TriBlock< LADescriptorCoupledS >;
} // namespace la
} // namespace hiflow
