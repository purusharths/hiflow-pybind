...
#include "linear_solver/preconditioner_parallel_naive.h"
...
template < class LAD >
void PreconditionerParallelNaive< LAD >::InitParameter(int num_iter,
                                                       DataType omega) {
  assert(num_iter >= 1);
...
template < class LAD >
void PreconditionerParallelNaive< LAD >::SetPreconditioner(
    PreconditionerBlockJacobi< LAD > &precond_diag) {
  this->precond_diag_ = &precond_diag;
}

template < class LAD >
void PreconditionerParallelNaive< LAD >::BuildImpl(VectorType const *b,
                                                   VectorType *x) {
  assert(this->precond_diag_ != nullptr);
  assert(this->op_ != nullptr);

  this->precond_diag_->Build(b, x);
...
template < class LAD >
LinearSolverState
PreconditionerParallelNaive< LAD >::SolveImpl(const VectorType &b,
                                              VectorType *x) {
  // auxiliary vectors
...
template < class LAD > void PreconditionerParallelNaive< LAD >::Clear() {
