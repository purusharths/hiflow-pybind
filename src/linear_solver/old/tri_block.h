...
/// The user has to specify matrix operators from which each submatrix should be
/// extracted and linear solvers for block A and block D. It is possible to
/// provide different operators for each submatrix. However, all matrix
/// operators must have the same coupling structure involving all occuring
/// variables, as it is the case for the Schur complement class.\n In order to
/// setup the solvers, the user can choose, to either
/// - provide a solver with already defined operator
/// - provide a solver with not defined operator. \n
/// In the later case, the defined solvers are initialized for the corresponding
/// submatrices and the user has to call SetupOperator(matrix_op, block_id) for
/// the corresponding subblock (0=A, 1=B, 2=C, 3=D). If no operator for block B
/// or C is set, they are assumed to be zero. If the user sets an operator for
/// block A or D, the solvers are initialized for the submatrices extracted from
/// these operators.
...
///
/// Finally, it is possible to obtain a solver \f$D^{-1} = eE^{-1}-sQ^{-1} Fp
/// H^{-1}\f$. To this end, the user needs to define scaling factors \f$e,s \f$,
/// solvers \f$E^{-1}, Q^{-1}, H^{-1}\f$ and the matrix \f$Fp\f$.\n \n
///
...
template < class LAD, int DIM > class TriBlock : public SchurComplement< LAD, DIM > {
public:
  ...
  virtual void SetupOperator(OperatorType &op, int block);

  /// Following functions of base class SchurComplement should not be used by
  /// TriBlock class
  ...
  virtual void SetupPreconditioner(Preconditioner< LAD > &precond) {
    std::cout << "TriBlock: Don't use SetupPreconditioner function "
              << std::endl;
    quit_program();
  }
  ...
  SetupSolverPrecondBlockTwo(LinearSolver< LAD > &solver_precond_block_two) {
    std::cout << "TriBlock: Don't use SetupSolverPrecondBlockTwo function "
              << std::endl;
    quit_program();
  }

  virtual void SetupBlockTwoMatrix(const OperatorType &op) {
    std::cout << "TriBlock: Don't use SetupBlockTwoMatrix function "
              << std::endl;
    quit_program();
  }

  virtual void SetBlockTwoMatrixScaling(ValueType scaling) {
    std::cout << "TriBlock: Don't use SetBlockTwoMatrixScaling function "
              << std::endl;
    quit_program();
  }
  ...
  virtual void SetPressConvDiffScaling(const ValueType scaling_E,
  ...
  virtual void UsePressConvDiff(bool flag) {
    if (flag) {
      this->precond_two_type_ = PCDPrecond;
    } else {
      this->precond_two_type_ = externalPrecond;
    }
    if (flag && block_op_[3] > 0) {
      std::cout << "TriBlock: Don't SetApproximateSchur if Operator D is "
                   "already defined. "
                << std::endl;
      quit_program();
    }
...
protected:
  ...
  virtual void SolveBlock(const VectorType &b, VectorType &x, int block);
  ...
  virtual void FilterSubVector(VectorType *y, VectorType *rhs_temp,
                               std::vector< int > *indexset_two);

  /// Apply the operator Sp^{-1} = scaling_E * E^{-1} - scaling_S * Q^{-1} * Fp
  ...
  /// matrix, H = pressure diffusion matrix \param[in] b right hand side vector
  /// \param[out] x solution vector
  void virtual ApplyPressureConvDiff(const VectorType &b, VectorType *x);

  /// Operator \f$E\f$ in approximate Schur inverse \f$D^{-1} = eE^{-1}-sQ^{-1}
  /// Fp H^{-1}\f$
  OperatorType E_;
