...
template < class LAD >
UmfpackSolver< LAD >::UmfpackSolver() : LinearSolver< LAD >() {
...
template < class LAD > UmfpackSolver< LAD >::~UmfpackSolver() {
  Clear();
  delete[] Control;
  delete[] Info;
}

template < class LAD > void UmfpackSolver< LAD >::InitParameter() {}
...
void UmfpackSolver< LAD >::SetupOperator(OperatorType &op) {
  ...
  this->op_ = &op;
  this->umf_n_ = this->op_->num_rows_local();
...
template < class LAD >
void UmfpackSolver< LAD >::BuildImpl(VectorType const *b, VectorType *x) {
...
template < class LAD > void UmfpackSolver< LAD >::FactorizeSymbolic() {
  assert(umf_n_ > 0);
  assert(Ap_ != nullptr);
  assert(Ai_ != nullptr);
  assert(Ax_ != nullptr);
  ...
  status = umfpack_di_symbolic(umf_n_, umf_n_, Ap_, Ai_, Ax_, &symbolic_,
                               Control, Info);

  umfpack_di_report_status(Control, status);

  assert(status == UMFPACK_OK);
  assert(symbolic_ != nullptr);

  is_symb_factorized_ = true;
}

template < class LAD > void UmfpackSolver< LAD >::FactorizeNumeric() {
  assert(umf_n_ > 0);
  assert(Ap_ != nullptr);
  assert(Ai_ != nullptr);
  assert(Ax_ != nullptr);
  ...
  status =
      umfpack_di_numeric(Ap_, Ai_, Ax_, symbolic_, &numeric_, Control, Info);

  umfpack_di_report_status(Control, status);

  assert(status == UMFPACK_OK);
  assert(numeric_ != nullptr);

  is_num_factorized_ = true;
}

template < class LAD >
LinearSolverState UmfpackSolver< LAD >::SolveFactorized(const VectorType &b,
                                                        VectorType *x) {
  assert(umf_n_ > 0);
  ...
  b.GetLocalValues(b_ptr);
  DataType *x_ptr = new DataType[umf_n_];
  for (int k = 0; k < umf_n_; ++k)
    x_ptr[k] = 0.0;

  status = umfpack_di_solve(UMFPACK_Aat, Ap_, Ai_, Ax_, x_ptr, b_ptr, numeric_,
                            Control, Info);
...
template < class LAD >
LinearSolverState
UmfpackSolver< LAD >::SolveFactorized(const std::vector< DataType > &b,
...
template < class LAD >
LinearSolverState UmfpackSolver< LAD >::SolveImpl(const VectorType &b,
                                                  VectorType *x) {
...
template < class LAD >
LinearSolverState
UmfpackSolver< LAD >::SolveImpl(const std::vector< DataType > &b,
                                std::vector< DataType > *x) {
  return this->SolveFactorized(b, x);
}

template < class LAD > void UmfpackSolver< LAD >::Clear() {
  if (Ap_b_)
    delete[] Ap_;
  if (Ai_b_)
    delete[] Ai_;
