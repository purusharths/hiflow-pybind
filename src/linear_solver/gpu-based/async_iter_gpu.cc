...
AsynchronousIterationGPU< LAD >::AsynchronousIterationGPU(const MPI_Comm &comm)
    : LinearSolver< LAD >(), A_(0), res_vec_(0), lmat_A_diag_(0),
      lmat_A_offdiag_(0), lvec_b_(0), lvec_x_(0), lvec_xg_(0), b_host(0),
      x_host(0), xg_host(0), block_res_host(0), A_diag_gpu(0), A_offdiag_gpu(0),
      b_gpu(0), x_gpu(0), xg_gpu(0), inv_D_gpu(0), D_gpu(0), block_res_gpu(0),
#ifdef WITH_CUDA
      stream_(0),
#endif
      n_gpus_(0), used_gpus_(0), enable_hyperq_(0), enable_p2p_(0),
      grid_dim_(0), block_dim_(128), n_global_iter_(0), n_block_iter_(0),
      n_inner_iter_(0), solve_mode_(0), max_threads_per_block_(0), w_(1.0),
      comm_(comm), rank_(-1), nproc_(-1), row_offs_(0), nnz_diag_(0),
      row_diag_(0), col_diag_(0), nnz_offdiag_(0), row_offdiag_(0),
      col_offdiag_(0), have_offdiag_(false), compute_residual_on_dev_(false) {
  ...
  if (n_gpus_ <= 0) {
    LOG_ERROR("AsynchronousIterationGPU rank "
              << rank_ << " couldn't detect any CUDA-capable device");
    exit(-1);
  }
  if (this->print_level_ > 2) {
    LOG_INFO("AsynchronousIterationGPU", " rank " << rank_ << " detected "
                                                  << n_gpus_
  ...
  for (int i = 0; i < n_gpus_; ++i) {
    ...
    max_threads_per_block_ =
        std::min(max_threads_per_block_, prop.maxThreadsPerBlock);
  ...
  if (enable_hyperq_ > 0) {
    if (this->print_level_ > 2) {
      LOG_INFO("AsynchronousIterationGPU", " may use Hyper-Q");
    }
  } else {
    if (this->print_level_ > 2) {
      LOG_INFO("AsynchronousIterationGPU", " Hyper-Q not available");
    }
  }
  if (this->print_level_ > 2) {
    LOG_INFO("AsynchronousIterationGPU", " CUDA kernels can use max. "
                                             << max_threads_per_block_
                                             << " threads per block");
  }
...
template < class LAD >
void AsynchronousIterationGPU< LAD >::Prepare(
    const MatrixType &A, const VectorType &b, VectorType &x,
    const bool compute_residual_on_dev, const int use_gpus,
  ...
  A_ = &A;
  lmat_A_diag_ =
      dynamic_cast< const CPU_CSR_lMatrix< DataType > * >(&A.diagonal());
  lmat_A_offdiag_ =
      dynamic_cast< const CPU_CSR_lMatrix< DataType > * >(&A.offdiagonal());
  ...
  if (lmat_A_offdiag_->get_nnz() > 0)
    have_offdiag_ = true;
  else
    have_offdiag_ = false;

  if (used_gpus_ > n_gpus_) {
    LOG_ERROR("Requested " << use_gpus << " devices but only " << n_gpus_
                           << " available.");
    exit(-1);
  }
  if (A.diagonal().get_matrix_format() != CSR) {
    LOG_ERROR("Called AsynchronousIterationGPU::Prepare with non-CSR matrix "
              "argument.");
    exit(-1);
  }
  ...
  if (have_offdiag_ &&
      (A.diagonal().get_num_row() != A.offdiagonal().get_num_row())) {
    LOG_ERROR("Number of rows in diagonal ("
              << A.diagonal().get_num_row() << ") and offdiagonal ("
              << A.offdiagonal().get_num_row()
              << ") part of matrix are not equal.")
    exit(-1);
  }

  if ((procs_per_node > used_gpus_) && (enable_hyperq_ == 0)) {
    LOG_ERROR("Requested for " << procs_per_node << " MPI processes to share "
                               << used_gpus_
                               << " GPUs, but Hyper-Q is not enabled.");
    exit(-1);
  }
  ...
  for (int i = 0; i < used_gpus_; ++i) {
    ...
    memcpy(tmpi, &(lmat_A_diag_->matrix.row[row_offs_[i]]),
           (nrows + 1) * sizeof(int));
    ...
    cudaMemcpy(row_diag_[i], tmpi, (nrows + 1) * sizeof(int),
               cudaMemcpyHostToDevice);

    if (have_offdiag_) {
      memcpy(tmpi, &(lmat_A_offdiag_->matrix.row[row_offs_[i]]),
             (nrows + 1) * sizeof(int));

      assert(tmpi[0] == 0);
      cudaMemcpy(row_offdiag_[i], tmpi, (nrows + 1) * sizeof(int),
                 cudaMemcpyHostToDevice);
    }
    ...
    for (int j = row_offs_[i]; j < row_offs_[i + 1]; ++j) {
      for (int k = lmat_A_diag_->matrix.row[j];
           k < lmat_A_diag_->matrix.row[j + 1]; ++k) {
        if (j != lmat_A_diag_->matrix.col[k])
          ++(nnz_diag_[i]);
      }
    }
    assert(nnz_diag_[i] == lmat_A_diag_->matrix.row[row_offs_[i + 1]] -
                               lmat_A_diag_->matrix.row[row_offs_[i]] - nrows);

    // ===================================================================
    // count nnz in offdiag part (anyway no diagonal elements)
    if (have_offdiag_) {
      nnz_offdiag_[i] = lmat_A_offdiag_->matrix.row[row_offs_[i + 1]] -
    ...
    if (have_offdiag_) {
      cudaMallocHost((void **)(&(xg_host[i])),
                     x.size_local_ghost() * sizeof(DataType));
      cudaMalloc((void **)(&(col_offdiag_[i])),
                 (nnz_offdiag_[i]) * sizeof(int));
      cudaMalloc((void **)(&(A_offdiag_gpu[i])),
                 (nnz_offdiag_[i]) * sizeof(DataType));
      cudaMalloc((void **)(&(xg_gpu[i])),
                 x.size_local_ghost() * sizeof(DataType));
    }
    ...
    // ===================================================================
    // CSR col arrays for diagonal and offdiagonal parts without diagonal
    // elements
    int ctr = 0;
    int inv_D_ctr = 0;
    for (int j = row_offs_[i]; j < row_offs_[i + 1]; ++j) {
      for (int k = lmat_A_diag_->matrix.row[j];
        ...
        if (j != lmat_A_diag_->matrix.col[k]) {
          ...
          tmpd[inv_D_ctr] =
              static_cast< DataType >(1.0) / lmat_A_diag_->matrix.val[k];
    ...
    cudaMemcpy(col_diag_[i], tmpi, (nnz_diag_[i]) * sizeof(int),
               cudaMemcpyHostToDevice);
    cudaMemcpy(inv_D_gpu[i], tmpd, nrows * sizeof(DataType),
               cudaMemcpyHostToDevice);
    ...
    if (compute_residual_on_dev_) {
      ...
      assert(ctr == nrows);
      cudaMemcpy(D_gpu[i], tmpd, nrows * sizeof(DataType),
                 cudaMemcpyHostToDevice);
    }

    if (have_offdiag_) {
      int idx_start = lmat_A_offdiag_->matrix.row[row_offs_[i]];
      memcpy(tmpi, &(lmat_A_offdiag_->matrix.col[idx_start]),
             (nnz_offdiag_[i]) * sizeof(int));
      cudaMemcpy(col_offdiag_[i], tmpi, (nnz_offdiag_[i]) * sizeof(int),
                 cudaMemcpyHostToDevice);
    }
    ...
    assert(ctr == nnz_diag_[i]);
    cudaMemcpy(A_diag_gpu[i], tmpd, (nnz_diag_[i]) * sizeof(DataType),
               cudaMemcpyHostToDevice);

    if (have_offdiag_) {
      int idx_start = lmat_A_offdiag_->matrix.row[row_offs_[i]];
      memcpy(tmpd, &(lmat_A_offdiag_->matrix.val[idx_start]),
             (nnz_offdiag_[i]) * sizeof(DataType));

      cudaMemcpy(A_offdiag_gpu[i], tmpd, (nnz_offdiag_[i]) * sizeof(DataType),
                 cudaMemcpyHostToDevice);
    }
...
template < class LAD > void AsynchronousIterationGPU< LAD >::CopyHostToDev_x() {
#ifdef WITH_CUDA
  assert(lvec_x_ != 0);
  ...
  for (int i = 0; i < used_gpus_; ++i) {
    ...
    if (have_offdiag_) {
      assert(lvec_xg_ != 0);
      memcpy(xg_host[i], lvec_xg_->buffer,
             lvec_xg_->get_size() * sizeof(DataType));
    }
  }
  ...
  for (int i = 0; i < used_gpus_; ++i) {
    ...
    if (have_offdiag_) {
      cudaMemcpyAsync(xg_gpu[i], xg_host[i],
                      lvec_xg_->get_size() * sizeof(DataType),
                      cudaMemcpyHostToDevice, stream_[i]);
    }
  }
...
void AsynchronousIterationGPU< LAD >::CopyHostToDev_b_x() {
  ...
  for (int i = 0; i < used_gpus_; ++i) {
    memcpy(b_host[i], &(lvec_b_->buffer[row_offs_[i]]),
  ...
  for (int i = 0; i < used_gpus_; ++i) {
    cudaSetDevice(i);
    cudaMemcpyAsync(b_gpu[i], b_host[i],
                    (row_offs_[i + 1] - row_offs_[i]) * sizeof(DataType),
                    cudaMemcpyHostToDevice, stream_[i]);
    cudaMemcpyAsync(x_gpu[i], x_host[i], lvec_x_->get_size() * sizeof(DataType),
                    cudaMemcpyHostToDevice, stream_[i]);
...
template < class LAD > void AsynchronousIterationGPU< LAD >::CopyDevToHost_x() {
#ifdef WITH_CUDA
  assert(lvec_x_ != 0);
  ...
  for (int ii = 0; ii < n_global_iter_; ++ii) {
    for (int jj = 0; jj < n_block_iter_; ++jj) {
      ...
      for (int i = 0; i < used_gpus_; ++i) {
        ...
        if (grid_dim_ * block_dim_ < nrows)
          ++grid_dim_;

        if (have_offdiag_) {
          cuda_async_inner_iter(A_diag_gpu[i], row_diag_[i], col_diag_[i],
                                nrows, row_offs_[i], A_offdiag_gpu[i],
                                row_offdiag_[i], col_offdiag_[i], inv_D_gpu[i],
                                b_gpu[i], x_gpu[i], xg_gpu[i], n_inner_iter_,
                                grid_dim_, block_dim_, stream_[i]);
        } else {
          cuda_async_inner_iter_no_offdiag(
              A_diag_gpu[i], row_diag_[i], col_diag_[i], nrows, row_offs_[i],
              inv_D_gpu[i], b_gpu[i], x_gpu[i], n_inner_iter_, grid_dim_,
              block_dim_, stream_[i]);
        }
      }
      ...
      if ((used_gpus_ > 1) && (jj < n_block_iter_ - 1))
        BlockUpdate();
    }
    // update across MPI procs
    if ((nproc_ > 1) && (ii < n_global_iter_ - 1))
      GlobalUpdate(x);
  }
#endif
}
  ...
  for (int ii = 0; ii < n_global_iter_; ++ii) {
    for (int jj = 0; jj < n_block_iter_; ++jj) {
      ...
      for (int i = 0; i < used_gpus_; ++i) {
        ...
        if (have_offdiag_) {
          cuda_async_inner_damped_iter(
              A_diag_gpu[i], row_diag_[i], col_diag_[i], nrows, row_offs_[i],
              A_offdiag_gpu[i], row_offdiag_[i], col_offdiag_[i], inv_D_gpu[i],
              b_gpu[i], x_gpu[i], xg_gpu[i], w_, n_inner_iter_, grid_dim_,
              block_dim_, stream_[i]);
        } else {
          cuda_async_inner_damped_iter_no_offdiag(
              A_diag_gpu[i], row_diag_[i], col_diag_[i], nrows, row_offs_[i],
              inv_D_gpu[i], b_gpu[i], x_gpu[i], w_, n_inner_iter_, grid_dim_,
              block_dim_, stream_[i]);
        }
      }
      ...
      // update inner
      if (jj < n_block_iter_ - 1)
        BlockUpdate();
    }
    if (ii < n_global_iter_ - 1)
      GlobalUpdate(x);
  }
#endif
}
...
void AsynchronousIterationGPU< LAD >::ComputeResidualOnDev() {
  ...
  MPI_Allreduce(&local_res, &res, 1, mpi_data_type< DataType >::get_type(),
                MPI_SUM, comm_);
...
AsynchronousIterationGPU< LAD >::ComputeBlockSquaredResidualOnDev() {
#ifdef WITH_CUDA
  // launch kernel on each GPU
  for (int i = 0; i < used_gpus_; ++i) {
    cudaSetDevice(i);
    ...
    if (have_offdiag_) {
      cuda_compute_block_squared_residual(
          A_diag_gpu[i], row_diag_[i], col_diag_[i], nrows, row_offs_[i],
          A_offdiag_gpu[i], row_offdiag_[i], col_offdiag_[i], D_gpu[i],
          b_gpu[i], x_gpu[i], xg_gpu[i], block_res_gpu[i], grid_dim_,
          block_dim_, max_threads_per_block_, stream_[i]);
    } else {
      cuda_compute_block_squared_residual_no_offdiag(
          A_diag_gpu[i], row_diag_[i], col_diag_[i], nrows, row_offs_[i],
          D_gpu[i], b_gpu[i], x_gpu[i], block_res_gpu[i], grid_dim_, block_dim_,
          max_threads_per_block_, stream_[i]);
    }

    cudaMemcpyAsync(&(block_res_host[i]), block_res_gpu[i], sizeof(DataType),
                    cudaMemcpyDeviceToHost, stream_[i]);
  }
...
template < class LAD >
LinearSolverState
AsynchronousIterationGPU< LAD >::SolveImpl(const VectorType &b, VectorType *x) {
  assert(block_dim_ > 0);
...
template < class LAD >
LinearSolverState
AsynchronousIterationGPU< LAD >::SolveNormal(const VectorType &b,
                                             VectorType *x) {
#ifdef WITH_CUDA
  this->iter_ = 0;
  ...
  IterateControl::State iterctrl =
      this->control().Check(this->iter_, this->res_);
  ...
  if (this->print_level_ > 0) {
    LOG_INFO("AsynchronousIterationGPU",
             " normal solve stops after " << this->iter_
                                          << " iterations with residual norm "
                                          << this->res_);
  }
...
template < class LAD >
LinearSolverState
AsynchronousIterationGPU< LAD >::SolveDamped(const VectorType &b,
                                             VectorType *x) {
  ...
  if (this->print_level_ > 0) {
    LOG_INFO("AsynchronousIterationGPU",
             " damped solve stops after " << this->iter_
                                          << " iterations with residual norm "
                                          << this->res_);
  }
...
template < class LAD >
LinearSolverState
AsynchronousIterationGPU< LAD >::SmoothNormal(const VectorType &b,
                                              VectorType *x) {
#ifdef WITH_CUDA
  x->UpdateGhost();
...
template < class LAD >
LinearSolverState
AsynchronousIterationGPU< LAD >::SmoothDamped(const VectorType &b,
                                              VectorType *x) {
...
template < class LAD > void AsynchronousIterationGPU< LAD >::BlockUpdate() {
#ifdef WITH_CUDA
  if (used_gpus_ > 1) {
    if (enable_p2p_) {
      ...
      for (int i = 0; i < used_gpus_; ++i) {
        cudaSetDevice(i);
        cudaMemcpyAsync(x_gpu[i], x_host[i],
                        lvec_x_->get_size() * sizeof(DataType),
                        cudaMemcpyHostToDevice, stream_[i]);
      }
    }
  }
...
template < class LAD > void AsynchronousIterationGPU< LAD >::Clear() {
#ifdef WITH_CUDA
  ...
  if (A_offdiag_gpu != 0) {
    for (int i = 0; i < used_gpus_; ++i) {
      cudaSetDevice(i);
      if (A_offdiag_gpu[i] != 0)
        cudaFree(A_offdiag_gpu[i]);
    }
  ...
  if (block_res_gpu != 0) {
    for (int i = 0; i < used_gpus_; ++i) {
      cudaSetDevice(i);
      if (block_res_gpu[i] != 0)
        cudaFree(block_res_gpu[i]);
    }
