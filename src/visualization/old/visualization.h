...
#ifndef HIFLOW_VISU_VISUALIZATION
#define HIFLOW_VISU_VISUALIZATION
...
public:
  ...
  Visualization(const MPI_Comm &comm)
  ...
  virtual void visualize_mesh (CoordTrafoFunction const * trafo = nullptr) = 0;
  
  virtual void visualize_cell_data(const std::vector< DataType > &cell_data, const std::string &name) = 0;
  ...
  std::map<std::string, std::vector< Vec<DIM,DataType> > > & get_grad_values()
  {
...
protected:
  ...
  mutable std::map< std::string, std::vector< Vec<DIM,DataType> > > functions_grad_;
...
template <class DataType, int DIM>
void Visualization<DataType, DIM>::communicate_data(int num_write_procs) const 
{
  int my_rank = -1;
  int num_proc = -1;
