...
public:
  ...
  explicit VTKWriter(Visualization<DataType, DIM>& visu, 
...
protected:
  ...
  const Visualization<DataType, DIM>& visu_;
  
  std::map< std::string, std::vector< DataType > >& functions_;
  std::map< std::string, std::vector< Vec<DIM,DataType> > > &functions_grad_;
