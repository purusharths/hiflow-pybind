...
template < class LAD > class EvalFeFunction {
  ...
  typedef boost::function3<
      ...
      std::vector< typename LAD::DataType > & // values of function at the
                                              // points
      >
      EvalFunction;
...
public:
  ...
  void operator()(const Entity &cell,
    ...
    space_.get_dof_indices(var_, cell, &global_dof_ids);
    const int num_dofs = global_dof_ids.size();

    std::vector< std::vector< typename LAD::DataType > > shape_fun(
        num_points, std::vector< typename LAD::DataType >(num_dofs, 1.e13));
    ...
    for (int i = 0; i < num_points; ++i) {
      ...
      space_.fe_manager()
          .get_fe_on_cell(cell.index(), var_)
          ->N(pt, shape_fun[i]);
    }
    ...
    if (num_procs == 1) {
      ...
      fun_.GetValues(&(global_dof_ids[0]), global_dof_ids.size(),
                     &(dof_values[0]));
    } else {
...
template < class LAD, int DIM > class EvalDerivativeFeFunction {
  typedef hiflow::doffem::DofID DofID;
...
public:
  ...
  void operator()(const Entity &cell,
    ...
    FunctionValues< std::vector< Vec< DIM, typename LAD::DataType > > >
        grad_phi_hat;
    grad_phi_hat.compute(
        q_points,
        EvalShapeFunctionGradients< DIM, typename LAD::DataType >(fe_types));

    // compute jacobians on physical cell
    FunctionValues< Mat< DIM, DIM, typename LAD::DataType > > J;
    const CellTransformation< typename LAD::DataType > &cell_transform =
        this->space_.get_cell_transformation(cell);

    J.compute(q_points, EvalPhysicalJacobian< DIM, typename LAD::DataType >(
                            cell_transform));
    ...
    FunctionValues< std::vector< Vec< DIM, typename LAD::DataType > > >
        grad_phi;
    grad_phi.compute(
        JinvT, EvalMappedShapeFunctionGradients< DIM, typename LAD::DataType >(
                   grad_phi_hat));
