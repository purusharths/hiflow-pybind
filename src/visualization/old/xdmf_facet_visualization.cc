...
#include "xdmf_facet_visualization.h"
...
void XdmfFacetVisualization<DataType>::clear_topology_data() {
  this->xdmf_topology_.clear();
}

// clear
template <class DataType>
void XdmfFacetVisualization<DataType>::clear() {
...
template <class DataType>
void XdmfFacetVisualization<DataType>::get_global_dimensions() {
...
// get offsets
template <class DataType>
void XdmfFacetVisualization<DataType>::get_offsets() {

  // mpi rank
  int my_rank = -1;
  int size_rank = -1;
  MPI_Comm_rank(this->mpi_comm_, &my_rank);
  MPI_Comm_size(this->mpi_comm_, &size_rank);
  ...
  for (typename std::map<std::string, std::vector<DataType> >::const_iterator it =
    ...
    if (my_rank > 0) {
      MPI_Status status;
      MPI_Recv(&offset_attr, 1, MPI_INT, my_rank - 1, my_rank - 1, this->mpi_comm_,
               &status);
    }
...
template <class DataType>
void XdmfFacetVisualization<DataType>::create_xdmf_topology(
...
/// \details This function write the xdmf file and hdf5 data. The simplest
...
template <class DataType>
void XdmfFacetVisualization<DataType>::write(const std::string& filename,
  ...
  // write solutions
  H5FilePtr file_ptr(new H5File(filepath + name_hdf5, "w", this->mpi_comm_));
...
template <class DataType>
void XdmfFacetVisualization<DataType>::write_xdmf_dataitem(tinyxml2::XMLElement*
...
template class XdmfFacetVisualization<double>;
template class XdmfFacetVisualization<float>;
}
