...
public:
  ...
  void transform (Vec<DIM, DataType>& pt) const 
  {}
                                                       ...
                                                       const std::vector<std::string> &names )
{
       ...
       it != end_it; ++it) 
  {
   ...
   //   continue;
    ...
    {
      Vec<DIM, DataType> phys_coords;
      Vec<DIM, DataType> ref_coords;
                                                           ...
                                                           const std::vector<std::string> &names)
{
  ...
  std::vector< Vec<DIM, DataType> > cell_values_grad(nb_var); 
...
void CellVisualization<DataType, DIM>::visualize_mesh(CoordTrafo * trafo) 
{
       ...
       it != end_it; ++it) 
  {
    ...
    std::shared_ptr<const doffem::CellTransformation<DataType, DIM> > cell_trans = this->space_.get_cell_transformation(it->index());
