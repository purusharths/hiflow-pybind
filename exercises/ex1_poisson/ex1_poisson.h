    ...
    for (int q = 0; q < num_q; ++q) 
    {
      ...
      { 
        ...
        for (int j = 0; j < num_dof; ++j) 
        {
          ...
          lm(i, j) += wq * 
                    (kappa_ * dot(this->grad_Phi(j, q, 0), this->grad_Phi(i, q, 0)) 
                     + beta_ * dot(conv, this->grad_Phi(j, q, 0)) * this->Phi(i, q, 0)
                     + gamma_ * this->Phi(j, q, 0) * this->Phi(i, q, 0))
                   * dJ;
